...
{
    public class ExpiryTradeICTest : ITest
    {
        //ExpiryTradeIC paTrader;
        ExpiryTradeIC paTrader;
        public void Execute(StraddleInput paInputs)
        {
        ...
        private ExpiryTradeIC ExecuteAlgo(StraddleInput paInputs)
        {
            ExpiryTradeIC paTrader =
                new ExpiryTradeIC(new TimeSpan(0, paInputs.CTF, 0), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID, paInputs.TP, paInputs.SL, true, paInputs.SPI, paInputs.TR,
        ...
        private void ObserverSubscription(ExpiryTradeIC paTrader)
        {
            //GlobalObjects.ObservableFactory ??= new ObservableFactory();
            //paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }

        //private async Task NMQClientSubscription(ExpiryTradeIC paTrader, uint token)
        //{
        ...
        //}
#if local
        private async Task KFKClientSubscription(ExpiryTradeIC paTrader, uint token)
        {
        ...
        //private async Task ObserverSubscription(ExpiryTradeIC paTrader, uint token)
        //{
        //    GlobalObjects.ObservableFactory ??= new ObservableFactory();
        //    paTrader.Subscribe(GlobalObjects.ObservableFactory);
        //}
        //private async Task MQTTCientSubscription(ExpiryTradeIC paTrader, uint token)
        //{
        ...
        //}
#endif
        //        private void PATrade_OnOptionUniverseChange(ExpiryTradeIC source)
        //        {
        //            try
        //            {
        ...
        //        {
        //            return Task.FromResult((int)AlgoIndex.ExpiryTradeIC);
        //        }

        //        [HttpPut("{ain}")]
        //        public bool Put(int ain, [FromBody] int start)
        //        {
        //            List<ExpiryTradeIC> activeAlgoObjects;
        //            if (!_cache.TryGetValue(key, out activeAlgoObjects))
        //            {
        //                activeAlgoObjects = new List<ExpiryTradeIC>();
        //            }

        //            ExpiryTradeIC algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
        //            if (algoObject != null)
        //            {
