...
export class TradeComponent implements OnInit, AfterViewInit {
  ...
  optionsellonemaForm: FormGroup;
  multitimeframesellonhtForm: FormGroup;
  cryptodirectionalFOForm: FormGroup;
  cryptoActiveStrangleBuyForm: FormGroup;
  tj3Form: FormGroup;
  tj4Form: FormGroup;
  tj5Form: FormGroup;
  bcForm: FormGroup;
  ...
  constructor(private http: HttpClient, private formBuilder: FormBuilder,
    ...
    http.get<Instrument[]>(baseUrl + 'api/optionsellonema').subscribe(result => {
      this.instrument = result;
    }, error => console.error(error));

    http.get<Instrument[]>(baseUrl + 'api/pricedirectionalfutureoption').subscribe(result => {
      this.instrument = result;
    }, error => console.error(error));
    http.get<Instrument[]>(baseUrl + 'api/activestranglebuy').subscribe(result => {
      this.instrument = result;
    }, error => console.error(error));
    ...
    http.get<any>(baseUrl + 'api/optionsellonema/activealgos').subscribe(result => {
    ...
    http.get<any>(baseUrl + 'api/pricedirectionalfutureoption/activealgos').subscribe(result => {
      if (this.orders == null || this.orders.length == 0) {
        this.activeAlgos = result;
      }
      else {
    ...
    http.get<any>(baseUrl + 'api/activestranglebuy/activealgos').subscribe(result => {
      if (this.orders == null || this.orders.length == 0) {
        this.activeAlgos = result;
      }
      else {
  ...
  getArrayLength(array: any[] | null | undefined): number {
  ...
  ngOnInit(): void {
    ...
    this.cryptodirectionalFOForm = this.formBuilder.group({
       qty: [1, Validators.required]
      , sl: [50, Validators.required]
      , tp: [100, Validators.required]
      , uid: ['', Validators.required]
    });
    this.cryptoActiveStrangleBuyForm = this.formBuilder.group({
    ...
    this.optionsellonemaForm = this.formBuilder.group({
      ctf: [5, Validators.required]
      , uid: ['', Validators.required]
      , tp: [100000, Validators.required]
      , qty: [1, Validators.required]
  ...
  executeOptionSellOnEMA() {
    const data = {
      btoken: this.selectedInst.value,
      expiry: this.selectedExpiry,
      ctf: this.optionsellonemaForm.value.ctf,
      uid: this.optionsellonemaForm.value.uid,
      tp: this.optionsellonemaForm.value.tp,
      qty: this.optionsellonemaForm.value.qty,
      intd: this.optionsellonemaForm.value.intd
    }
    if (data.intd == "") { data.intd = false }
    this.http.post<ActiveAlgo>(this._baseUrl + 'api/optionsellonema', data).subscribe(result => {
      this.activeAlgos.push(result);
    }, error => console.error(error));
  }
  ...
  executeCryptoDirectionalFutureOption() {
    const data = {
      btoken: this.selectedInst.value,
      qty: this.cryptodirectionalFOForm.value.qty,
      sl: this.cryptodirectionalFOForm.value.sl,
      tp: this.cryptodirectionalFOForm.value.tp,
      uid: this.cryptodirectionalFOForm.value.uid,
    }
    this.http.post<ActiveAlgo>(this._baseUrl + 'api/pricedirectionalfutureoption', data).subscribe(result => {
      this.activeAlgos.push(result);
    }, error => console.error(error));
  }
  executeCryptoActiveStrangleBuy() {
    const data = {
      btoken: this.selectedInst.value,
      qty: this.cryptoActiveStrangleBuyForm.value.qty,
      sl: this.cryptoActiveStrangleBuyForm.value.sl,
      tp: this.cryptoActiveStrangleBuyForm.value.tp,
      uid: this.cryptoActiveStrangleBuyForm.value.uid,
    }
    this.http.post<ActiveAlgo>(this._baseUrl + 'api/activestranglebuy', data).subscribe(result => {
      this.activeAlgos.push(result);
    }, error => console.error(error));
  }
  ...
  startstopalgo(ain, aid, s) {
    ...
    if (aid == "52") {
      ...
      ctrl = "pricedirectionalfutureoption";
    }
    else if (aid == "57") {
      ctrl = "activestranglebuy";
    }
    else if (aid == "1032") {
      ctrl = "optionsellonema";
    }
    const start = s;
