...
cdk.$overlay-container-z-index: 1000 !default;
cdk.$overlay-z-index: 1000 !default;
cdk.$overlay-backdrop-z-index: 1000 !default;

// Background color for all of the backdrops
cdk.$overlay-backdrop-color: rgba(0, 0, 0, 0.32) !default;
...
@mixin cdk-overlay() {
  ...
  .cdk-overlay-backdrop {
    ...
    z-index: cdk.$overlay-backdrop-z-index;
    pointer-events: auto;
...
@mixin cdk-text-field {
  ...
  // by watching for the animation events that are fired when they start. Note: the __<<ngThemingMigrationEscapedComment0>>__ comment is
...
@mixin _mat-theme-elevation($zValue, $config, $opacity: 1) {
...
@mixin _mat-theme-overridable-elevation($zValue, $config, $opacity: 1) {
  $foreground: map-get($config, foreground);
  $elevation-color: map-get($foreground, elevation);
  $elevation-color-or-default: if($elevation-color == null, #000, $elevation-color);
...
mat.$gray-palette: mat.$grey-palette;
...
// Alias for alternate spelling.
mat.$blue-gray-palette: mat.$blue-grey-palette;
...
// @param $color-map
// @param $hue
@function mat.get-contrast-color-from-palette($palette, $hue) {
...
// @param $primary
// @param $lighter
@function mat.define-palette($base-palette, $default: 500, $lighter: 100, $darker: 700, $text: $default) {
  $result: map_merge($base-palette, (
    ...
    default-contrast: mat.get-contrast-color-from-palette($base-palette, $default),
    lighter-contrast: mat.get-contrast-color-from-palette($base-palette, $lighter),
    darker-contrast: mat.get-contrast-color-from-palette($base-palette, $darker)
  ));

  // For each hue in the palette, add a "-contrast" color to the map.
  @each $hue, $color in $base-palette {
    $result: map_merge($result, (
      '#{$hue}-contrast': mat.get-contrast-color-from-palette($base-palette, $hue)
    ));
  }
...
@function mat.define-light-theme($primary, $accent: null, $warn: mat.define-palette(mat.$red-palette)) {
...
// TODO: Remove legacy API and rename `$primary` to `$config`. Currently it cannot be renamed
// as it would break existing apps that set the parameter by name.
@function mat.define-dark-theme($primary, $accent: null, $warn: mat.define-palette(mat.$red-palette)) {
  // This function creates a container object for the individual component theme mixins. Consumers
  // can construct such an object by calling this function, or by building the object manually.
  // There are two possible ways to invoke this function in order to create such an object:
  //
...
@mixin mat-strong-focus-indicators-color($config-or-theme) {
  $config: mat-get-color-config($config-or-theme);
  @include _mat-strong-focus-indicators-border-color(mat.get-color-from-palette(map_get($config, primary)));
}
...
@mixin mat-pseudo-checkbox-color($config-or-theme) {
  ...
  .mat-pseudo-checkbox {
    color: mat.get-color-from-palette(map-get($config, foreground), secondary-text);
  ...
  .mat-primary .mat-pseudo-checkbox-indeterminate {
    background: mat.get-color-from-palette(map-get($config, primary));
  }
  ...
  .mat-accent .mat-pseudo-checkbox-indeterminate {
    background: mat.get-color-from-palette(map-get($config, accent));
  }

  .mat-warn .mat-pseudo-checkbox-checked,
  .mat-warn .mat-pseudo-checkbox-indeterminate {
    background: mat.get-color-from-palette(map-get($config, warn));
  }
...
@function mat.define-typography-level(
  $font-size,
...
$mat-badge-small-size: 22px - 6;
$mat-badge-large-size: 22px + 6;
...
@mixin mat-badge-color($config-or-theme) {
  ...
  .mat-badge-small {
    @include _mat-badge-size(16px);
  }
  .mat-badge-medium {
    @include _mat-badge-size(22px);
  }
  .mat-badge-large {
    @include _mat-badge-size(28px);
  }
}
...
$mat-button-toggle-standard-maximum-height: 48px !default;
...
@mixin _mat-chip-theme-color($palette) {
  @include _mat-chip-element-color(mat.get-color-from-palette($palette, default-contrast), mat.get-color-from-palette($palette));
...
@mixin _mat-datepicker-color($palette) {
  @include mat-date-range-colors(
    mat.get-color-from-palette($palette, default, 0.2));
  ...
  .mat-calendar-body-disabled > .mat-calendar-body-selected {
    ...
    @if (type-of($background) == color) {
      background-color: fade-out($background, 0.6);
    }
    @else {
...
@mixin mat-datepicker-color($config-or-theme) {
  ...
  .mat-calendar-body-disabled > .mat-calendar-body-today:not(.mat-calendar-body-selected) {
    ...
    @if (type-of($color) == color) {
      border-color: fade-out($color, 0.2);
    }
    @else {
...
@mixin mat-expansion-panel-theme($theme-or-color-config) {
  ...
  @include _mat-check-duplicate-theme-styles($theme, 'mat-expansion-panel') {
    ...
    @if $color != null {
      @include mat.expansion-color($color);
    }
    @if $density != null {
...
$mat-paginator-maximum-height: 56px !default;
...
@mixin mat-sidenav-color($config-or-theme) {
  ...
  .mat-drawer-backdrop.mat-drawer-shown {
    $opacity: 0.6;
    $backdrop-color: mat.get-color-from-palette($background, card, $opacity);
...
@mixin mat-slide-toggle-color($config-or-theme) {
  ...
  .mat-slide-toggle-thumb {
    ...
    background-color: mat.get-color-from-palette(mat.$grey-palette, $thumb-unchecked-hue);
  }
...
@mixin mat-slider-color($config-or-theme) {
  ...
  $mat-slider-tick-color: mat.get-color-from-palette($foreground, base, $mat-slider-tick-opacity);
...
$mat-stepper-header-maximum-height: 72px !default;
...
@mixin _mat-tabs-background($background-color) {
  ...
  .mat-tab-header-pagination-disabled .mat-tab-header-pagination-chevron {
    border-color: mat.get-color-from-palette($background-color, default-contrast, 0.4);
  }

  // Set ripples color to be the contrast color of the new background. Otherwise the ripple
  // color will be based on the app background color.
  .mat-ripple-element {
    background-color: mat.get-color-from-palette($background-color, default-contrast, 0.12);
  }
}
...
$mat-toolbar-maximum-height-desktop: 64px !default;
44px-desktop: 44px !default;

$mat-toolbar-height-mobile: 56px !default;
$mat-toolbar-maximum-height-mobile: 56px !default;
44px-mobile: 44px !default;
...
$mat-tooltip-vertical-padding: (22px - 10px) / 2;
...
@mixin mat-form-field-fill-color($config-or-theme) {
  ...
  $fill-disabled-background: mat.get-color-from-palette($foreground, base, if($is-dark-theme, 0.05, 0.02));
...
@mixin mat-form-field-color($config-or-theme) {
  ...
  $focused-label-color: mat.get-color-from-palette($primary, text);
  ...
  $underline-color-accent: mat.get-color-from-palette($accent, text);
...
$mat-tree-node-maximum-height: 48px !default;
