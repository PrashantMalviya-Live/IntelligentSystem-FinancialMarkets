...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class EMAScalpingKbController : ControllerBase
    {
        ...
        public EMAScalpingKbController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            EMAScalpingKB paTrader = await ExecuteAlgo(paInputs);
            ...
            List<EMAScalpingKB> activeAlgoObjects = _cache.Get<List<EMAScalpingKB>>(key);
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.EMAScalpingKB),
                an = Convert.ToString((AlgoIndex)AlgoIndex.EMAScalpingKB),
        ...
        private async Task<EMAScalpingKB> ExecuteAlgo(PriceActionInput paInputs)
        {
            EMAScalpingKB paTrader =
                new EMAScalpingKB(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Qty, paInputs.UID, httpClientFactory: _httpClientFactory);
