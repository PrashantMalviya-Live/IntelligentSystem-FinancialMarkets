    ...
    public class RSITradeController : ControllerBase
    {
        ...
        public async Task<IEnumerable<ActiveAlgosView>> GetActiveAlgos()
        {
            ...
            foreach (DataRow drAlgo in dtActiveAlgos.Rows)
            {
                ...
                var lastPutOrder = orders.Where(x => x.OrderTimestamp == lastCallOrderTime && x.Tradingsymbol.TakeLast(2).First().ToString().ToLower() == "p").FirstOrDefault();
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] OptionBuywithRSIInput optionBuywithRSIInput, int algoInstance = 0)
        {
            ...
            OptionBuyWithRSICandle optionBuywithRSI =
                ...
                optionBuywithRSIInput.EMA, optionBuywithRSIInput.DCHL, algoInstance, optionBuywithRSIInput.EAC, false, 0);
            ...
            List<OptionBuyWithRSICandle> activeAlgoObjects = _cache.Get<List<OptionBuyWithRSICandle>>(key);
        ...
        {
            List<OptionBuyWithRSICandle> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<OptionBuyWithRSICandle>();
            }

            OptionBuyWithRSICandle algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
