...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class TJ4Controller : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_TJ4_OBJECT";
        ...
        public TJ4Controller(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            TJ4 paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.TJ4, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<TJ4> activeAlgoObjects = _cache.Get<List<TJ4>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<TJ4>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.TJ4),
                an = Convert.ToString((AlgoIndex)AlgoIndex.TJ4),
        ...
        private async Task<TJ4> ExecuteAlgo(PriceActionInput paInputs)
        {
            ...
            TJ4 paTrader =
                new TJ4(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.RToken.Value, paInputs.Expiry, paInputs.Qty, paInputs.UID, paInputs.TP, paInputs.SL, //pdOHLC, pwOHLC,
        ...
        private async Task NMQClientSubscription(TJ4 paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(TJ4 paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(TJ4 paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(TJ4 paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(TJ4 source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.TJ4);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<TJ4> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<TJ4>();
            }

            TJ4 algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
