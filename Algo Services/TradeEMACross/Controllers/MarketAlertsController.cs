...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class MarketAlertsController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_MAC_OBJECT";
        ...
        public MarketAlertsController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            StockAlerts paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            ...
            List<StockAlerts> activeAlgoObjects = _cache.Get<List<StockAlerts>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<StockAlerts>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.MA),
                an = Convert.ToString((AlgoIndex)AlgoIndex.MA),
        ...
        private async Task<StockAlerts> ExecuteAlgo(PriceActionInput paInputs)
        {
            StockAlerts paTrader =
                new StockAlerts(TimeSpan.FromMinutes(5), TimeSpan.FromMinutes(120), paInputs.BToken, paInputs.Qty, paInputs.UID, httpClientFactory: _httpClientFactory);
        ...
        private async Task NMQClientSubscription(StockAlerts paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(StockAlerts paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(StockAlerts paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(StockAlerts paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(StockAlerts source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.MA);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<StockAlerts> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<StockAlerts>();
            }

            StockAlerts algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
