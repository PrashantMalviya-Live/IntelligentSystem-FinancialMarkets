...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class IVTradeController : ControllerBase
    {
        ...
        public IVTradeController(IMemoryCache cache)
        {
            this._cache = cache;
        }
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] OptionIVSpreadInput optionIVSpreadInput, int algoInstance = 0)
        {
            ...
            CalendarSpreadIVScalping calendarSpreadIVScalper =
                new CalendarSpreadIVScalping(endDateTime, instrumentToken, expiry1, expiry2,
                optionQuantity, targetProfit: optionIVSpreadInput.TP, stopLoss: 0, // optionIVSpreadInput.SL,
            ...
            List<CalendarSpreadIVScalping> activeAlgoObjects = _cache.Get<List<CalendarSpreadIVScalping>>(okey);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<CalendarSpreadIVScalping>();
            }
        ...
        private async Task NMQClientSubscription(CalendarSpreadIVScalping buyWithRSI, uint token)
        {
        ...
        private void OptionBuywithRSI_OnOptionUniverseChange(CalendarSpreadIVScalping source)
        {
            try
            {
        ...
        {
            List<CalendarSpreadIVScalping> activeAlgoOptions;
            if (_cache.TryGetValue(okey, out activeAlgoOptions))
            {
                CalendarSpreadIVScalping algoObject = activeAlgoOptions.FirstOrDefault(x => x.AlgoInstance == ain);
                if (algoObject != null)
                {
