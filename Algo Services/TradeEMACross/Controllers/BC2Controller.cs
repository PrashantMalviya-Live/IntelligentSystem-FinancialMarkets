...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class BC2Controller : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_BC2_OBJECT";
        ...
        public BC2Controller(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        public async Task<IEnumerable<ActiveAlgosView>> GetActiveAlgos()
        {
            try
            {
                ...
                DataLogic dl = new DataLogic();
                DataSet ds = dl.GetActiveAlgos(AlgoIndex.BC2, userId);
                ...
                foreach (DataRow drAlgo in dtActiveAlgos.Rows)
                {
                    ...
                    PriceActionInput algoInput = new PriceActionInput
                    {
        ...
        {
            BC paTrader = await ExecuteAlgo(paInputs, algoInstance);

            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.BC2, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            paTrader.OnTradeEntry += BC2_OnTradeEntry;
            paTrader.OnTradeExit += BC2_OnTradeExit;

            List<BC> activeAlgoObjects = _cache.Get<List<BC>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<BC>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.BC2),
                an = Convert.ToString((AlgoIndex)AlgoIndex.BC2),
        ...
        private async Task<BC> ExecuteAlgo(PriceActionInput paInputs, int algoInstance)
        {
            ...
            BC paTrader =
                new BC(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID,
        ...
        private void BC2_OnTradeExit(Order st)
        {
        ...
        private void BC2_OnTradeEntry(Order st)
        {
            //publish trade details and count
            OrderCore.PublishOrder(st);
            Thread.Sleep(100);
        ...
        private async Task NMQClientSubscription(BC paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(BC paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(BC paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(BC paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(BC source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.BC2);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<BC> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<BC>();
            }

            BC algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
