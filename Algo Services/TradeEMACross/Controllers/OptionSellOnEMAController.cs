...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class OptionSellOnEMAController : ControllerBase
    {
        ...
        public OptionSellOnEMAController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            OptionSellOnEMA paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.OptionSellOnEMA, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<OptionSellOnEMA> activeAlgoObjects = _cache.Get<List<OptionSellOnEMA>>(key);
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.OptionSellOnEMA),
                an = Convert.ToString((AlgoIndex)AlgoIndex.OptionSellOnEMA),
        ...
        private async Task<OptionSellOnEMA> ExecuteAlgo(PriceActionInput paInputs)
        {
            ...
            OptionSellOnEMA paTrader =
                new OptionSellOnEMA(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID, paInputs.TP, paInputs.SL, 
                true, 0, //pdOHLC, pwOHLC,
        ...
        public async Task<IEnumerable<ActiveAlgosView>> GetActiveAlgos()
        {
            try
            {
                ...
                DataLogic dl = new DataLogic();
                DataSet ds = dl.GetActiveAlgos(AlgoIndex.OptionSellOnEMA, userId);
