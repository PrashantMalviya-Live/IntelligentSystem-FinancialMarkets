...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class OptionSellOnHTController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_OptionSellOnHT_OBJECT";
        ...
        public OptionSellOnHTController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        public async Task<IEnumerable<ActiveAlgosView>> GetActiveAlgos()
        {
            try
            {
                ...
                DataLogic dl = new DataLogic();
                DataSet ds = dl.GetActiveAlgos(AlgoIndex.OptionSellonHT, userId);
        ...
        {
            OptionSellonHT paTrader = await ExecuteAlgo(paInputs, algoInstance);

            paTrader.LoadActiveOrders(paInputs.ActiveOrderTrios);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.OptionSellonHT, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<OptionSellonHT> activeAlgoObjects = _cache.Get<List<OptionSellonHT>>(key);
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.OptionSellonHT),
                an = Convert.ToString((AlgoIndex)AlgoIndex.OptionSellonHT),
        ...
        private async Task<OptionSellonHT> ExecuteAlgo(PriceActionInput paInputs, int algoInstance)
        {
            ...
            OptionSellonHT paTrader =
                new OptionSellonHT(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID, 
                ...
                positionSizing: false, maxLossPerTrade: 0, httpClientFactory: _httpClientFactory, algoInstance: algoInstance);// firebaseMessaging);
