...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class TJ5Controller : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_TJ5_OBJECT_1";
        ...
        public TJ5Controller(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            TJ5 paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.TJ5, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<TJ5> activeAlgoObjects = _cache.Get<List<TJ5>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<TJ5>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.TJ5),
                an = Convert.ToString((AlgoIndex)AlgoIndex.TJ5),
        ...
        private async Task<TJ5> ExecuteAlgo(PriceActionInput paInputs)
        {
            ...
            TJ5 paTrader =
                new TJ5(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.RToken.Value, paInputs.Expiry, paInputs.Qty, paInputs.UID, paInputs.TP, paInputs.SL, //pdOHLC, pwOHLC,
        ...
        private async Task NMQClientSubscription(TJ5 paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(TJ5 paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(TJ5 paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(TJ5 paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(TJ5 source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.TJ5);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<TJ5> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<TJ5>();
            }

            TJ5 algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
