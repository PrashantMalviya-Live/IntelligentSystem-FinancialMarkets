...
namespace TradeEMACross.Controllers
{
    ...
    public class BCController : ControllerBase
    {
        ...
        public BCController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            BreakoutCandles paTrader = await ExecuteAlgo(paInputs, algoInstance);

            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.BreakoutCandles, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<BreakoutCandles> activeAlgoObjects = _cache.Get<List<BreakoutCandles>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<BreakoutCandles>();
            }
        ...
        private async Task<BreakoutCandles> ExecuteAlgo(PriceActionInput paInputs, int algoInstance)
        {
            ...
            BreakoutCandles paTrader =
                new BreakoutCandles(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID,
        ...
        private async Task NMQClientSubscription(BreakoutCandles paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(BreakoutCandles paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(BreakoutCandles paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(BreakoutCandles paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(BreakoutCandles source)
        {
            try
            {
        ...
        {
            List<BreakoutCandles> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<BreakoutCandles>();
            }

            BreakoutCandles algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
