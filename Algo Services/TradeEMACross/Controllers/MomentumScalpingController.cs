...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class MomentumScalpingController : ControllerBase
    {
        ...
        public MomentumScalpingController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            SchScalping paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.SchScalping, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<SchScalping> activeAlgoObjects = _cache.Get<List<SchScalping>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<SchScalping>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.SchScalping),
                an = Convert.ToString((AlgoIndex)AlgoIndex.SchScalping),
        ...
        private async Task<SchScalping> ExecuteAlgo(PriceActionInput paInputs)
        {
            ...
            SchScalping paTrader =
                new SchScalping(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.TP, paInputs.SL, //pdOHLC, pwOHLC,
        ...
        private async Task NMQClientSubscription(SchScalping paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(SchScalping paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(SchScalping paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(SchScalping paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(SchScalping source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.SchScalping);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<SchScalping> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<SchScalping>();
            }

            SchScalping algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
