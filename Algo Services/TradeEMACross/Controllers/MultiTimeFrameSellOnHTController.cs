...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class MultiTimeFrameSellOnHTController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_MultiTimeFrameSellOnHT_OBJECT";
        ...
        public MultiTimeFrameSellOnHTController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            MultiTimeFramSellOnHT paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.MultiTimeFramSellOnHT, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            paTrader.OnTradeEntry += OptionSellonHT_OnTradeEntry;
            paTrader.OnTradeExit += OptionSellonHT_OnTradeExit;

            List<MultiTimeFramSellOnHT> activeAlgoObjects = _cache.Get<List<MultiTimeFramSellOnHT>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<MultiTimeFramSellOnHT>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.MultiTimeFrameSellOnHT),
                an = Convert.ToString((AlgoIndex)AlgoIndex.MultiTimeFrameSellOnHT),
        ...
        private async Task<MultiTimeFramSellOnHT> ExecuteAlgo(PriceActionInput paInputs)
        {
            ...
            MultiTimeFramSellOnHT paTrader =
                new MultiTimeFramSellOnHT(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Expiry, paInputs.Qty, paInputs.UID, paInputs.TP, paInputs.SL,
        ...
        private void OptionSellonHT_OnTradeExit(Order st)
        {
        ...
        private void OptionSellonHT_OnTradeEntry(Order st)
        {
            //publish trade details and count
            OrderCore.PublishOrder(st);
            Thread.Sleep(100);
        ...
        private async Task NMQClientSubscription(MultiTimeFramSellOnHT paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(MultiTimeFramSellOnHT paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(MultiTimeFramSellOnHT paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(MultiTimeFramSellOnHT paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(MultiTimeFramSellOnHT source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.MultiTimeFrameSellOnHT);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<MultiTimeFramSellOnHT> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<MultiTimeFramSellOnHT>();
            }

            MultiTimeFramSellOnHT algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
