...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class EMACrossVolumeController : Controller
    {
        private const string key = "ACTIVE_EMACROSS_OBJECTS";
        IConfiguration configuration;
        ZMQClient zmqClient;
        private IMemoryCache _cache;
        public EMACrossVolumeController(IMemoryCache cache)
        {
            this._cache = cache;
        }
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] OptionBuyOnEMACrossInputs optionBuyOnEMACrossInputs, int algoInstance = 0)
        {
            ...
            FutureBuyonEMACrossVolumeCandle optionBuyonEMACross =
                new FutureBuyonEMACrossVolumeCandle(endDateTime, candleTimeSpan, volumeThreshold, instrumentToken, expiry,
                optionQuantity, optionBuyOnEMACrossInputs.TP, optionBuyOnEMACrossInputs.SL, 
                optionBuyOnEMACrossInputs.SEMA, optionBuyOnEMACrossInputs.LEMA, 25, algoInstance, false, 0);
            ...
            List<FutureBuyonEMACrossVolumeCandle> activeAlgoObjects = _cache.Get<List<FutureBuyonEMACrossVolumeCandle>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<FutureBuyonEMACrossVolumeCandle>();
            }
        ...
        private async Task NMQClientSubscription(FutureBuyonEMACrossVolumeCandle optionBuyWithRSI, uint token)
        {
        ...
        private void OptionBuywithRSI_OnOptionUniverseChange(FutureBuyonEMACrossVolumeCandle source)
        {
            try
            {
        ...
        {
            List<FutureBuyonEMACrossVolumeCandle> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<FutureBuyonEMACrossVolumeCandle>();
            }

            FutureBuyonEMACrossVolumeCandle algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
