...
namespace TradeEMACross.Controllers
{
    ...
    [ApiController]
    public class StockMomentumController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_SM_OBJECT";
        ...
        public StockMomentumController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            StockMomentumOptions paTrader = await ExecuteAlgo(paInputs);
            //#if local
            //            DataLogic dl = new DataLogic();
            //            DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.StockInitialMomentum, Convert.ToDateTime("2021-12-01"), Convert.ToDateTime("2021-12-30"));
            ...
            List<StockMomentumOptions> activeAlgoObjects = _cache.Get<List<StockMomentumOptions>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<StockMomentumOptions>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.StockInitialMomentum),
                an = Convert.ToString((AlgoIndex)AlgoIndex.StockInitialMomentum),
        ...
        private async Task<StockMomentumOptions> ExecuteAlgo(PriceActionInput paInputs)
        {
            StockMomentumOptions paTrader =
                new StockMomentumOptions(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Qty, paInputs.UID, httpClientFactory: _httpClientFactory);
        ...
        private async Task NMQClientSubscription(StockMomentumOptions paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(StockMomentumOptions paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(StockMomentumOptions paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(StockMomentumOptions paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(StockMomentumOptions source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.StockInitialMomentum);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<StockMomentumOptions> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<StockMomentumOptions>();
            }

            StockMomentumOptions algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
