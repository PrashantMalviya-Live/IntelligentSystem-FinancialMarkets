...
namespace RSICross.Controllers
{
    ...
    [ApiController]
    public class InitialBreakoutController : ControllerBase
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private const string key = "ACTIVE_IB_OBJECT";
        ...
        public InitialBreakoutController(IMemoryCache cache, IHttpClientFactory httpClientFactory)
        {
            this._cache = cache;
        ...
        {
            InitialRangeBreakout paTrader = await ExecuteAlgo(paInputs);
            ...
            List<InitialRangeBreakout> activeAlgoObjects = _cache.Get<List<InitialRangeBreakout>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<InitialRangeBreakout>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.InitialRangeBreakout),
                an = Convert.ToString((AlgoIndex)AlgoIndex.InitialRangeBreakout),
        ...
        private async Task<InitialRangeBreakout> ExecuteAlgo(PriceActionInput paInputs)
        {
            InitialRangeBreakout paTrader =
                new InitialRangeBreakout(TimeSpan.FromMinutes(paInputs.CTF), paInputs.BToken, paInputs.Qty, uid: paInputs.UID, httpClientFactory: _httpClientFactory);
        ...
        private async Task NMQClientSubscription(InitialRangeBreakout paTrader, uint token)
        {
...
#if local
        //private async Task KFKClientSubscription(InitialRangeBreakout paTrader, uint token)
        //{
        ...
        //}
        private async Task ObserverSubscription(InitialRangeBreakout paTrader, uint token)
        {
            GlobalObjects.ObservableFactory ??= new ObservableFactory();
            paTrader.Subscribe(GlobalObjects.ObservableFactory);
        }
        //private async Task MQTTCientSubscription(InitialRangeBreakout paTrader, uint token)
        //{
        ...
        //}
#endif
        private void PATrade_OnOptionUniverseChange(InitialRangeBreakout source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.InitialRangeBreakout);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<InitialRangeBreakout> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<InitialRangeBreakout>();
            }

            InitialRangeBreakout algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
