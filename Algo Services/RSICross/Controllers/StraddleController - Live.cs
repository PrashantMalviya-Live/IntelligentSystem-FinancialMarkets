...
namespace RSIManagerService.Controllers
{
    ...
    public class StraddleController : ControllerBase
    {
        ...
        public StraddleController(IMemoryCache cache)
        {
            this._cache = cache;
        }
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] OptionBuyWithStraddleInput optionBuyWithStraddleInput, int algoInstance = 0)
        {
            ...
            DirectionalWithStraddleShift straddleManager =
                new DirectionalWithStraddleShift(endDateTime, candleTimeSpan, instrumentToken, expiry,
                optionQuantity, 0, 500, optionBuyWithStraddleInput.SL, optionBuyWithStraddleInput.SS, optionBuyWithStraddleInput.TR, 
            ...
            List<DirectionalWithStraddleShift> activeAlgoObjects = _cache.Get<List<DirectionalWithStraddleShift>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<DirectionalWithStraddleShift>();
            }
        ...
        private async Task NMQClientSubscription(DirectionalWithStraddleShift straddleManager, uint token)
        {
        ...
        private void ExpiryTrade_OnOptionUniverseChange(DirectionalWithStraddleShift source)
        {
            try
            {
        ...
        {
            List<DirectionalWithStraddleShift> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<DirectionalWithStraddleShift>();
            }

            DirectionalWithStraddleShift algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
