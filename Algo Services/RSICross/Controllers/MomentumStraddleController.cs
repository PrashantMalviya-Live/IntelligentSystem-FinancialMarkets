...
namespace RSIManagerService.Controllers
{
    ...
    public class MomentumStraddleController : ControllerBase
    {
        ...
        public MomentumStraddleController(IMemoryCache cache)
        {
            this._cache = cache;
        }
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] OptionBuyWithStraddleInput optionBuyWithStraddleInput, int algoInstance = 0)
        {
            ...
            //PriceVolumeRateOfChange straddleManager =
            //    new PriceVolumeRateOfChange(endDateTime, candleTimeSpan, instrumentToken, expiry,
            //    optionQuantity, 0, optionBuyWithStraddleInput.TP, 100, optionBuyWithStraddleInput.SS, algoInstance, false, 0);

            PriceVolumeRateOfChange straddleManager =
                new PriceVolumeRateOfChange(endDateTime, candleTimeSpan, instrumentToken, expiry,
                optionQuantity, optionBuyWithStraddleInput.TP, optionBuyWithStraddleInput.SL, 0,0,0,algoInstance, false, 0);
            ...
            List<PriceVolumeRateOfChange> activeAlgoObjects = _cache.Get<List<PriceVolumeRateOfChange>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<PriceVolumeRateOfChange>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.OptionBuyWithStraddle),
                an = Convert.ToString((AlgoIndex)AlgoIndex.OptionBuyWithStraddle),
        ...
        private async Task NMQClientSubscription(PriceVolumeRateOfChange straddleManager, uint token)
        {
        ...
        private void ExpiryTrade_OnOptionUniverseChange(PriceVolumeRateOfChange source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.OptionBuyWithStraddle);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<PriceVolumeRateOfChange> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<PriceVolumeRateOfChange>();
            }

            PriceVolumeRateOfChange algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
