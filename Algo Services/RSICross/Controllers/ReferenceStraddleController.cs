...
namespace RSIManagerService.Controllers
{
    ...
    [ApiController]
    public class ReferenceStraddleController : ControllerBase
    {
        private const string key = "ACTIVE_STRADDLE_OBJECTS";
        IConfiguration configuration;
        ZMQClient zmqClient;
        private IMemoryCache _cache;
        public ReferenceStraddleController(IMemoryCache cache)
        {
            this._cache = cache;
        }
        ...
        [HttpPost]
        public async Task<ActiveAlgosView> Trade([FromBody] OptionBuyWithStraddleInput optionBuyWithStraddleInput, int algoInstance = 0)
        {
            uint instrumentToken = optionBuyWithStraddleInput.BToken;
            DateTime endDateTime = DateTime.Now;
            TimeSpan candleTimeSpan = TimeSpan.FromMinutes(optionBuyWithStraddleInput.CTF);

            DateTime expiry = optionBuyWithStraddleInput.Expiry;
            ...
            ManageStraddleWithReferenceValue straddleManager =
                new ManageStraddleWithReferenceValue(endDateTime, candleTimeSpan, instrumentToken, expiry,
                optionQuantity, 0, 500, optionBuyWithStraddleInput.SL, 
            ...
            List<ManageStraddleWithReferenceValue> activeAlgoObjects = _cache.Get<List<ManageStraddleWithReferenceValue>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<ManageStraddleWithReferenceValue>();
            }
            ...
            {
                aid = Convert.ToInt32(AlgoIndex.ManageReferenceStraddle),
                an = Convert.ToString((AlgoIndex)AlgoIndex.ManageReferenceStraddle),
                ...
                mins = optionBuyWithStraddleInput.CTF
            };
        }
        ...
        private async Task NMQClientSubscription(ManageStraddleWithReferenceValue straddleManager, uint token)
        {
        ...
        private void ExpiryTrade_OnOptionUniverseChange(ManageStraddleWithReferenceValue source)
        {
            try
            {
        ...
        {
            return Task.FromResult((int)AlgoIndex.ManageReferenceStraddle);
        }

        [HttpPut("{ain}")]
        public bool Put(int ain, [FromBody] int start)
        {
            List<ManageStraddleWithReferenceValue> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<ManageStraddleWithReferenceValue>();
            }

            ManageStraddleWithReferenceValue algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
