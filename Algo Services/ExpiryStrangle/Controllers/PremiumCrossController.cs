...
namespace RSICross.Controllers
{
    ...
    public class PremiumCrossController : ControllerBase
    {
        ...
        public async Task<ActiveAlgosView> Trade([FromBody] PremiumCrossInput premiumCrossInput, int algoInstance = 0)
        {
            ...
            OptionBuyOnPremiumCrossShift straddleManager =
                new OptionBuyOnPremiumCrossShift(endDateTime, instrumentToken, expiry,
            ...
            List<OptionBuyOnPremiumCrossShift> activeAlgoObjects = _cache.Get<List<OptionBuyOnPremiumCrossShift>>(key);

            if (activeAlgoObjects == null)
            {
                activeAlgoObjects = new List<OptionBuyOnPremiumCrossShift>();
            }
        ...
        private async Task NMQClientSubscription(OptionBuyOnPremiumCrossShift straddleManager, uint token)
        {
        ...
        private void ExpiryTrade_OnOptionUniverseChange(OptionBuyOnPremiumCrossShift source)
        {
            try
            {
        ...
        {
            List<OptionBuyOnPremiumCrossShift> activeAlgoObjects;
            if (!_cache.TryGetValue(key, out activeAlgoObjects))
            {
                activeAlgoObjects = new List<OptionBuyOnPremiumCrossShift>();
            }

            OptionBuyOnPremiumCrossShift algoObject = activeAlgoObjects.FirstOrDefault(x => x.AlgoInstance == ain);
            if (algoObject != null)
            {
