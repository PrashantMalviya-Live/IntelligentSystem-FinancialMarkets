...
{
    public class DirectionalWithStraddleShiftCandle
    {
        ...
        private Dictionary<uint, AverageTrueRange> _instrumentATR;
        public Dictionary<uint, uint> MappedTokens { get; set; }
        public Candle _bCandle;
        ...
        public Dictionary<uint, SuperTrend> _instrumentSuperTrend;
            ...
            decimal maxLossPerTrade = 0)
        {
            ...
            _instrumentATR = new Dictionary<uint, AverageTrueRange>();
            _instrumentSuperTrend = new Dictionary<uint, SuperTrend>();
            ZConnect.Login();
            KoConnect.Login();
        ...
        public void RunAlgo(Dictionary<DateTime, HistoricalData> historicalData)
        {
            foreach (var historicalCandles in historicalData)
            {
                Trade(GetCandle(historicalCandles.Value.HistoricalValue, historicalCandles.Value.InstrumentToken));
            }
        }
        private Candle GetCandle(Historical historical, uint instrumentToken)
        {
            Candle c = new TimeFrameCandle();
            c.ClosePrice = historical.Close;
            c.ClosePrice = historical.Open;
            c.HighPrice = historical.High;
            c.LowPrice = historical.Low;
            c.CloseTime = historical.TimeStamp;
            c.TotalVolume = historical.Volume;
            c.InstrumentToken = instrumentToken;

            return c;
        }

        private void Trade(Candle candle)
        {
            SuperTrend superTrend = _instrumentSuperTrend[candle.InstrumentToken];
            superTrend.Process(candle);
            Candle previousCandle = TimeCandles[candle.InstrumentToken].Last();
            TimeCandles[candle.InstrumentToken].Add(candle);

            LoadOptionsToTrade(candle.CloseTime);

            if (candle.InstrumentToken == _baseInstrumentToken)
            {
                _bCandle = candle;
                // if( (previousCandle.ClosePrice > superTrend.GetValue() && candle.ClosePrice < superTrend.GetValue())
                //    || (previousCandle.ClosePrice < superTrend.GetValue() && candle.ClosePrice > superTrend.GetValue())
                //    )
                //{
                //    _crossed = true;
                //}
            ...
            {
                decimal strike = GetATMStrike(_bCandle.ClosePrice);
                TimeCandles[candle.InstrumentToken].Add(candle);
        ...
        private decimal GetATMStrike(decimal _baseTokenPrice)
        {
            return Math.Round(_baseTokenPrice / 100, 0) * 100;
        }
        private Instrument GetATMInstrument(decimal strike, string instrumentType)
        {
            if(instrumentType.Trim(' ').ToLower() == "ce")
            {
                return OptionUniverse[0][strike];
            }
            else
            {
                return OptionUniverse[0][strike];
        ...
        private void HedgeStraddle(DateTime? currentTime)
        {
            if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(15, 20, 00))// && _referenceStraddleValue != 0)
            {
                ...
                if (_straddleCallOrderTrio != null && _straddlePutOrderTrio != null)
                {
                    ...
                    //Hedge Call trade
                    _hedgeCallOrderTrio = TradeEntry(callHedgeOption, currentTime.Value, callHedgeOption.LastPrice, _tradeQty, true);
                    _hedgePutOrderTrio = TradeEntry(putHedgeOption, currentTime.Value, putHedgeOption.LastPrice, _tradeQty, true);
            ...
            else if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(09, 20, 00))// && _referenceStraddleValue != 0)
            {
                ...
                {
                    //Hedge Call trade
                    _hedgeCallOrderTrio = TradeEntry(_hedgeCallOrderTrio.Option, currentTime.Value, _hedgeCallOrderTrio.Option.LastPrice, _tradeQty, false);
                    _hedgePutOrderTrio = TradeEntry(_hedgePutOrderTrio.Option, currentTime.Value, _hedgePutOrderTrio.Option.LastPrice, _tradeQty, false);
                    ...
                    if (_straddleCallOrderTrio != null && _straddlePutOrderTrio != null)
                    {
                        ...
                        if (_straddleShift)
                        {
                                ...
                                || ((Math.Abs(Math.Abs(activeCE.Delta) - Math.Abs(activePE.Delta))) < (Math.Abs(Math.Abs(_activeCall.Delta) - Math.Abs(_activePut.Delta))))))
                            {
                                ...
                                OrderTrio putOrderTrio = TradeEntry(_activePut, currentTime, _activePut.LastPrice, _tradeQty, true);
                                OrderTrio callOrderTrio = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, _tradeQty, true);
