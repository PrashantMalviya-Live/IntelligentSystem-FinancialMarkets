    ...
    public class FutureBuyonEMACrossVolumeCandle : IZMQ
    {
            ...
            IIndicatorValue sema, IIndicatorValue lema, IIndicatorValue bema, IIndicatorValue signalEMA)
        {
            ...
            try
            {
                ...
                decimal entryRSI = 0;
                if (option != null && CheckEntryCriteria(token, currentTime, sema, lema, bema, signalEMA, lastPrice, out entryRSI))
                {
        ...
        private void LoadHistoricalCandles(string tokenList, int candlesCount, DateTime lastCandleEndTime)
        {
            try
            {
                lock (lTokenEMA)
                {
                    ...
                    Dictionary<uint, List<decimal>> historicalCandlePrices = dl.GetHistoricalVCandlePrices(candlesCount, lastCandleEndTime, tokenList, _candleTimeSpan, false);
