...
{
    public class OptionBuyonEMACross : IZMQ
    {
        ...
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(OptionBuyonEMACross source);
        [field: NonSerialized]
        ...
        private User _user;
        Dictionary<uint, ExponentialMovingAverage> lTokenVolumeEMA;
        Dictionary<uint, ExponentialMovingAverage> sTokenVolumeEMA;
        Dictionary<uint, ExponentialMovingAverage> signalTokenVolumeEMA;
        Dictionary<uint, RelativeStrengthIndex> tokenVolumeRSI;

        Dictionary<uint, IIndicatorValue> stokenVolumeEMAIndicator;
        Dictionary<uint, IIndicatorValue> ltokenVolumeEMAIndicator;
        Dictionary<uint, IIndicatorValue> signalVolumeEMAIndicator;
        private Dictionary<uint, bool> _belowVolumeEMA;
        private Dictionary<uint, bool> _aboveVolumeEMA;
        ...
        List<uint> _VolumeEMALoaded;
        List<uint> _VolumeSQLLoading;

        Dictionary<uint, bool> _firstCandleOpenPriceNeeded;
        Dictionary<uint, bool> _firstVolumeCandleOpenPriceNeeded;

        private uint _baseInstrumentToken;
        private decimal _baseInstrumentPrice;
        private ExponentialMovingAverage _bVolumeEMA, _bEMA;
        private IIndicatorValue _bEMAValue, _bVolumeEMAValue;
        private bool _bEMALoaded = false, _bEMALoadedFromDB = false;
        private bool _bEMALoading = false;


        private bool _bVolumeEMALoaded = false, _bVolumeEMALoadedFromDB = false;
        private bool _bVolumeEMALoading = false;
        ...
        private readonly int _sVolumeEMALength;
        private readonly int _lVolumeEMALength;
        private readonly int _signalVolumeEMALength;

        private uint _volumeThreshold;
        ...
        CandleManger volumeCandleManger;
        ...
        public OptionBuyonEMACross(TimeSpan candleTimeSpan, uint baseInstrumentToken,
            int quantity, string uid, decimal targetProfit, decimal stopLoss, int sEMALength,
            int lEMALength, int signalEMALength = 200, int algoInstance = 0, bool positionSizing = false, decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            //_endDateTime = endTime;
            _candleTimeSpan = candleTimeSpan;
            _volumeThreshold = 50000;
            //_expiryDate = expiry;
            _baseInstrumentToken = baseInstrumentToken;
            _sEMALength = sEMALength;
            _lEMALength = lEMALength;
            _httpClientFactory = httpClientFactory;
            _sVolumeEMALength = sEMALength;
            _lVolumeEMALength = lEMALength;
            ...
            _stopLoss = stopLoss;
            _stopTrade = true;
            _signalVolumeEMALength = _signalEMALength = signalEMALength;
            _emaBandForExit = 0;
            _timeBandForExit = 2;
            ...
            stokenVolumeEMAIndicator = new Dictionary<uint, IIndicatorValue>();
            ltokenVolumeEMAIndicator = new Dictionary<uint, IIndicatorValue>();
            signalVolumeEMAIndicator = new Dictionary<uint, IIndicatorValue>();
            _belowVolumeEMA = new Dictionary<uint, bool>();
            _aboveVolumeEMA = new Dictionary<uint, bool>();
            //EMAs
            lTokenVolumeEMA = new Dictionary<uint, ExponentialMovingAverage>();
            sTokenVolumeEMA = new Dictionary<uint, ExponentialMovingAverage>();
            tokenVolumeRSI = new Dictionary<uint, RelativeStrengthIndex>();
            signalTokenVolumeEMA = new Dictionary<uint, ExponentialMovingAverage>();
            _bVolumeEMA = new ExponentialMovingAverage(BASE_EMA_LENGTH);
            _VolumeEMALoaded = new List<uint>();
            _VolumeSQLLoading = new List<uint>();

            


            _firstCandleOpenPriceNeeded = new Dictionary<uint, bool>();
            _firstVolumeCandleOpenPriceNeeded = new Dictionary<uint, bool>();
            ...
            volumeCandleManger = new CandleManger(VolumeCandles, CandleType.Volume);
            volumeCandleManger.VolumeCandleFinished += VolumeCandleManger_VolumeCandleFinished;

            _algoInstance = algoInstance != 0 ? algoInstance :
                Utility.GenerateAlgoInstance(algoIndex, baseInstrumentToken, DateTime.Now,
                DateTime.Now, quantity, candleTimeFrameInMins:
                ...
                lock (TimeCandles)
                {
                    lock (VolumeCandles)
                    {
                        ...
                        else if (tick.LastTradeTime.HasValue)
                        {
                            ...
                            if (!_VolumeEMALoaded.Contains(token))
                            {
                                LoadHistoricalVolumeEMAs(currentTime);
                            }
                            if (!sTokenVolumeEMA.ContainsKey(token) || !lTokenVolumeEMA.ContainsKey(token) || !signalTokenVolumeEMA.ContainsKey(token))
                            {
                                return;
                            }
                            ...
                            IIndicatorValue sVolumeema = sTokenVolumeEMA[token].Process(tick.LastPrice, isFinal: false);
                            IIndicatorValue lVolumeema = lTokenVolumeEMA[token].Process(tick.LastPrice, isFinal: false);
                            IIndicatorValue signalVolumeEMA = signalTokenVolumeEMA[token].Process(tick.LastPrice, isFinal: false);

                            if (stokenVolumeEMAIndicator.ContainsKey(token))
                            {
                                stokenVolumeEMAIndicator[token] = sVolumeema;
                            }
                            else
                            {
                                stokenVolumeEMAIndicator.Add(token, sVolumeema);
                            }
                            if (ltokenVolumeEMAIndicator.ContainsKey(token))
                            {
                                ltokenVolumeEMAIndicator[token] = lVolumeema;
                            }
                            else
                            {
                                ltokenVolumeEMAIndicator.Add(token, lVolumeema);
                            }

                            if (signalVolumeEMAIndicator.ContainsKey(token))
                            {
                                signalVolumeEMAIndicator[token] = signalVolumeEMA;
                            }
                            else
                            {
                                signalVolumeEMAIndicator.Add(token, signalVolumeEMA);
                            }
                            if (!_belowVolumeEMA.ContainsKey(token))
                            {
                                _belowVolumeEMA.Add(token, false);
                            }

                            if (sVolumeema.IsEmpty || lVolumeema.IsEmpty || !sVolumeema.IsFormed || !lVolumeema.IsFormed || signalVolumeEMA.IsEmpty || !signalVolumeEMA.IsFormed)
                            {
                                return;
                            }
                            ...
                            if (ActiveOptions.Any(x => x.InstrumentToken == token))
                            {
                                ...
                                if (orderTrio == null)
                                {
                                    orderTrio = TradeEntry(token, tick.LastPrice, currentTime, sema,
                                        lema, _bEMAValue, signalEMA, sVolumeema, lVolumeema, _bVolumeEMAValue, signalVolumeEMA, isOptionCall);
                                }
                                else
                                {
        ...
        private void MonitorVolumeCandles(Tick tick, DateTime currentTime)
        {
            try
            {
                ...
                volumeCandleManger.StreamingVolumeCandle(tick, token, _volumeThreshold);
        ...
        private void LoadHistoricalVolumeEMAs(DateTime currentTime)
        {
            DateTime lastCandleEndTime;
            DateTime? candleStartTime = CheckCandleStartTime(currentTime, out lastCandleEndTime);
            ...
            try
            {
                var tokens = SubscriptionTokens.Where(x => x != _baseInstrumentToken && !_VolumeEMALoaded.Contains(x));
                StringBuilder sb = new StringBuilder();

                lock (lTokenVolumeEMA)
                {
                    foreach (uint t in tokens)
                    {
                        if (!_firstVolumeCandleOpenPriceNeeded.ContainsKey(t))
                        {
                            _firstVolumeCandleOpenPriceNeeded.Add(t, candleStartTime != lastCandleEndTime);
                        }

                        if (!lTokenVolumeEMA.ContainsKey(t) && !_VolumeSQLLoading.Contains(t))
                        {
                            sb.AppendFormat("{0},", t);
                            _SQLLoading.Add(t);
                        }
                ...
                {
                    LoadHistoricalVolumeCandles(tokenList, _signalVolumeEMALength * 2, lastCandleEndTime);
                    //Task task = Task.Run(() => LoadHistoricalVolumeCandles(tokenList, _signalVolumeEMALength * 2, lastCandleEndTime));
                }
                foreach (uint tkn in tokens)
                {
                    if (VolumeCandles.ContainsKey(tkn) && lTokenVolumeEMA.ContainsKey(tkn))
                    {
                        if (_firstVolumeCandleOpenPriceNeeded[tkn])
                        {
                            //The below EMA token input is from the candle that just started, All historical prices are already fed in.
                            sTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().OpenPrice, isFinal: true);
                            lTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().OpenPrice, isFinal: true);
                            signalTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().OpenPrice, isFinal: true);
                        ...
                        if (VolumeCandles[tkn].Count > 1)
                        {
                            ...
                            {
                                sTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().ClosePrice, isFinal: true);
                                lTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().ClosePrice, isFinal: true);
                                signalTokenVolumeEMA[tkn].Process(VolumeCandles[tkn].First().OpenPrice, isFinal: true);
                            }
                        }
                    }
                    if ((firstCandleFormed == 1 || !_firstVolumeCandleOpenPriceNeeded[tkn]) && lTokenVolumeEMA.ContainsKey(tkn))
                    {
                        _VolumeEMALoaded.Add(tkn);
        ...
        private void VolumeCandleManger_VolumeCandleFinished(object sender, Candle e)
        {
            try
            {
                ...
                else if (_VolumeEMALoaded.Contains(e.InstrumentToken))
                {
                    if (!lTokenVolumeEMA.ContainsKey(e.InstrumentToken) || !sTokenVolumeEMA.ContainsKey(e.InstrumentToken) || !signalTokenVolumeEMA.ContainsKey(e.InstrumentToken))
                    {
                        return;
                    }

                    sTokenVolumeEMA[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    lTokenVolumeEMA[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    signalTokenVolumeEMA[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    ...
                    if (ActiveOptions.Any(x => x.InstrumentToken == e.InstrumentToken))
                    {
                        ...
                        decimal sema = sTokenVolumeEMA[e.InstrumentToken].GetValue<decimal>(0);
                        decimal lema = lTokenVolumeEMA[e.InstrumentToken].GetValue<decimal>(0);
                        decimal signalema = signalTokenVolumeEMA[e.InstrumentToken].GetValue<decimal>(0);

                        LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, e.CloseTime,
                            String.Format("Volume Candle ({4}) OHLC: {0} | {1} | {2} | {3}. sEMA:{6}. lEMA:{5}. Signal EMA: {7}", e.OpenPrice, e.HighPrice, e.LowPrice, e.ClosePrice
        ...
        private void LoadHistoricalEMAs(DateTime currentTime)
        {
            ...
            try
            {
                ...
                if (tokenList != string.Empty)
                {
                    LoadHistoricalCandles(tokenList, _signalEMALength * 2, lastCandleEndTime);
            ...
            DateTime currentTime, OrderTrio orderTrio, decimal entryRSI)
        {
            try
            {
                ...
                Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType,
                    ...
                    _tradeQty * Convert.ToInt32(option.LotSize), algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, httpClient: KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                ...
                order = MarketOrders.PlaceOrder(_algoInstance, newInstrument.TradingSymbol, newInstrument.InstrumentType, lastPrice, //newInstrument.LastPrice,
                    ...
                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, httpClient: KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
        ...
        private OrderTrio TradeEntry(uint token, decimal lastPrice, DateTime currentTime,
            ...
            IIndicatorValue sVolumeema, IIndicatorValue lVolumeema, IIndicatorValue bVolumeema, IIndicatorValue signalVolumeEMA, bool isOptionCall)
        {
            ...
            try
            {
                ...
                decimal entryRSI = 0;
                if (option != null && CheckEntryCriteria(token, currentTime, sema, lema, bema, signalEMA, sVolumeema, 
                    lVolumeema, bVolumeema, signalVolumeEMA, isOptionCall, lastPrice, out entryRSI))
                {
                    decimal stopLoss = 0, targetProfit = 0;
                    GetCriticalLevels(lastPrice, out stopLoss, out targetProfit);
        ...
        private bool CheckEntryCriteria(uint token, DateTime currentTime, IIndicatorValue sema, 
            IIndicatorValue lema, IIndicatorValue bema, IIndicatorValue signalEMA,
            IIndicatorValue sVolumeema,
            IIndicatorValue lVolumeema, IIndicatorValue bVolumeema, IIndicatorValue signalVolumeEMA,
            bool isOptionCall, decimal currentPrice, out decimal entryRSI)
        {
            entryRSI = 0;
            try
            {
                ...
                decimal sVolumeemaValue = sVolumeema.GetValue<decimal>();
                decimal lVolumeemaValue = lVolumeema.GetValue<decimal>();
                decimal bVolumeemaValue = bVolumeema == null ? 0 : bVolumeema.GetValue<decimal>();
                decimal signalVolumeemaValue = signalVolumeEMA.GetValue<decimal>();
                //entryRSI = rsiValue;

                if (sVolumeemaValue < lemaValue)
                {
                    _belowEMA[token] = true;
                }
                ...
                if (sema.IsFormed && lema.IsFormed
                    && sVolumeemaValue > lemaValue
                    && _belowEMA[token]
                    //&& currentPrice < sVolumeemaValue + 2 //temp comment
        ...
        private void LoadHistoricalCandles(string tokenList, int candlesCount, DateTime lastCandleEndTime)
        {
            try
            {
                lock (lTokenEMA)
                {
                    ...
                    DateTime previousTradingDate = dl.GetPreviousTradingDate(lastCandleEndTime, 6);
                    ...
                    foreach (uint token in tokenList)
                    {
                        ...
                        foreach (var price in historicals)
                        {
                            ...
                            lema.Process(price.Close, isFinal: true);
                            signalEMA.Process(price.Close, isFinal: true);
                        }
                        sTokenEMA.Add(token, sema);
                        lTokenEMA.Add(token, lema);
                        signalTokenEMA.Add(token, signalEMA);
                    }
        ...
        private void LoadHistoricalVolumeCandles(string tokenList, int candlesCount, DateTime lastCandleEndTime)
        {
            try
            {
                lock (lTokenEMA)
                {
                    ...
                    foreach (string stoken in tokenList.Split(','))
                    {
                       // uint token = Convert.ToUInt32(stoken);


                        //The below is from ticks
                        //List<decimal> historicalCandlePrices = dl.GetHistoricalCandlePrices(candlesCount, lastCandleEndTime, token.ToString(), _candleTimeSpan);
                        Dictionary<uint, List<decimal>> historicalCandlePrices = dl.GetHistoricalCandlePrices(candlesCount, lastCandleEndTime, stoken, TimeSpan.Zero, false, CandleType.Volume, _volumeThreshold);
                        //The below is from candles
                        //List<decimal> historicalCandlePrices = dl.GetHistoricalClosePricesFromCandles(candlesCount, lastCandleEndTime, token, _candleTimeSpan);
                        //Dictionary<uint, List<decimal>> historicalCandlePrices = dl.GetHistoricalClosePricesFromCandles(candlesCount, lastCandleEndTime, tokenList, _candleTimeSpan);
                        ...
                        foreach (uint t in historicalCandlePrices.Keys)
                        {
                            sema = new ExponentialMovingAverage(_sVolumeEMALength);
                            lema = new ExponentialMovingAverage(_lVolumeEMALength);
                            signalEMA = new ExponentialMovingAverage(_signalVolumeEMALength);
                            foreach (var price in historicalCandlePrices[t])
                            {
                                sema.Process(price, isFinal: true);
                                lema.Process(price, isFinal: true);
                            ...
                            sTokenVolumeEMA.TryAdd(t, sema);
                            lTokenVolumeEMA.TryAdd(t, lema);
                            signalTokenVolumeEMA.TryAdd(t, signalEMA);
                        }
                    }
                }
            }
