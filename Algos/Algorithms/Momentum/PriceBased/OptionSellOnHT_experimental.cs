...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private bool _putQtyTraded = false, _callQtyTraded = false;
                ...
                if (_baseCandleLoaded)
                {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                       ...
                       // bool suspensionWindow = _bCandle.CloseTime.TimeOfDay > SUSPENSION_WINDOW_START && _bCandle.CloseTime.TimeOfDay < SUSPENSION_WINDOW_END;
                        ...
                        else if (htred == 1 && (_bCandle.LowPrice < _downtrendlowswingindexvalue || _downtrendlowswingindexvalue == 0))
                        {
                            _downtrendlowswingindexvalue = _bCandle.LowPrice;
                        }
                        ...
                        if(_putTriggered )//&& _bCandle.ClosePrice > _peReferencePrice + 6)
                        {
                            _callTrigerred = false;
                            //_putTriggered = true;
                        ...
                        if(_callTrigerred )//&& _bCandle.ClosePrice < _ceReferencePrice - 6)
                        {
                            _ceReferencePrice = 0;
                            _putTriggered = false;
                            //_callTrigerred = true;
                        ...
                        {
                            _putTriggered = true;
                            _peReferencePrice = _bCandle.HighPrice;
                        }
                        else if (previoushtred == 0 && htred == 1)
                        {
                            _callTrigerred = true;
                            _ceReferencePrice = _bCandle.LowPrice;
                        }
                       ...
                       // }
                        if(_callorderTrios.Count != 0)
                        {
                            bool callTradeTriggered = false;
                            for(int i = 0; i< _callorderTrios.Count; i++)
                            {
                                var orderTrio = _callorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice > orderTrio.StopLoss && htred == 0 && !_ceSLTrigerred)
                                {
                                    _ceSLReference = _bCandle.HighPrice;
                                    _ceSLTrigerred = true;
                                }
                                if (_ceSLTrigerred && _bCandle.ClosePrice > _ceSLReference + 10)
                                {
                                    Instrument option = orderTrio.Option;
                                    callTradeTriggered = true;
                            ...
                            if (callTradeTriggered)
                                _ceSLTrigerred = false;
                        }
                        if (_putorderTrios.Count != 0)
                        {
                            bool putTradeTriggered = false;
                            for (int i = 0; i < _putorderTrios.Count; i++)
                            {
                                var orderTrio = _putorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice < orderTrio.StopLoss && htred == 1 && !_peSLTrigerred)
                                {
                                    _peSLReference = _bCandle.LowPrice;
                                    _peSLTrigerred = true;
                                }
                                if (_peSLTrigerred && _bCandle.ClosePrice < _peSLReference - 10)
                                {
                                    Instrument option = orderTrio.Option;
                                    putTradeTriggered = true;
                            ...
                            if (putTradeTriggered)
                                _peSLTrigerred = false;
                        }
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - 11 && _putTriggered && _triggerPutTrade)
                    {
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + 11) && _callTrigerred && _triggerCallTrade)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
    ...
    ref bool thirdTradeActive)
        ...
        private int GetTradedQuantity(decimal triggerLevel, decimal SL, int totalQuantity, int bookedQty, bool thirdTargetActive)
        {
            ...
            if (Math.Abs(triggerLevel - SL) < 40)
            {
                //tradedQuantity = bookedQty == 0 ? totalQuantity * 3 / 4 : totalQuantity * 4 / 4;
                tradedQuantity = totalQuantity;
                tradedQuantity -= tradedQuantity % 25;
            ...
            //    tradedQuantity -= tradedQuantity % 25;
            //}
            else if (Math.Abs(triggerLevel - SL) < 100 && thirdTargetActive)
            {
                tradedQuantity = totalQuantity * 2 / 4;
                //tradedQuantity = bookedQty == 0 ? totalQuantity * 2 / 4 : totalQuantity * 3 / 4;
                tradedQuantity -= tradedQuantity % 25;
