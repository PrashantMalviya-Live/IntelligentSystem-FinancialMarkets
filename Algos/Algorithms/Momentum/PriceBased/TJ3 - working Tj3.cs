    ...
    public class TJ3 : IZMQ, IObserver<Tick>//, IKConsumer
    {
       ...
       // decimal _previoushtred;
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (_baseCandleLoaded)//&& _referenceCandleLoaded)
            {
                ...
                if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                {
                    ...
                    if (_orderTrios.Count > 0 && ((_orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell" && _activeFuture.LastPrice > _orderTrios[0].StopLoss)
                        || (_orderTrios[0].Order.TransactionType.ToLower().Trim() == "buy" && _activeFuture.LastPrice < _orderTrios[0].StopLoss)
                        ))
                    {
        ...
        private void LastSwingHighlow(DateTime currentTime, uint token, out decimal swingHigh, out decimal swingLow)
        {
            ...
            if (TimeCandles[token].Count >= 12 || _candleTimeSpan.Minutes == 5)
            {
