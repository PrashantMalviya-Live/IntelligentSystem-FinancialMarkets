    ...
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    if (_stockTokenHalfTrendLong.ContainsKey(token) && (e.CloseTime.Minute - MARKET_START_TIME.Minutes) % _candleTimeSpanLong.Minutes == 0)
                    {
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5?31:11) && _putTriggered && _triggerPutTrade)
                    {
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes == 5 ? 31 : 11)) && _callTrigerred && _triggerCallTrade)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
