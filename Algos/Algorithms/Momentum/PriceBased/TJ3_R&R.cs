       ...
       // decimal _previoushtred;
        ...
        private void TakeTrade()
        {
                   ...
                   && _putTriggered)
            {
                OrderTrio orderTrio = new OrderTrio();
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                   _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, _bCandle.CloseTime,
                   Tag: "",
                    ...
                    && _callTrigerred)
            {
                OrderTrio orderTrio = new OrderTrio();
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                   _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, _bCandle.CloseTime,
                   Tag: "",
                   product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                   broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
            ...
            if (_baseCandleLoaded)//&& _referenceCandleLoaded)
            {
                ...
                if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                {
                    ...
                    if (previoushtred == 1 && htred == 0)// && htredLong == 1)
                    {
                        ...
                        else if (_orderTrios.Count != 0 && _orderTrios[0].Order.TransactionType.ToLower() == "buy")
                        {
                            //NO TRAILING
                    ...
                    if (_orderTrios.Count > 0 && ((_orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell" && ((_activeFuture.LastPrice > _orderTrios[0].StopLoss)/*||(_activeFuture.LastPrice < _orderTrios[0].TargetProfit)*/))
                        || (_orderTrios[0].Order.TransactionType.ToLower().Trim() == "buy" && ((_activeFuture.LastPrice < _orderTrios[0].StopLoss) /*||(_activeFuture.LastPrice > _orderTrios[0].TargetProfit)*/))
                        ))
                    {
        ...
        private void LastSwingHighlow(DateTime currentTime, uint token, out decimal swingHigh, out decimal swingLow)
        {
            ...
            IEnumerable<Candle> candles = TimeCandles[token].TakeLast(12);
            swingHigh = 0;
            swingLow = 1000000;
            ...
            foreach (var ohlc in candles.OrderByDescending(x => x.CloseTime))
            {
