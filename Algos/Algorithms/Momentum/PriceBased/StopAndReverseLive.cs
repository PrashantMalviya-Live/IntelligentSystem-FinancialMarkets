...
{
    public class StopAndReverse : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(StopAndReverse source);
        [field: NonSerialized]
        ...
        public List<decimal> _indexValues;
        public List<uint> tokenExits;
        public decimal _minimumValue, _maximumValue;
        public decimal _maximumBeforeMinimum, _minimumBeforeMaximum;
        ...
        private  decimal THRESHOLD = 40;
        private decimal PROFIT_TARGET = 100;
        private decimal STOP_LOSS = 40;
        ...
        public StopAndReverse( uint baseInstrumentToken,
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            //_indexValues = new Queue<decimal>(25000);
            _indexValues = new List<decimal>(25000);
        ...
        void UpdateFuturePrice(decimal lastPrice)
        {
            ...
            _indexValues.Add(lastPrice);
            if (_indexValues.Count >=VALUES_THRESHOLD)
            {
                _indexValues.RemoveAt(0);
            }
        }
        bool CheckReturn(decimal lastPrice, out decimal sl, out decimal target)
        {
            if(lastPrice < _minimumValue || _minimumValue == 0)
            {
                _minimumValue = lastPrice;
                _maximumBeforeMinimum = _indexValues.Max();
            }
            else if (lastPrice > _maximumValue)
            {
                _maximumValue = lastPrice;
                _minimumBeforeMaximum = _indexValues.Min();
            }
            if (_maximumBeforeMinimum - _minimumValue > THRESHOLD && lastPrice - _minimumValue > THRESHOLD)
            {
                ...
                _minimumValue = 0;
                _maximumValue = 0;
                _minimumBeforeMaximum = 0;
                _maximumBeforeMinimum = 0;
                return true;

            }
            if (_maximumValue - _minimumBeforeMaximum > THRESHOLD && _maximumValue - lastPrice > THRESHOLD)
            {
                sl = lastPrice + STOP_LOSS;
                target = lastPrice - PROFIT_TARGET;// THRESHOLD - 2;
        ...
        //            }
        //        }
        //        else if (lastPrice < _indexValues.ElementAt(i))
        //        {
        //            break;
        //        }
        //    }
        ...
        private void TradeTPSL(DateTime currentTime, bool tp, bool sl)
        {
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    if ((orderTrio.Order.TransactionType == "buy") && (((_activeFuture.LastPrice > orderTrio.TargetProfit) && tp) 
                        || ((_activeFuture.LastPrice < orderTrio.StopLoss) && sl)))
                    {
                        if (_activeFuture.LastPrice > orderTrio.TargetProfit)
                        {
                        ...
                        else if (_activeFuture.LastPrice < orderTrio.StopLoss && currentTime - orderTrio.IntialSlHitTime.Value > new TimeSpan(8, 5, 0))
                        {
                            tradePrice = CloseTrade(buy: false, currentTime);
                            _pnl += (tradePrice - orderTrio.Order.AveragePrice);
                            _orderTrios.Remove(orderTrio);
                    ...
                    else if (orderTrio.Order.TransactionType == "sell" && (((_activeFuture.LastPrice < orderTrio.TargetProfit) && tp) || ((_activeFuture.LastPrice > orderTrio.StopLoss) && sl)))
                    {
                        if (_activeFuture.LastPrice < orderTrio.TargetProfit)
                        {
                        ...
                        else if (_activeFuture.LastPrice >= orderTrio.StopLoss && currentTime - orderTrio.IntialSlHitTime.Value > new TimeSpan(8, 1, 0))
                        {
                            tradePrice = CloseTrade(buy: true, currentTime);
                            _pnl += (orderTrio.Order.AveragePrice - tradePrice);
                            _orderTrios.Remove(orderTrio);
        ...
        //{
        //    DataLogic dl = new DataLogic();
        //    DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.StopAndReverse, Convert.ToDateTime("2021-11-30"), Convert.ToDateTime("2021-12-30"));
