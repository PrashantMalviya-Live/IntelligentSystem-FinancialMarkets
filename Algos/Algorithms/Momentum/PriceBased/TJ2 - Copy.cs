...
namespace Algorithms.Algorithms
{
    public class TJ2 : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        public TJ2(TimeSpan candleTimeSpan, uint baseInstrumentToken, uint referenceBaseToken,
        ...
        private void ActiveTradeIntraday(Tick tick)
        {
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    {
                        LoadCriticalLevels(_baseInstrumentToken, _referenceBToken, currentTime);
                    }

                    if (!_indexEMALoaded)
                    {
                        LoadBInstrumentEMA(_baseInstrumentToken, 20, currentTime);
                    }
                    else
                    {
                        _indexEMAValue = _indexEMA.Process(_baseInstrumentPrice, isFinal: false);
                    }
                    if (!_rindexEMALoaded)
                    {
                        LoadBInstrumentEMA(_referenceBToken, 20, currentTime);
                    }
                    else
                    {
                        _rindexEMAValue = _rindexEMA.Process(_referenceBToken, isFinal: false);
                    }
                    //}
#if local
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (_baseCandleLoaded && _referenceCandleLoaded)
            {
                ...
                //{
                    ...
                    //                        ((e.ClosePrice < _cpr.Prices[(int)PivotLevel.R3] || e.ClosePrice > _cpr.Prices[(int)PivotLevel.S3]) ? ValidateRiskReward(e, out tl, out sl, out s2r2Level) : ValidateRiskRewardFromWeeklyPivots(e))
                    ...
                    if (_bCandle.OpenTime.TimeOfDay == MARKET_START_TIME)
                    {
                        decimal candleBody = Math.Abs(_bCandle.ClosePrice - _bCandle.OpenPrice);
                        decimal candleSize = _bCandle.HighPrice - _bCandle.LowPrice;
                        bool bullish = _bCandle.ClosePrice > _bCandle.OpenPrice;
                        if ((candleBody > CANDLE_BODY_EXTREME) &&
                            ((candleBody > candleSize * 0.75m) || ((bullish && ((_bCandle.OpenPrice - _bCandle.LowPrice) >= 2.0m * (_bCandle.HighPrice - _bCandle.OpenPrice)))
                            || (!bullish && ((_bCandle.OpenPrice - _bCandle.LowPrice) * 2.0m <= (_bCandle.HighPrice - _bCandle.OpenPrice))))))
                        {
                            _extremeScenario = (bullish) ? MarketScenario.Bullish : MarketScenario.Bearish;
                        }
                    }
                    else if (_bCandle.OpenTime.TimeOfDay == EXTREME_END_TIME)
                    {
                        _extremeScenario = MarketScenario.Neutral;
                    }
                if(_bCandle.OpenTime.TimeOfDay >= MARKET_START_TIME 
                    && _bCandle.OpenTime.TimeOfDay <= INITIAL_BALANCE_END_TIME)
                {
                    _ibHighestPrice = Math.Max(_ibHighestPrice, _bCandle.HighPrice);
                    _ibrHighestPrice = Math.Max(_ibrHighestPrice, _rCandle.HighPrice);

                    _ibLowestPrice = Math.Max(_ibLowestPrice, _bCandle.LowPrice);
                    _ibrLowestPrice = Math.Max(_ibrLowestPrice, _rCandle.LowPrice);

                }
                else if (_bCandle.OpenTime.TimeOfDay > INITIAL_BALANCE_END_TIME && !_ibLoaded)
                {
                    _ibLoaded = true;
                    _criticalLevels.TryAdd(_ibLowestPrice, 51);
                    _criticalLevels.TryAdd(_ibHighestPrice, 52);
                    _rcriticalLevels.TryAdd(_ibrLowestPrice, 51);
                    _rcriticalLevels.TryAdd(_ibrHighestPrice, 52);
                }

                decimal sHigh = 0, sLow = 0;
                //critical levels for swing high and low at 60 & 70 range
                if(_swingHighPrice < _bCandle.HighPrice)
                {
                    LastSwingHigh(e.CloseTime, _baseInstrumentToken, _bCandle.ClosePrice, true, out sHigh, out sLow);
                    _swingHighPrice = Math.Min(sHigh, sLow);
                    _criticalLevels.TryAdd(sHigh, _swingCounter++);
                    _criticalLevels.TryAdd(sLow, _swingCounter++);
                }
                 if (_swingLowPrice > _bCandle.LowPrice)
                {
                    LastSwingHigh(e.CloseTime, _baseInstrumentToken, _bCandle.ClosePrice, false, out sHigh, out sLow);
                    _swingLowPrice = Math.Max(sHigh, sLow);
                    _criticalLevels.TryAdd(sHigh, _swingCounter++);
                    _criticalLevels.TryAdd(sLow, _swingCounter++);
                }
                if (_rswingHighPrice < _rCandle.HighPrice)
                {
                    LastSwingHigh(e.CloseTime, _referenceBToken, _rCandle.ClosePrice, true, out sHigh, out sLow);
                    _rswingHighPrice = Math.Min(sHigh, sLow);
                    _rcriticalLevels.TryAdd(sHigh, _rswingCounter++);
                    _rcriticalLevels.TryAdd(sLow, _rswingCounter++);
                }
                if (_rswingLowPrice > _rCandle.LowPrice)
                {
                    LastSwingHigh(e.CloseTime, _referenceBToken, _rCandle.ClosePrice, false, out sHigh, out sLow);
                    _rswingLowPrice = Math.Max(sHigh, sLow);
                    _rcriticalLevels.TryAdd(sHigh, _rswingCounter++);
                    _rcriticalLevels.TryAdd(sLow, _rswingCounter++);
                }
                ...
                //LastSwingHigh(e.CloseTime, _referenceBToken, _rCandle.ClosePrice, out sHigh, out sLow);
                ...
                decimal targetLevel, stopLevel;
                    ...
                    if (ValidateRiskReward(_bCandle, out targetLevel, out stopLevel, out s2r2Level)
                    && ValidateCandleSize(_bCandle, s2r2Level)
                    && ValidateMarketScenario(_bCandle, _pCandle, _extremeScenario)
                    && ValidateRRiskReward(_rCandle)
                    )
                    {
                        //PostOrderInKotak(_activeFuture, _bCandle.CloseTime, _tradeQty, _bCandle.ClosePrice > _bCandle.OpenPrice);

                        if (_orderTrios.Count > 0
                            && ((_orderTrios[0].Order.TransactionType.ToLower() == "buy" && _bCandle.ClosePrice > _bCandle.OpenPrice)
                            || (_orderTrios[0].Order.TransactionType.ToLower() == "sell" && _bCandle.ClosePrice < _bCandle.OpenPrice)))
                        {
                        }
                        else
                        {
                            int qty = _tradeQty;
                            //if (_orderTrios.Count > 0
                            //&& ((_orderTrios[0].Order.TransactionType.ToLower() == "sell" && _bCandle.ClosePrice > _bCandle.OpenPrice) || (_orderTrios[0].Order.TransactionType.ToLower() == "buy" && _bCandle.ClosePrice < _bCandle.OpenPrice)))
                            //{
                            //    qty = _tradeQty * 2;
                            //}
                            if (_orderTrios.Count == 0)
                            {
                                bool buy = _bCandle.ClosePrice > _bCandle.OpenPrice;
                                OrderTrio orderTrio = new OrderTrio();
                                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice, //_bCandle.ClosePrice,
                                   _activeFuture.KToken, buy, qty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, _bCandle.CloseTime, Tag: String.Format("TP:{0}, SL{1}", targetLevel, stopLevel),
                                ...
                                //orderTrio.TPOrder = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", targetLevel,
                                //   _activeFuture.KToken, _bCandle.ClosePrice < _bCandle.OpenPrice, qty, algoIndex, _bCandle.CloseTime, orderType: "LIMIT",
                                //   broker: Constants.KOTAK, httpClient: KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
...
#if !BACKTEST
                                OnCriticalEvents(_bCandle.CloseTime.ToShortTimeString(), String.Format("{0} @ {1}", buy ? "Bought " : "Sold ", Math.Round(orderTrio.Order.AveragePrice, 2)));
#endif
                            }
                        }
                    }
                    CheckSL(_bCandle.CloseTime, _bCandle.ClosePrice, _pCandle);
                    ...
                    //CheckSLForEMABasedOrders(_bCandle.CloseTime, _bCandle.ClosePrice, _pCandle);
                    //Closes all postions at 3:20 PM
                    TriggerEODPositionClose(_bCandle.CloseTime, _bCandle.ClosePrice);
                //}
        ...
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 25, 00))
            {
        ...
        //{
        //    DataLogic dl = new DataLogic();
        //    DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.TJ2, Convert.ToDateTime("2021-11-30"), Convert.ToDateTime("2021-12-30"));
