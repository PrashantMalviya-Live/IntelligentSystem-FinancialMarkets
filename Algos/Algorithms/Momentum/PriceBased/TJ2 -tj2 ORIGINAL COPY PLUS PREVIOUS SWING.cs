...
namespace Algorithms.Algorithms
{
    public class TJ3 : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        public TJ3(TimeSpan candleTimeSpan, uint baseInstrumentToken,
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                int tradeQty = 0;
                if (htred == 0 && (!_peSLHit || (htvalue > _previousUpswingValue && _peSLHit)))
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - 5)// && _putTriggered)
                    {
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + 5))//&& _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive = true;
