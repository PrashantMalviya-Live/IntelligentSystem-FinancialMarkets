...
//#endif
        ...
        private void ActiveTradeIntraday(Tick tick)
        {
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    {
                        if (_cpr.ContainsKey(_baseInstrumentToken) == null || _weeklycpr == null)
                        {
                    ...
                    if (!_HTLoaded.Contains(_activeFuture.InstrumentToken))
                    {
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (e.InstrumentToken == _baseInstrumentToken)
            {
                ...
                if (_stockTokenHalfTrend.ContainsKey(_baseInstrumentToken) && e.CloseTime.TimeOfDay > MARKET_START_TIME)
                {
                    _stockTokenHalfTrend[_baseInstrumentToken].Process(e);
                }
                ...
                if (_firstCandleOutsideRange.GetValueOrDefault(false))
                {
                    ...
                    decimal htred = _stockTokenHalfTrend[e.InstrumentToken].GetTrend(); // 0 is up trend
                    decimal htvalue = _stockTokenHalfTrend[e.InstrumentToken].GetCurrentValue<decimal>(); // 0 is up trend
                    ...
                    if ((e.ClosePrice > _previousDayOHLC.High || e.ClosePrice < _previousDayOHLC.Low)
                        ...
                        ((e.ClosePrice < _cpr.Prices[(int)PivotLevel.R3] 
                        || e.ClosePrice > _cpr.Prices[(int)PivotLevel.S3]) ? ValidateRiskReward(e, htred, htvalue, out tl, out sl, out s2r2Level) : ValidateRiskRewardFromWeeklyPivots(e))
                ...
                if (e.CloseTime.TimeOfDay.Minutes % 15 == 0)
                {
                    ...
                    if (ValidateRiskReward(e, htred, htvalue, out targetLevel, out stopLevel, out s2r2Level) 
                        ...
                        && ValidateMarketScenario(e, _pCandle, _extremeScenario))
                    {
                        ...
                        else
                        {
                            ...
                            if (_orderTrios.Count == 0  && ((buy && htred == 0) || (!buy && htred == 1)))
                            {
                                ...
                                OrderTrio orderTrio = new OrderTrio();
                                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice, //e.ClosePrice,
                                   _activeFuture.KToken, buy, qty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, e.CloseTime, Tag: "Algo1",
                    ...
                    CheckSL(e.CloseTime, e.ClosePrice, _pCandle, htred);
        ...
        private bool ValidateRiskReward (Candle c, decimal htred, decimal htvalue, out decimal targetLevel, out decimal stoploss, out bool s2r2level)
        {
            ...
            else //if (ValidateCandleSize(c))
            {
                ...
                if (c.ClosePrice > _criticalLevels.Keys.Max())
                {
                ...
                else if (c.ClosePrice < _criticalLevels.Keys.Min())
                {
                ...
                if (htred == 0 && c.ClosePrice > _indexEMAValue.GetValue<decimal>() && ((c.ClosePrice > c.OpenPrice) || ((c.ClosePrice < c.OpenPrice) && (c.LowPrice <= htvalue) && (c.ClosePrice< htvalue + 10)))) // bullish scenario
                {
                    decimal plevel = 0;
                ...
                else if (htred == 1 && c.ClosePrice < _indexEMAValue.GetValue<decimal>() && ((c.ClosePrice < c.OpenPrice) || ((c.ClosePrice > c.OpenPrice) && (c.HighPrice >= htvalue) && (c.ClosePrice > htvalue - 10)))) // bearish scenario
                {
                    decimal plevel = 0;
                    foreach (var level in _criticalLevels)
                    {
