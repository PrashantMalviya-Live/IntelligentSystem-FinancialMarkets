...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>, IKConsumer
    {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (previoushtred == 1 && htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                decimal atmStrike = Math.Floor(_swingLowPrice / 100) * 100;
                        ...
                        else if (previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                decimal atmStrike = Math.Ceiling(_swingHighPrice / 100) * 100;
                                int qty = orderTrio.Order.Quantity;
        ...
        private void TakeInitialTrade(DateTime currentTime)
        {
            if (_stockTokenHalfTrend.ContainsKey(_baseInstrumentToken) && TimeCandles.ContainsKey(_baseInstrumentToken) && currentTime.TimeOfDay >= new TimeSpan(09,20,10))
            {
                ...
                if (htred == 0 && _putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - 10)
                {
                    //this line can be updated for postion sizing, based on the SL.
                    int qty = _tradeQty;
                ...
                else if (htred == 1 && _callorderTrios.Count == 0 && htvalue <= _baseInstrumentPrice + 10)
                {
                    //this line can be updated for postion sizing, based on the SL.
                    int qty = _tradeQty;
        ...
        private void LastSwingHighlow(DateTime currentTime, uint token, out decimal swingHigh, out decimal swingLow)
        {
            ...
            {
                Candle e = GetLastCandle(candles.ElementAt(i), candles.ElementAt(i++)) ;
                dcandles.Add(e);
            }
...
//        }
        ...
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 08, 00))
            {
                if (_callorderTrios.Count > 0)
                {
                    for (int i = 0; i < _callorderTrios.Count; i++)
