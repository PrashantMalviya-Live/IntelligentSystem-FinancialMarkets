...
using static Algorithms.Algorithms.BreakoutCandles;
    ...
    public class BreakoutCandles : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (e.InstrumentToken == _referenceIndex.InstrumentToken)
            {
                ...
                if (e.HighPrice >= _upwingindexvalue && (_pCandle == null || _pCandle.HighPrice < e.HighPrice))
                {
                    _upwingindexvalue = e.HighPrice;
                }
                else
                {
                    if (!_uphighswingindexvalues.Contains(_upwingindexvalue) && _upwingindexvalue !=0)
                    {
                        _uphighswingindexvalues.Add(_upwingindexvalue);
                        _upwingindexvalue = 0;
                    }
                }
                if ((e.LowPrice < _downswingindexvalue && (_pCandle == null || e.LowPrice < _pCandle.LowPrice)) || _downswingindexvalue == 0)
                {
                    _downswingindexvalue = e.LowPrice;
                }
                else
                {
                    if (!_downlowswingindexvalues.Contains(_downswingindexvalue) && _downswingindexvalue != -1)
                    {
                        _downlowswingindexvalues.Add(_downswingindexvalue);
                        _downswingindexvalue = -1;
                    }
                }
                ...
                if (_pCandle == null)
                {
                    ...
                    (_pCandle.ClosePrice > _pCandle.OpenPrice) ? (((Math.Max(e.HighPrice, _pCandle.HighPrice) - _pCandle.LowPrice) * 0.382m > Math.Abs((e.LowPrice - Math.Max(e.HighPrice, _pCandle.HighPrice)))) 
                    || ( (_pCandle.HighPrice - _pCandle.LowPrice) * 0.382m > Math.Abs(_pCandle.HighPrice - e.LowPrice) )) :
                    (((_pCandle.HighPrice - Math.Min(_pCandle.LowPrice, e.LowPrice)) * 0.382m > Math.Abs((e.HighPrice - Math.Min(_pCandle.LowPrice, e.LowPrice)))) 
                    || ( (_pCandle.HighPrice- _pCandle.LowPrice) *.382m > Math.Abs (_pCandle.LowPrice - e.HighPrice) ) )
                    )
                {
                    _pCandle = JoinCandle(_pCandle, e);
                    return;
                }
                ...
                //Two red Candles
                if (e.ClosePrice <  e.OpenPrice && _pCandle.ClosePrice< _pCandle.OpenPrice)
                {
                    //low breakdown
                    if(e.ClosePrice < _pCandle.LowPrice)
                    {
                        decimal sl = Math.Min(e.HighPrice, _pCandle.HighPrice); //e.HighPrice  (e.HighPrice > (_pCandle.HighPrice - (_pCandle.HighPrice - _pCandle.LowPrice) * 0.615m) ? e.HighPrice : _pCandle.HighPrice);
                        decimal target = _pCandle.LowPrice - (sl - _pCandle.LowPrice) * 0.768m;

                        //_shortTrigerred.Add(target, sl);
                        Trigger trigger = new Trigger() { tradePrice = _pCandle.LowPrice, sl = sl, tp = target, slOrder = false, up = false };
                        _shortTrigerred ??= new List<Trigger>();
                        _shortTrigerred.Add(trigger);

                        //if (e.ClosePrice - target > MINIMUM_TARGET)
                        //{
                        //    //Short future
                        ...
                        //       _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, e.CloseTime,
                ...
                //Two green Candles
                else if (e.ClosePrice > e.OpenPrice && _pCandle.ClosePrice > _pCandle.OpenPrice)
                {
                    //high breakout
                    if (e.ClosePrice > _pCandle.HighPrice)
                    {
                        decimal sl = Math.Max(e.LowPrice, _pCandle.LowPrice); //(e.LowPrice < (_pCandle.LowPrice + (_pCandle.HighPrice - _pCandle.LowPrice) * 0.615m) ? e.LowPrice : _pCandle.LowPrice);
                        //decimal target = 2 * _pCandle.HighPrice - sl;
                        decimal target = _pCandle.HighPrice + (_pCandle.HighPrice - sl)*0.768m;

                        //_longTrigerred.Add(target, sl);

                        Trigger trigger = new Trigger() { tradePrice = _pCandle.HighPrice, sl = sl, tp = target, slOrder = false, up = true };
                        _longTrigerred ??= new List<Trigger>();
                        _longTrigerred.Add(trigger);


                        //if (target - e.ClosePrice > MINIMUM_TARGET)
                        //{
                        ...
                        //    OrderTrio orderTrio = new OrderTrio();
                        //    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                        //       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, e.CloseTime,
                        //       Tag: "",// String.Format("K {0}, D {1}, Pivot Price {2}", k, d, price),
                        //       product: Constants.PRODUCT_MIS, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                ...
                else if (e.ClosePrice > e.OpenPrice && _pCandle.ClosePrice < _pCandle.OpenPrice)
                {
                    ...
                    if (e.ClosePrice > _pCandle.HighPrice)
                    {
                        decimal sl = Math.Min(e.LowPrice, _pCandle.LowPrice);
                        decimal target = _pCandle.HighPrice + (_pCandle.HighPrice - sl)*0.768m;
                ...
                else if (e.ClosePrice < e.OpenPrice && _pCandle.ClosePrice > _pCandle.OpenPrice)
                {
                    ...
                    if (e.ClosePrice < _pCandle.LowPrice)
                    {
                        decimal sl = Math.Max(e.HighPrice, _pCandle.HighPrice);
                        decimal target = _pCandle.LowPrice - (sl - _pCandle.LowPrice)*0.768m;
                ...
                //{
                //    var upswingbrokenlevels = _uphighswingindexvalues.FindAll(x => x < e.ClosePrice);

                //    for(int i = 0; i < upswingbrokenlevels.Count; i++)
                //    {
                //        decimal upswinglevel = upswingbrokenlevels[i];

                //        decimal sl = _downlowswingindexvalues.Last();

                //        decimal target = upswinglevel + (upswinglevel - sl) * 0.768m;

                //        Trigger trigger = new Trigger() { tradePrice = upswinglevel, sl = sl, tp = target, slOrder = false, up = true };
                //        _longTrigerred ??= new List<Trigger>();
                //        _longTrigerred.Add(trigger);

                //        upswingbrokenlevels.RemoveAt(i);
                //        _uphighswingindexvalues.Remove(upswinglevel);
                //        i--;
                //    }
                //}
                //else
                //{
                //    var downswingbrokenlevels = _downlowswingindexvalues.FindAll(x => x > e.ClosePrice);

                //    for (int i = 0; i < downswingbrokenlevels.Count; i++)
                //    {
                //        decimal downswinglevel = downswingbrokenlevels[i];

                //        decimal sl = _uphighswingindexvalues.Last();

                //        decimal target = downswinglevel - (_upwingindexvalue - downswinglevel) * 0.768m;

                //        Trigger trigger = new Trigger() { tradePrice = downswinglevel, sl = sl, tp = target, slOrder = false, up = true };
                //        _shortTrigerred ??= new List<Trigger>();
                //        _shortTrigerred.Add(trigger);

                //        downswingbrokenlevels.RemoveAt(i);
                //        _downlowswingindexvalues.Remove(downswinglevel);
                //        i--;
                //    }
                //}
...
//        }
        ...
        //}
        private void CheckSL(DateTime currentTime, List<OrderTrio> orderTrios)
        {
            if (orderTrios.Count > 0)
            {
                for (int i = 0; i < orderTrios.Count; i++)
...
//                    {
//                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
//                       _activeFuture.KToken, true, orderTrio.Order.Quantity, algoIndex, currentTime, Tag: Convert.ToString(Guid.NewGuid()),
//                       product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
//                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
