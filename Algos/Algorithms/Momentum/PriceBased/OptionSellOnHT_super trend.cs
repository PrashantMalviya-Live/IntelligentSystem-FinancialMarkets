...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
                ...
                if (currentTime.TimeOfDay > MARKET_START_TIME)
                {
                    ...
                    lock (tradeLock)
                    {
                        ...
                        if (SubscriptionTokens.Contains(token))
                        {
                            ...
                            if (OptionUniverse != null || !_stockTokenHalfTrend.ContainsKey(_baseInstrumentToken))
                            {
                                UpdateOptionPrice(tick);
                                decimal htrend = _stockTokenHalfTrend[_baseInstrumentToken].GetTrend();
                                if (tick.InstrumentToken == _baseInstrumentToken || tick.InstrumentToken == _referenceBToken)
                                {
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                 if (e.InstrumentToken == _referenceBToken)
                {
                    if (e.TotalVolume != 0)
                    {
                ...
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (suspensionWindow && previoushtred == 1 && htred == 0 && e.OpenTime.TimeOfDay > MARKET_START_TIME)
                        {
                            ...
                            if (e.ClosePrice > _bSuperTrendFast.GetValue() && e.ClosePrice > _bSuperTrendSlow.GetValue())// && _aboveVWAP)
                            {
                                //_callTrigerred = false;
                                _putTriggered = true;
                        ...
                        if (_callorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _callorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice > _bSuperTrendFast.GetValue() && htred == 0)
                                {
                                    Instrument option = orderTrio.Option;
                        ...
                        if (_putorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _putorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice < _bSuperTrendFast.GetValue() && htred == 1)
                                {
                                    Instrument option = orderTrio.Option;
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putTriggered && _putorderTrios.Count == 0 && _aboveVWAP && (htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5? 31: 11) ))
                    {
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && _callTrigerred && !_aboveVWAP && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes == 5 ? 31 : 11)))
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive = true;
        ...
        private decimal GetPEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            else
            {
                ...
                strike = Math.Floor(Math.Max(_bSuperTrendSlow.GetValue(), bPrice) / 100) * 100;
            }
        ...
        private decimal GetCEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            else
            {
                ...
                strike = Math.Ceiling(Math.Max(_bSuperTrendSlow.GetValue(), bPrice) / 100) * 100;
    ...
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit, ref bool firstTargetMoved, ref bool secondTargetMoved)
        ...
        private void LoadOptionsToTrade(DateTime currentTime, bool forceReload = false)
        {
            try
            {
                   ...
                   || forceReload )
                {
                    ...
                    DateTime monthlyExpiry = dl.GetCurrentMonthlyExpiry(currentTime, _baseInstrumentToken);
                    Instrument future = dl.GetInstrument(monthlyExpiry, _baseInstrumentToken, 0, "Fut");
                    _referenceBToken = future.InstrumentToken;
        ...
        private void UpdateInstrumentSubscription(DateTime currentTime)
        {
            try
            {
                ...
                if (!SubscriptionTokens.Contains(_referenceBToken))
                {
                    SubscriptionTokens.Add(_referenceBToken);
                    dataUpdated = true;
                }
