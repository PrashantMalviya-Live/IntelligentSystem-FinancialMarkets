...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private readonly int SLMARGIN = 0;//-50;//30;
        ...
        private int _putLossBookedQty = 0;
        private int _callLossBookedQty = 0;
                        ...
                        if (suspensionWindow && previoushtred == 1 && htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    ...
                                    orderTrio.Option = _atmOption;
                                    orderTrio.BaseInstrumentStopLoss = _downtrendlowswingindexvalue - SLMARGIN;
                        ...
                        else if (suspensionWindow && previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    ...
                                    orderTrio.Option = _atmOption;
                                    orderTrio.BaseInstrumentStopLoss = _uptrendhighswingindexvalue + SLMARGIN;
                                ...
                                else
                                {
                                    ...
                                    else if (shouldbeQty < totalTradedQty)
                                    {
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                int tradeQty = 0;
                if (_putTriggered && (!_peSLHit || (htvalue > _previousUpswingValue && _peSLHit)))//&& _putTriggered)
                {
                    decimal stopLoss = 0;
                    if (/*htred == 0 && */_putTriggered && _putorderTrios.Count == 0 && (htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5 ? 31 : 11)))
                    {
                        _firstPETargetActive = true;
                        _secondPETargetActive = true;
                        _thirdPETargetActive = true;
                        ...
                        _putProfitBookedQty = 0;
                        _putLossBookedQty = 0;
                        stopLoss = _downtrendlowswingindexvalue - SLMARGIN;
                    ...
                    else if (_putTriggered && _putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].BaseInstrumentStopLoss < 300))
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), _putProfitBookedQty + _putLossBookedQty, _thirdPETargetActive) - _putorderTrios.Sum(x => x.Order.Quantity);
                    ...
                    //No ITM selling also
                    if (tradeQty == 0 || (_baseInstrumentPrice > stopLoss + 100) || (tradeQty < 0 && (_baseInstrumentPrice - stopLoss) > 50) 
                        || (_putorderTrios.Count == 0 && (_baseInstrumentPrice - stopLoss) < 50))
                    {
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        _putLossBookedQty = tradeQty;
                        for (int i = 0; i < _putorderTrios.Count; i++)
                        {
                            orderTrio = _putorderTrios[i];
                            qty = orderTrio.Order.Quantity;

                            if (qty > Math.Abs(tradeQty))
                            {
                                Order order = MarketOrders.PlaceOrder(_algoInstance, orderTrio.Option.TradingSymbol, "pe", orderTrio.Option.LastPrice,
                                    orderTrio.Option.KToken, true, Math.Abs(tradeQty), algoIndex, currentTime, Tag: "", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                ...
                                //    _orderTrios.Add(orderTrio);
                                //}
                                orderTrio.Order.Quantity = qty - Math.Abs(tradeQty);
                                _pnl += order.Quantity * orderTrio.Order.AveragePrice * -1;
                        ...
                        _putLossBookedQty = _putorderTrios.Count == 0? 0 : _putLossBookedQty;
                        _putProfitBookedQty = _putorderTrios.Count == 0 ? 0 : _putProfitBookedQty;
                    }
                }
                else if (_callTrigerred && (!_ceSLHit || (_ceSLHit &&  htvalue < _previousDownswingValue)))// && _callTrigerred)
                {
                    decimal stopLoss = 0;
                    if (/*htred == 1 &&*/ _callorderTrios.Count == 0 && _callTrigerred && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes == 5 ? 31 : 11)))
                    {
                        ...
                        stopLoss = _uptrendhighswingindexvalue + SLMARGIN;
                    ...
                    else if (_callTrigerred && _callorderTrios.Count != 0 && (_callorderTrios[0].BaseInstrumentStopLoss - _baseInstrumentPrice < 300))
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), _callProfitBookedQty + _callLossBookedQty, _thirdCETargetActive) - _callorderTrios.Sum(x => x.Order.Quantity);
                    ...
                    if (tradeQty == 0 || (_baseInstrumentPrice < stopLoss - 100) || (tradeQty < 0 && (stopLoss - _baseInstrumentPrice) > 50)
                        || (_callorderTrios.Count == 0 && (stopLoss - _baseInstrumentPrice) < 50))
                    {
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < _callorderTrios.Count; i++)
                        {
                            ...
                            if (qty > Math.Abs(tradeQty))
                            {
                                Order order = MarketOrders.PlaceOrder(_algoInstance, orderTrio.Option.TradingSymbol, "ce", orderTrio.Option.LastPrice,
                           orderTrio.Option.KToken, false, qty, algoIndex, currentTime, Tag: "", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                           broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                        ...
                        _callLossBookedQty = _callorderTrios.Count == 0 ? 0 : _callLossBookedQty;
                        _callProfitBookedQty = _callorderTrios.Count == 0 ? 0 : _callProfitBookedQty;
                    }
                }
            }
        }

        private void TrailStopLoss(List<OrderTrio> orderTrios, ref decimal tradeCurrentProfit)
        {
            if (orderTrios.Count > 0)
            {
                ...
                decimal stoploss = orderTrios[0].BaseInstrumentStopLoss;
                ...
                else if (currentProfit - tradeCurrentProfit > SL_TRAIL_LIMIT)
                {
                    if (tradeCurrentProfit == 0)
                    {
                        tradeCurrentProfit = 2;
                    }
                }

                if (currentProfit < tradeCurrentProfit && tradeCurrentProfit != 0)
                {
                    ...
                    //    {
                    //        stoploss = Math.Min(stoploss, orderTrios[0].Order.AveragePrice - SL_TRAIL_LIMIT * (j - 1));
                    //        break;
                    //    }
                    //}
                    //if (orderTrios[0].Option.LastPrice > stoploss)
                    //{
        ...
        private void TrailPNLStopLoss(DateTime currentTime)
        {
            ...
            for (int i = 0; i < _putorderTrios.Count; i++)
            {
                ...
                currentProfit += orderTrio.Option.LastPrice * order.Quantity * -1;
            }
            for (int i = 0; i < _callorderTrios.Count; i++)
            {
                var orderTrio = _callorderTrios[i];
            ...
            if (currentProfit > PNL_PROFIT_TARGET)
            {
        ...
        private decimal GetPEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            //if (_putorderTrios.Count > 0)
            //{
            //    strike = _putorderTrios[0].Option.Strike;
            //}
            //else
            //{
        ...
        private decimal GetCEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            //if (_callorderTrios.Count > 0)
            //{
            //    strike = _callorderTrios[0].Option.Strike;
            //}
            //else
            //{
                if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
    ...
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit)
        ...
        private int GetTradedQuantity(decimal triggerLevel, decimal SL, int totalQuantity, int bookedQty, bool thirdTargetActive)
        {
            ...
            else if (Math.Abs(triggerLevel - SL) < 150)
            {
                ...
                tradedQuantity = totalQuantity * 4 / 4;
                tradedQuantity -= tradedQuantity % 25;
            }
            else if (Math.Abs(triggerLevel - SL) < 270)
            {
                //tradedQuantity = bookedQty != 0 ? totalQuantity * 2 / 4 : totalQuantity * 3 / 4;
                tradedQuantity = totalQuantity * 2 / 4;
                tradedQuantity -= tradedQuantity % 25;
