    ...
    public class TJ4 : IZMQ, IObserver<Tick>, IKConsumer
    {
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (_baseCandleLoaded)// && _referenceCandleLoaded)
            {
                ...
                if (_stockTokenStochastic.ContainsKey(_bCandle.InstrumentToken))
                {
                    _stockTokenStochastic[_bCandle.InstrumentToken].Process(_bCandle);
                    ...
                    if (e.OpenTime.TimeOfDay >= new TimeSpan(10, 45, 0) && _stockTokenCPR.ContainsKey(_bCandle.InstrumentToken))
                    {
                        foreach (decimal price in _stockTokenCPR[_bCandle.InstrumentToken].Prices)
                        {
                            if (e.ClosePrice < price && e.OpenPrice > price //&& price != _stockTokenCPR[_bCandle.InstrumentToken].Prices[(int)PivotLevel.LCPR]
                                                                            //&& price != _stockTokenCPR[_bCandle.InstrumentToken].Prices[(int)PivotLevel.UCPR]
                                && k < d
                            ...
                            else if (e.ClosePrice > price && e.OpenPrice < price //&& price != _stockTokenCPR[_bCandle.InstrumentToken].Prices[(int)PivotLevel.LCPR]
                                                                                 //&& price != _stockTokenCPR[_bCandle.InstrumentToken].Prices[(int)PivotLevel.UCPR]
                                && k > d
                                    & _bCandle.ClosePrice > _stockTokenVWAP[_bCandle.InstrumentToken]
                                    )
        ...
        private void LoadHistoricalCandlesForSch(List<uint> tokenList, int candlesCount, DateTime lastCandleEndTime, TimeSpan timeSpan)
        {
            ...
            lock (_stockTokenStochastic)
            {
                ...
                foreach (uint token in tokenList)
                {
                    ...
                    foreach (var price in historicals)
                    {
                        ...
                        if (_stockTokenStochastic.ContainsKey(tc.InstrumentToken))
                        {
                            _stockTokenStochastic[tc.InstrumentToken].Process(tc);
                        }
                        else
                        {
                            sch = new StochasticOscillator();
                            sch.Process(tc);
                            _stockTokenStochastic.TryAdd(tc.InstrumentToken, sch);
                        }
                    }
                }
            }
