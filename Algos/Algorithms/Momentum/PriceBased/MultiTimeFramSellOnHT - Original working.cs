...
{
    public class MultiTimeFramSellOnHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(MultiTimeFramSellOnHT source);
        [field: NonSerialized]
        ...
        //minutes and half trend for index
        Dictionary<uint, HalfTrend> _indexTimeHalfTrend;
        ...
        public const AlgoIndex algoIndex = AlgoIndex.MultiTimeFrameSellOnHT;
        ...
        private List<HalfTrend> ctriggerHT, ptriggerHT;

        public MultiTimeFramSellOnHT(TimeSpan candleTimeSpan, uint baseInstrumentToken,
        ...
        private void ActiveTradeIntraday(Tick tick)
        {
            ...
            try
            {
                ...
                EvaluateTargetStopLoss(stopLoss: false, currentTime);
        ...
        {
            decimal h1tred = _indexTimeHalfTrend[1].GetTrend(); // 0 is up trend
            decimal ht1value = _indexTimeHalfTrend[1].GetCurrentValue<decimal>(); // 0 is up trend
            decimal h3tred = _indexTimeHalfTrend[3].GetTrend(); // 0 is up trend
            decimal ht3value = _indexTimeHalfTrend[3].GetCurrentValue<decimal>(); // 0 is up trend
            decimal h5tred = _indexTimeHalfTrend[5].GetTrend(); // 0 is up trend
            decimal ht5value = _indexTimeHalfTrend[5].GetCurrentValue<decimal>(); // 0 is up trend
            decimal h15tred = _indexTimeHalfTrend[15].GetTrend(); // 0 is up trend
            decimal ht15value = _indexTimeHalfTrend[15].GetCurrentValue<decimal>(); // 0 is up trend
            decimal h30tred = _indexTimeHalfTrend[30].GetTrend(); // 0 is up trend
            decimal ht30value = _indexTimeHalfTrend[30].GetCurrentValue<decimal>(); // 0 is up trend
            decimal h45tred = _indexTimeHalfTrend[45].GetTrend(); // 0 is up trend
            decimal ht45value = _indexTimeHalfTrend[45].GetCurrentValue<decimal>(); // 0 is up trend

            //short term bearish
            if (h1tred == 1 && h3tred == 0)
            {
                Candle timeCandle = TimeCandles[_baseInstrumentToken].LastOrDefault(x => x.State == CandleStates.Inprogress) as TimeFrameCandle;
                _indexTimeHalfTrend[1].Process(timeCandle);
        ...
        /// <summary>
        /// one minute candles time span..and will be consolidated for 3 mins, 5 mins and 15 mins
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ///Take 3 min candle, 5 min candle and 15 mins candle
            ///In 3 mins candle determine when the trend is opposite to 5 mins candle, and near 5 mins candle sL, and take opossite trade. and book profit when the trend is in the same direction.
            ///same for all the types of candles.
            ///update HT live with not-final inputs and at candles with final inputs. SL hit at high time frame candle close and then trend becomes same as lower time frame candle.
            ...
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    //Candle c1 = GetLastCandle(1, e.CloseTime);
                    _indexTimeHalfTrend[1].Process(e);

                    //Below logic runs at 3 mins candle
                    if ((e.CloseTime.TimeOfDay.TotalMinutes - MARKET_START_TIME.TotalMinutes) % 3 == 0)
                    {
                        Candle c3 = GetLastCandle(3, e.CloseTime);
                        _indexTimeHalfTrend[3].Process(c3);
                    }
                    //Below logic runs at 5 mins candle
                    if ((e.CloseTime.TimeOfDay.TotalMinutes - MARKET_START_TIME.TotalMinutes) % 5 == 0)
                    {
                        Candle c5 = GetLastCandle(5, e.CloseTime);
                        _indexTimeHalfTrend[5].Process(c5);
                    }
                    //Below logic runs at 15 mins candle
                    if ((e.CloseTime.TimeOfDay.TotalMinutes - MARKET_START_TIME.TotalMinutes) % 15 == 0)
                    {
                        Candle c15 = GetLastCandle(15, e.CloseTime);
                        _indexTimeHalfTrend[15].Process(c15);
                    }
                    ////Below logic runs at 30 mins candle  
                    //if ((e.CloseTime.TimeOfDay.TotalMinutes - MARKET_START_TIME.TotalMinutes) % 30 == 0)
                    //{
                    //    Candle c30 = GetLastCandle(30, e.CloseTime);
                    //    _indexTimeHalfTrend[30].Process(c30);
                    //}
                    ////Below logic runs at 45 mins candle
                    //if ((e.CloseTime.TimeOfDay.TotalMinutes - MARKET_START_TIME.TotalMinutes) % 45 == 0)
                    //{
                    //    Candle c45 = GetLastCandle(45, e.CloseTime);
                    //    _indexTimeHalfTrend[45].Process(c45);
                    //}
                    ...
                    _triggers.Update(h1tred, h3tred, h5tred, h15tred);

                    //Long term trend is positive, middle term trend is negative, short term trend became positive, take postive trend and vice versa


                    // _triggers = new Triggers(h1tred, h3tred, h5tred, h15tred);



                    //Case 1: 5 min trend is positive, 3 min trend is negative and 1 min trend has become positive

                    // if ((h5tred == 0 && h3tred == 1 && h1tred == 0) || (h15tred == 0 && h5tred == 1 && h3tred == 0))
                    //{

                    //LastSwingHighlow(e.CloseTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice, h3tred == 0 ? 1 : 3);

                    //if (h3tred == 1 && !ptriggerHT.Contains(_indexTimeHalfTrend[3]) || h5tred == 1 && !ptriggerHT.Contains(_indexTimeHalfTrend[5]))
                    //{
                    //if (h3tred == 1)
                    //{
                    //    ptriggerHT.Add(_indexTimeHalfTrend[3]);
                    //}
                    //else if (h5tred == 1)
                    //{
                    //    ptriggerHT.Add(_indexTimeHalfTrend[5]);
                    //}

                    if (((h5tred == 0 && h3tred == 1 && h1tred == 0) && !_triggers.T1) || (h15tred == 0 && h5tred == 1 && h3tred == 0) && !_triggers.T2)
                    {
                        _triggers.T1 = (h5tred == 0 && h3tred == 1 && h1tred == 0);
                        _triggers.T2 = (h15tred == 0 && h5tred == 1 && h3tred == 0);
                        //_triggers = new Triggers(h1tred, h3tred, h5tred, h15tred);
                        ...
                        //Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][Math.Round(_swingLowPrice / 100) * 100];
                        Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][Math.Round(_baseInstrumentPrice / 100) * 100];
                        ...
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, "pe", _atmOption.LastPrice,
                           ...
                           Tag: String.Format("h3tred:{0}", h3tred),
                        ...
                        orderTrio.TargetProfit = Math.Max(2m * _baseInstrumentPrice - _swingLowPrice, 10);
                        //orderTrio.TargetProfit = Math.Max(Math.Max(1.7m * _baseInstrumentPrice - 0.7m * _swingLowPrice, _swingHighPrice), 10);
                        ...
                        //decimal atmStrike = Math.Round(_swingLowPrice / 100) * 100;
                        ...
                        //       Tag: "",
                        //       product: Constants.PRODUCT_MIS, broker: Constants.KOTAK,
                        ...
                        //    decimal targetProfit = orderTrio.TargetProfit;
                        //    decimal stopLoss = orderTrio.StopLoss;
                        ...
                        //    //buyback existing option, and sell at new swing low
                        //    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, "pe", _atmOption.LastPrice,
                        //       _atmOption.KToken, false, orderTrio.Order.Quantity, algoIndex, _bCandle.CloseTime,
                        //       Tag: "",
                        //       product: Constants.PRODUCT_MIS, broker: Constants.KOTAK,
                        //       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                        ...
                        //    orderTrio.Option = _atmOption;
                        //    orderTrio.StopLoss = stopLoss;// _swingLowPrice;
                        //    _initialStopLoss = _swingLowPrice;
                        //    //first target is 1:1 R&R
                        //    orderTrio.TargetProfit = targetProfit;// Math.Max(1.7m * _baseInstrumentPrice - 0.7m*_swingLowPrice, _swingHighPrice);
                        //    _lastOrderTransactionType = "sell";
                        //    orderTrio.Order.Quantity = _tradeQty * Convert.ToInt32(_atmOption.LotSize);
                    ...
                    // }
                    //}
                    //else if ((h5tred == 1 && h3tred == 0 && h1tred == 1) || (h15tred == 1 && h5tred == 0 && h3tred == 1))
                    else if (((h5tred == 1 && h3tred == 0 && h1tred == 1) && !_triggers.T3) || ((h15tred == 1 && h5tred == 0 && h3tred == 1) && !_triggers.T4))
                    {
                        //_triggers = new Triggers(h1tred, h3tred, h5tred, h15tred);
                        _triggers.T3 = (h5tred == 1 && h3tred == 0 && h1tred == 1);
                        _triggers.T4 = (h15tred == 1 && h5tred == 0 && h3tred == 1);


                        //LastSwingHighlow(e.CloseTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice, h3tred == 0 ? 1 : 3);

                        //if (h3tred == 0 && !ctriggerHT.Contains(_indexTimeHalfTrend[3]) || h5tred == 0 && !ctriggerHT.Contains(_indexTimeHalfTrend[5]))
                        //{
                        //    if (h3tred == 1)
                        //    {
                        //        ctriggerHT.Add(_indexTimeHalfTrend[3]);
                        //    }
                        //    else if (h5tred == 1)
                        //    {
                        //       ctriggerHT.Add(_indexTimeHalfTrend[5]);
                        //    }
                        ...
                        OrderTrio orderTrio = new OrderTrio();
                        //Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Round(_swingHighPrice / 100) * 100];
                        Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Round(_baseInstrumentPrice / 100) * 100];
                        ...
                        //first target is 1:1 R&R
                        orderTrio.TargetProfit =  Math.Max(2 * _baseInstrumentPrice - _swingHighPrice, 10);
                        //orderTrio.TargetProfit = Math.Max(Math.Min(2 * _baseInstrumentPrice - _swingHighPrice, _swingLowPrice), 10);
                        _lastOrderTransactionType = "sell";
                        orderTrio.Order.Quantity = _tradeQty * Convert.ToInt32(_atmOption.LotSize);
                        _pnl += orderTrio.Order.Quantity * orderTrio.Order.AveragePrice * 1;
                        ...
                        //decimal atmStrike = Math.Round(_swingHighPrice / 100) * 100;
                        ...
                        //    orderTrio.Option = _atmOption;
                        //    orderTrio.StopLoss = stopLoss;// _swingHighPrice;
                        //    _initialStopLoss = _swingHighPrice;
                        //    //first target is 1:1 R&R
                        //    //Do not change the target trofit while trailing
                        //    orderTrio.TargetProfit = targetProfit; //Math.Min(1.7m * _baseInstrumentPrice - 0.7m*_swingHighPrice, _swingLowPrice);
                        //    _lastOrderTransactionType = "sell";
                        //    orderTrio.Order.Quantity = _tradeQty * Convert.ToInt32(_atmOption.LotSize);
                    ...
                    EvaluateTargetStopLoss(stopLoss: true, e.CloseTime);


                    #region Halftrend (5 mins), Candle time frame 5 mins.

                    //if (_indexTimeHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    //{
                    //    _indexTimeHalfTrend[_bCandle.InstrumentToken].Process(_bCandle);

                    //    decimal htred = _indexTimeHalfTrend[_bCandle.InstrumentToken].GetTrend(); // 0 is up trend
                    //    decimal htvalue = _indexTimeHalfTrend[_bCandle.InstrumentToken].GetCurrentValue<decimal>(); // 0 is up trend
                    ...
                    //            orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, "pe", _atmOption.LastPrice,
                    //               _atmOption.KToken, false, qty * Convert.ToInt32(_atmOption.LotSize), algoIndex, _bCandle.CloseTime,
                    //               Tag: String.Format("htred {0}, htvalue {1}", htred, htvalue),
                    //               product: Constants.PRODUCT_MIS, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                    ...
                    //        }
                    //    }
                    //    else if (_previoushtred == 0 && htred == 1)
                    //    {
                    //        LastSwingHighlow(e.CloseTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                    ...
                    //            OrderTrio orderTrio = new OrderTrio();
                    //            Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Round(_swingLowPrice / 100) * 100];
                    ...
                    //            if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _baseInstrumentPrice > orderTrio.StopLoss)
                    //            {
                    //                Instrument option = orderTrio.Option;
        ...
        private void EvaluateTargetStopLoss(bool stopLoss, DateTime currentTime)
        {
            decimal livePnl = _pnl;
            if (_callorderTrios.Count != 0)
            {
                for (int i = 0; i < _callorderTrios.Count; i++)
                {
                    var orderTrio = _callorderTrios[i];
                    livePnl += orderTrio.Order.Quantity * orderTrio.Option.LastPrice * -1;
                    if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && (((_baseInstrumentPrice > orderTrio.StopLoss) && (stopLoss)) || _baseInstrumentPrice < orderTrio.TargetProfit))
                    {
                        Instrument option = orderTrio.Option;
            ...
            if (livePnl < _stopLoss * -1)
            {
                TriggerEODPositionClose(currentTime, /*lastPrice,*/ closeAll: true);
            }
        }
        ...
        private Candle GetLastCandle(List<Candle> c)
        {
            TimeFrameCandle tC = new TimeFrameCandle();

            //for (int i = 0; i < c.Count; i++)
            //{
                tC.InstrumentToken = c[0].InstrumentToken;
                tC.OpenPrice = c[0].OpenPrice;
                tC.OpenTime = c[0].OpenTime;
                tC.ClosePrice = c[c.Count - 1].ClosePrice;
                tC.CloseTime = c[c.Count - 1].CloseTime;
                tC.Final = true;
                tC.State = CandleStates.Finished;
                tC.HighPrice = c.Max(x=>x.HighPrice);
                tC.LowPrice = c.Min(x => x.LowPrice);
            //}
            return tC;
        }
        ...
        private void LoadHistoricalHT(DateTime currentTime)
        {
            try
            {
                ...
                foreach (uint t in tokens)
                {
                    ...
                    if (!_indexTimeHalfTrend.ContainsKey(t) && !_SQLLoadingHT.Contains(t))
                    {
                        newTokens.Add(t);
                        sb.AppendFormat("{0},", t);
                        _SQLLoadingHT.Add(t);
                ...
                foreach (uint tkn in tokens)
                {
                    ...
                    if (TimeCandles.ContainsKey(tkn) && _indexTimeHalfTrend.ContainsKey(tkn))
                    {
                        if (_firstCandleOpenPriceNeeded[tkn])
                        {
                            ...
                            _indexTimeHalfTrend[tkn].Process(TimeCandles[tkn].First());
                    ...
                    if ((firstCandleFormed == 1 || !_firstCandleOpenPriceNeeded[tkn]) && _indexTimeHalfTrend.ContainsKey(tkn))
                    {
                        _HTLoaded.Add(tkn);
        ...
        private void LoadHT(List<uint> tokenList, DateTime previousTradingDate, DateTime lastCandleEndTime, TimeSpan timeSpan)
        {
            HalfTrend ht;
            foreach (uint token in tokenList)
            {
                List<Historical> historicals = ZObjects.kite.GetHistoricalData(token.ToString(), previousTradingDate, lastCandleEndTime.AddSeconds(-10), string.Format("{0}minute", timeSpan.Minutes == 1?"": timeSpan.Minutes));
                ...
                foreach (var price in historicals)
                {
                    ...
                    tc.Final = true;
                    tc.State = CandleStates.Finished;
                    if (_indexTimeHalfTrend.ContainsKey(Convert.ToUInt32(timeSpan.Minutes)))
                    {
                        _indexTimeHalfTrend[Convert.ToUInt32(timeSpan.Minutes)].Process(tc);
                    }
                    else
                    {
                        ht = new HalfTrend(2, 2);
                        ht.Process(tc);
                        _indexTimeHalfTrend.TryAdd(Convert.ToUInt32(timeSpan.Minutes), ht);
                    }
                }
            }
        }
        ...
        private void LastSwingHighlow(DateTime currentTime, uint token, out decimal swingHigh, out decimal swingLow, int numberOfCandlesToMerge)
        {
            //max and min for last 1 hour
            IEnumerable<Candle> candles = TimeCandles[token].TakeLast(60 * numberOfCandlesToMerge*2);

            //do the analysis with double candle size
            List<Candle> dcandles = new List<Candle>();
            for (int i = 0; i < candles.Count(); i++)
            {
                List<Candle> mergedCandles = new List<Candle>();
                for (int j = 0; j < Math.Min(numberOfCandlesToMerge * 2, candles.Count() - i); j++)
                {
                    mergedCandles.Add(candles.ElementAt(i + j));
                }
                Candle e = GetLastCandle(mergedCandles);
                dcandles.Add(e);
                i = i + numberOfCandlesToMerge * 2;
            }
            ...
            ////Temp
            //candles = TimeCandles[token].TakeLast(60);
            //swingHigh = candles.Max(x => x.HighPrice);
            //swingLow = candles.Min(x => x.LowPrice);
    ...
    //trigger1: h5tred == 0 && h3tred == 1 && h1tred == 0
    //trigger2: h15tred == 0 && h5tred == 1 && h3tred == 0
    //trigger3: h5tred == 1 && h3tred == 0 && h1tred == 1
    //trigger4: h15tred == 1 && h5tred == 0 && h3tred == 1

    class Triggers
    {
        private decimal _h1tred, _h3tred, _h5tred, _h15tred;
        private bool _t1 = false, _t2 = false, _t3 = false, _t4 = false;
        public Triggers(decimal h1tred, decimal h3tred, decimal h5tred, decimal h15tred)
        {
            _h1tred = h1tred;
            _h3tred = h3tred;
            _h5tred = h5tred;
        ...
        public void Update(decimal h1tred, decimal h3tred, decimal h5tred, decimal h15tred)
        {
            _h1tred = h1tred;
            _h3tred = h3tred;
            _h5tred = h5tred;
        ...
        public bool T1
        {
            get
            {
                _t1 =  _t1 && (_h5tred == 0 && _h3tred == 1 && _h1tred == 0);
                return _t1;
            }
            set
            {
        ...
        public bool T2
        {
            get
            {
                 _t2 = _t2 && (_h3tred == 0 && _h5tred == 1 && _h15tred == 0);
                return _t2;
            }
            set
            {
        ...
        public bool T3
        {
            get
            {
                _t3 = _t3 && (_h5tred == 1 && _h3tred == 0 && _h1tred == 1);
                return _t3;
            }
            set
            {
        ...
        public bool T4
        {
            get
            {
                _t4 =_t4 && (_h15tred == 1 && _h5tred == 0 && _h3tred == 1);
                return _t4;
            }
            set
            {
