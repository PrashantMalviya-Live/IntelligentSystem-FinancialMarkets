...
namespace Algorithms.Algorithms
{
    public class BreakoutCandles : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private SortedList<decimal, decimal> _longTrigerred;
        private SortedList<decimal, decimal> _shortTrigerred;
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            _longTrigerred = new SortedList<decimal, decimal>();
            _shortTrigerred = new SortedList<decimal, decimal>();
        ...
        private void PurgeTriggers(SortedList< decimal, decimal> triggers, decimal lastPrice, bool longTrigger)
        {
            for (int i = 0; i < triggers.Count; i++)
            {
                var trigger = triggers.ElementAt(i);
                if ((longTrigger && (trigger.Value > lastPrice || trigger.Key < lastPrice)) || (!longTrigger && (trigger.Value < lastPrice || trigger.Key > lastPrice)))
                {
                    triggers.RemoveAt(i);
                    i--;
                }
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
            ...
            {
                var trigger = _longTrigerred.ElementAt(i);
                
                if (trigger.Key - _referenceIndex.LastPrice > _minimumTarget)
                {
                    ...
                    bool _intraday = true;
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][GetCEStrike(_baseInstrumentPrice, trigger.Value)]; //Change -  Take position 50 points farther to avoid gamma loss
                    OrderTrio orderTrio = new OrderTrio();
            ...
            {
                var trigger = _shortTrigerred.ElementAt(i);

                if (_referenceIndex.LastPrice - trigger.Key > _minimumTarget)
                {
                    ...
                    bool _intraday = true;
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][GetPEStrike(_baseInstrumentPrice, trigger.Value)]; //Change -  Take position 50 points farther to avoid gamma loss
                    OrderTrio orderTrio = new OrderTrio();
                    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, _atmOption.InstrumentType.ToLower(), _atmOption.LastPrice,
                       _atmOption.KToken, true, qty, algoIndex, currentTime,
                       Tag: "",
...
//        }
        ...
        private decimal GetPEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            {
                strike = Math.Floor((Math.Min(stoploss, bPrice) + 200) / 100) * 100;
            }
        ...
        private decimal GetCEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            {
                strike = Math.Ceiling((Math.Max(stoploss, bPrice) - 200) / 100) * 100;
