...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        Instrument _activeCall, _activePut;
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            _maxDistanceFromBInstrument = 1500;
            ...
            try
            {
                if (currentTime.TimeOfDay > MARKET_START_TIME)
                {
                    ...
                    lock (tradeLock)
                    {
                            ...
                            if (OptionUniverse != null)
                            {
                               ...
                               && BookStopLoss(_callorderTrios, htrend == 1, ref _ceCurrentProfit)
                                ...
                                else
                                {
                                    ...
                                    /*ref _preFirstPETargetActive,*/ ref _firstPETargetActive, ref _secondPETargetActive, ref _thirdPETargetActive, ref _peCurrentProfit);
                                    _callProfitBookedQty += BookPartialProfit(_callorderTrios, tick, htrend == 0,
                                        /*ref _preFirstCETargetActive,*/ ref _firstCETargetActive, ref _secondCETargetActive, ref _thirdCETargetActive, ref _ceCurrentProfit);
                                }
                            }
#if BACKTEST
                        }
                        ...
                        TrailOptions(_callorderTrios, currentTime, _baseInstrumentPrice, InstrumentType.CE);
                        TrailOptions(_putorderTrios, currentTime, _baseInstrumentPrice, InstrumentType.PE);
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        else if (suspensionWindow && previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                //decimal atmStrike = GetCEStrike(_baseInstrumentPrice, _uptrendhighswingindexvalue);
                                ...
                                //if (/*option.Strike != atmStrike &&*/ newQty > 0 && qty)
                                //{
                                //    DataLogic dl = new DataLogic();
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                if (htred == 0 && (!_peSLHit || (htvalue > _previousUpswingValue && _peSLHit)))//&& _putTriggered)
                {
                    ...
                    Instrument activePut = GetOptions(currentTime, _baseInstrumentPrice, InstrumentType.PE);

                    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, activePut.TradingSymbol, "pe", activePut.LastPrice,
                       activePut.KToken, false, qty, algoIndex, currentTime,
                ...
                else if (htred == 1 && (!_ceSLHit || (_ceSLHit &&  htvalue < _previousDownswingValue)))// && _callTrigerred)
                {
                    ...
                    Instrument activeCall = GetOptions(currentTime, _baseInstrumentPrice, InstrumentType.CE);

                    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, activeCall.TradingSymbol, "ce", activeCall.LastPrice,
                       activeCall.KToken, false, qty, algoIndex, currentTime,
                       Tag: "",
                       product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
        ...
        private void TrailOptions(List<OrderTrio> orderTrios, DateTime currentTime, decimal bPrice, InstrumentType instrumentType)
        {
            if (orderTrios.Count > 0)
            {
                OrderTrio orderTrio = orderTrios[0];
                var option = orderTrio.Option;
                double delta = option.UpdateDelta((double)option.LastPrice, 0.1, currentTime, (double)bPrice);
                if(Math.Abs(delta) > _maxDelta)
                {
                    var newOption = GetFurtherOption(currentTime, instrumentType, option);
                    if (newOption != null)
                    {
                        MoveOption(orderTrios, newOption);
                    }
                }
                else if (Math.Abs(delta) < _minDelta)
                {
                    var newOption = GetClosureOption(currentTime, instrumentType, option);
                    if (newOption != null)
                    {
                        MoveOption(orderTrios, newOption);
                    }
        ...
        private Instrument GetClosureOption(DateTime currentTime, InstrumentType instrumentType, Instrument activeOption)
        {
            ...
            if (instrumentType == InstrumentType.PE)
            {
                Instrument newPut = null;
                foreach (var putref in OptionUniverse[(int) InstrumentType.PE])
                {
                    put = putref.Value;
                    if (put.LastPrice != 0)
                    {
                ...
                if (newPut == null || newPut.Strike == activeOption.Strike)
                {
                    return null;
                }
                return newPut;
                ...
                //    _activePut.LastPrice, MappedTokens[_activePut.InstrumentToken], true, _tradedQty, algoIndex: _algoIndex,
                //    tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                //OnTradeExit(order);

                //_activePut = newPut;
                //_activePutOrders.Clear();

                //order = MarketOrders.PlaceOrder(_algoInstance, _activePut.TradingSymbol, _activePut.InstrumentType,
                //              _activePut.LastPrice, MappedTokens[_activePut.InstrumentToken], false, _tradedQty, algoIndex: _algoIndex,
                //              tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                //order.InstrumentToken = _activePut.InstrumentToken;
                //_activePutOrders.Add(order);

                //OnTradeEntry(order);
            }
            else if (instrumentType == InstrumentType.CE)
            {
                Instrument newCall = null;
                foreach (var callref in OptionUniverse[(int) InstrumentType.CE])
                {
                    call = callref.Value;
                    if (call.LastPrice != 0)
                    {
                ...
                if (newCall == null || newCall.Strike == activeOption.Strike)
                {
                    return null;
                }
                ...
                //    _activeCall.LastPrice, MappedTokens[_activeCall.InstrumentToken], true, _tradedQty, algoIndex: _algoIndex,
                //    tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);

                //OnTradeExit(order);
                //_activeCallOrders.Clear();

                //_activeCall = newCall;

                //order = MarketOrders.PlaceOrder(_algoInstance, _activeCall.TradingSymbol, _activeCall.InstrumentType,
                //            _activeCall.LastPrice, MappedTokens[_activeCall.InstrumentToken], false, _tradedQty, algoIndex: _algoIndex,
                //            tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                //order.InstrumentToken = _activeCall.InstrumentToken;
                //_activeCallOrders.Add(order);
        ...
        private Instrument GetFurtherOption(DateTime currentTime, InstrumentType instrumentType, Instrument activeOption)
        {
            Instrument call = null, put = null;
            Instrument prevCall = null, prevPut = null;
            double? prevDelta = null;
            ...
            if (instrumentType == InstrumentType.PE)
            {
                ...
                //Order order = MarketOrders.PlaceOrder(_algoInstance, _activePut.TradingSymbol, _activePut.InstrumentType,
                //    _activePut.LastPrice, MappedTokens[_activePut.InstrumentToken], true, qty, algoIndex: algoIndex,
                //    tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                //OnTradeExit(order);
                //_activePutOrders.Clear();
        ...
        //private void MoveClosure(DateTime currentTime, string instrumentType, int buyBackQty)
        //{
        //    Instrument call = null, put = null;
        //    Instrument prevCall = null, prevPut = null;
        //    double? prevDelta = null;
        ...
        //            if (buyBackQty != 0)
        //            {
        //                BuyBackTrade(buyBackQty, currentTime);
        //                _tradedQty -= buyBackQty;
        //            }
        //            return;
        //        }
        ...
        //            _tradedQty -= buyBackQty;
        //            order = MarketOrders.PlaceOrder(_algoInstance, _activeCall.TradingSymbol, _activeCall.InstrumentType,
        //                      _activeCall.LastPrice, MappedTokens[_activeCall.InstrumentToken], buyBackQty > 0, Math.Abs(buyBackQty), algoIndex: _algoIndex,
        //                      tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        //            _tradedQty -= buyBackQty;
        //            order = MarketOrders.PlaceOrder(_algoInstance, _activePut.TradingSymbol, _activePut.InstrumentType,
        //                      _activePut.LastPrice, MappedTokens[_activePut.InstrumentToken], buyBackQty > 0, Math.Abs(buyBackQty), algoIndex: _algoIndex,
        //                      tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        {
            //decimal minDelta = 0.25m;
            //decimal maxDelta = 0.35m;
            ...
            else if(instrumentType == InstrumentType.PE)
            {
                foreach (var putref in OptionUniverse[(int)InstrumentType.PE])
                {
                    put = putref.Value;
                    if (put.LastPrice != 0)
                    {
                        ...
                        if (prevDelta != null && prevDelta != 0)
                        {
                            if (Math.Abs(currentDelta) > _entryDelta && Math.Abs(prevDelta.Value) <= _entryDelta)
                            {
                                activeOption = prevPut;
                                break;
                            }
                            else if (Math.Abs(currentDelta) < _entryDelta && Math.Abs(prevDelta.Value) >= _entryDelta)
                            {
                                activeOption = put;
                                break;
                            }
        ...
        private int BookPartialProfit(List<OrderTrio> orderTrios, Tick tick, bool trendChanged,/* ref bool preFirstTradeActive,*/ ref bool firstTradeActive, ref bool secondTradeActive,
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit)
        {
