...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        //5 mins: 50,70,100. SL: 50 points
        // 1 min: 30,50,70
        //25% quantity booked in each target

        private const decimal BREAKEVEN_TARGET = 30;
        bool _cebreakeventargetreached = false;
        bool _pebreakeventargetreached = false;
        ...
        private const decimal SL_TRAIL_LIMIT = 20;
                ...
                if (currentTime.TimeOfDay > MARKET_START_TIME)
                {
                    ...
                    lock (tradeLock)
                    {
                        ...
                        if (OptionUniverse != null)
                        {
                           ...
                           && BookStopLoss(_putorderTrios, htrend == 0, ref _peCurrentProfit, ref _pebreakeventargetreached)
                           )
                                {
                           ...
                           && BookStopLoss(_callorderTrios, htrend == 1, ref _ceCurrentProfit, ref _cebreakeventargetreached)
                           )
                                {
                            ...
                            else
                            {
                                ...
                                ref _peCurrentProfit, ref _pebreakeventargetreached, ref _putTriggered);
                                _callProfitBookedQty += BookPartialProfit(_callorderTrios, tick, htrend == 0,
                                    ...
                                    ref _thirdCETargetActive, ref _ceCurrentProfit, ref _cebreakeventargetreached, ref _callTrigerred);
                            }
                        }
#if BACKTEST
                        }
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    uint token = _bCandle.InstrumentToken;
                    if (_stockTokenHalfTrend.ContainsKey(token))
                    {
                        decimal previoushtred = _stockTokenHalfTrend[token].GetTrend(); // 0 is up trend

                        if ((e.CloseTime.Minute - MARKET_START_TIME.Minutes) % (_candleTimeSpan.Minutes * 2) == 0)
                        {
                            Candle bigCandle = GetLastCandle(2, _bCandle.CloseTime);
                            _stockTokenHalfTrend[token].Process(bigCandle);
                        }
                        ...
                        if (suspensionWindow && previoushtred == 1 && htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                int newQty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, 
                                    _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, false);
                                ...
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, false);
                                ...
                                else
                                {
                                    ...
                                    int shouldbeQty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, 
                                        _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _pebreakeventargetreached);
                        ...
                        else if (suspensionWindow && previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                int qty = orderTrio.Order.Quantity;
                                int newQty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, 
                                    _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, false);
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    DataLogic dl = new DataLogic();
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, 
                                        _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, false  );
                                ...
                                else
                                {
                                    ...
                                    int shouldbeQty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _cebreakeventargetreached);
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                if (htred == 0 )//&& _putTriggered)
                {
                    decimal stopLoss = 0;
                    if (_putTriggered 
                        && (_putorderTrios.Count == 0 || (_putorderTrios.Count != 0 && _pebreakeventargetreached))
                        && (htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes >= 5? 31: 11))
                         )
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, 
                            _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _pebreakeventargetreached);
                    ...
                    else if (_putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].BaseInstrumentStopLoss < 300))
                    {
                        // TODO: COMMENTED FOR NOT INCREASING QTY LATER
                        stopLoss = _putorderTrios[0].BaseInstrumentStopLoss;
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, 
                            _tradeQty * Convert.ToInt32(_lotSize),
                            _putProfitBookedQty, _thirdPETargetActive, _pebreakeventargetreached) - _putorderTrios.Sum(x => x.Order.Quantity);
                ...
                {
                    decimal stopLoss = 0;
                    if ((_callorderTrios.Count == 0 || (_callorderTrios.Count != 0 && _cebreakeventargetreached))
                        && _callTrigerred && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes >= 5 ? 31 : 11)))
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive = true;
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _cebreakeventargetreached);
                    ...
                    else if (_callorderTrios.Count != 0 && (_callorderTrios[0].BaseInstrumentStopLoss - _baseInstrumentPrice < 300))
                    {
                        //TODO: COMMENTED FOR NOT INCREASING QTY LATER
                        stopLoss = _callorderTrios[0].BaseInstrumentStopLoss;
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), 
                            _callProfitBookedQty, _thirdCETargetActive, _cebreakeventargetreached) - _callorderTrios.Sum(x => x.Order.Quantity);
        ...
        private void TrailStopLoss(List<OrderTrio> orderTrios, ref decimal tradeCurrentProfit)
        {
            if (orderTrios.Count > 0)
            {
                ...
                {
                    bool breakeventarget = false;
                    BookStopLoss(orderTrios, false, ref tradeCurrentProfit, ref breakeventarget, closeAll: true);
        ...
        private bool BookStopLoss(List<OrderTrio> orderTrios, bool supportingTrend, ref decimal tradeCurrentProfit, ref bool breakeventargetreached,
            bool closeAll = false)
        {
            int totalTradedQty = orderTrios.Sum(x => x.Order.Quantity);
            ...
            if (currentLoss > maxLoss || closeAll || (breakeventargetreached && orderTrios[0].StopLoss < orderTrios[0].Option.LastPrice))
            {
    ...
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit, ref bool breakeventtargetreached, ref bool optionTrigerred)
        {
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                if (orderTrio.Option.InstrumentToken == tick.InstrumentToken)
                {
                    ...
                    //))
                    {
                        ...
                        // }
                        // else
                        if (!breakeventtargetreached && orderTrio.Order.AveragePrice > tick.LastPrice + BREAKEVEN_TARGET)
                        {
                            breakeventtargetreached = true;
                            orderTrio.StopLoss = orderTrio.Order.AveragePrice;
                            optionTrigerred = true;
                        }
        ...
        private int GetTradedQuantity(decimal triggerLevel, decimal SL, int totalQuantity, 
            int bookedQty, bool thirdTargetActive, bool breakeventtargetachieved)
        {
            int tradedQuantity = 0;
            ...
            //tradedQuantity = totalQuantity;
            return breakeventtargetachieved ? tradedQuantity : tradedQuantity / 2 - bookedQty;
        }
        ...
        {
            LoadHistoricalCandlesForHT(tokenList, candlesCount, lastCandleEndTime,new TimeSpan(0, _candleTimeSpan.Minutes,0));
