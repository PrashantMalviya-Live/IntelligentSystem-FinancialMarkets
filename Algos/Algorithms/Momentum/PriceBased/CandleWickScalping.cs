...
namespace Algorithms.Algorithms
{
    public class CandleWickScalping : IZMQ, IObserver<Tick>////, IKConsumer
    {
        ...
        private const decimal QUALIFICATION_ZONE_THRESHOLD = 50;
        private const decimal TRADING_ZONE_THRESHOLD = 25; // This should be % of qualification zone. 50% is good.
        private const decimal RR_BREAKDOWN = 1; // Risk Reward
        private const decimal RR_BREAKOUT = 1; // Risk Reward
        private const decimal MINIMUM_TARGET = 2000;
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    if (_activeFuture.InstrumentToken == token)
                    {
                        ...
                        TakeTrade(tick.LastPrice, currentTime);
        ...
        public void TakeTrade(decimal currentPrice, DateTime currentTime)
        {
            if (_resistancePriceRange.Upper != 0
                       && currentPrice < _resistancePriceRange.Upper
                       && _resistancePriceRange.Upper - currentPrice < TRADING_ZONE_THRESHOLD)
            {
                //breakdown
                TakeTrade(buy: false, currentTime, Math.Max(RR_BREAKDOWN * (_resistancePriceRange.Upper - currentPrice), MINIMUM_TARGET), _resistancePriceRange.Upper);
            }
            //else if (_resistancePriceRange.Upper != 0
            //    && currentPrice > _resistancePriceRange.Upper
            //    && currentPrice - _resistancePriceRange.Upper < TRADING_ZONE_THRESHOLD)
            //{
            //    //breakout
            //    TakeTrade(buy: true, currentTime, Math.Max(RR_BREAKOUT * (currentPrice - _resistancePriceRange.Upper), MINIMUM_TARGET), _resistancePriceRange.Upper);
            //}

            if (_supportPriceRange.Lower != 0
                      && currentPrice > _supportPriceRange.Lower
                      && currentPrice - _supportPriceRange.Lower < TRADING_ZONE_THRESHOLD)
            {
                TakeTrade(buy: true, currentTime, Math.Max(RR_BREAKDOWN * (currentPrice - _supportPriceRange.Lower), MINIMUM_TARGET), _supportPriceRange.Lower);
            }
            //else if (_supportPriceRange.Lower != 0
            //    && currentPrice < _supportPriceRange.Lower
            //    && _supportPriceRange.Lower - currentPrice < TRADING_ZONE_THRESHOLD)
            //{
            //    //breakout
            //    TakeTrade(buy: false, currentTime, Math.Max(RR_BREAKOUT * (_supportPriceRange.Lower - currentPrice), MINIMUM_TARGET), _supportPriceRange.Lower);
            //}
        }
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (e.InstrumentToken == _baseInstrumentToken)
            {
                ...
                else
                {
                    ...
                    {
                        //breakdown
                        TakeTrade(buy: false, e.CloseTime, Math.Max( RR_BREAKDOWN * (_resistancePriceRange.Upper - e.ClosePrice), MINIMUM_TARGET), _resistancePriceRange.Upper);
                    }
                    else if (_resistancePriceRange.Upper != 0 
                        && e.ClosePrice > _resistancePriceRange.Upper
                    ...
                    {
                        //breakout
                        TakeTrade(buy: true, e.CloseTime, Math.Max( RR_BREAKOUT * (e.ClosePrice - _resistancePriceRange.Upper), MINIMUM_TARGET), _resistancePriceRange.Upper);
                    }
                }
                ...
                else
                {
                    ...
                    {
                        TakeTrade(buy: true, e.CloseTime, Math.Max( RR_BREAKDOWN * (e.ClosePrice - _supportPriceRange.Lower), MINIMUM_TARGET), _supportPriceRange.Lower);
                    }
                    else if (_supportPriceRange.Lower != 0 
                        && e.ClosePrice < _supportPriceRange.Lower
                    ...
                    {
                        //breakout
                        TakeTrade(buy: false, e.CloseTime, Math.Max( RR_BREAKOUT * (_supportPriceRange.Lower - e.ClosePrice), MINIMUM_TARGET), _supportPriceRange.Lower);
                    }
        ...
        private void TakeTrade(bool buy, DateTime currentTime, decimal targetLevel, decimal stopLevel)
        {
            ...
            if (qty > 0 && currentTime.TimeOfDay <  new TimeSpan(14,30,0))
            {
