...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private readonly int STOP_LOSS_POINTS = 1120;//50;//30; //40 points max loss on an average 
        private readonly int STOP_LOSS_POINTS_GAMMA = 1120;//50;//30;
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (_callorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _callorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice > orderTrio.BaseInstrumentStopLoss && htred == 0)
                                {
                                    Instrument option = orderTrio.Option;
                        ...
                        if (_putorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _putorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _bCandle.ClosePrice < orderTrio.BaseInstrumentStopLoss && htred == 1)
                                {
                                    Instrument option = orderTrio.Option;
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && (htvalue >= _baseInstrumentPrice - 5))
                    {
                        ...
                        tradeQty = Convert.ToInt32(_firstTargetQty * _tradeQty * Convert.ToInt32(_lotSize));
                    ...
                    else if (_putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].BaseInstrumentStopLoss < 300))
                    {
                        ...
                        tradeQty = 0;
                        if (previousOrderTrio.Option.LastPrice > previousOrderTrio.Order.AveragePrice + NEXTPRICEDELTA)
                        {
                            ...
                            {
                                tradeQty = Convert.ToInt32(_secondTargetQty * _tradeQty * _lotSize);
                                tradeQty -= tradeQty % 25;
                            }
                        }
                    }
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 &&  (htvalue <= _baseInstrumentPrice + 5 ))
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive = true;
                        _thirdCETargetActive = true;
                        _fourthCETargetActive= true;
                    ...
                    decimal htvalue = _stockTokenHalfTrend[_baseInstrumentToken].GetCurrentValue<decimal>(); // 0 is up trend
                        ...
                        if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell")
                        {
                            ...
                            if (orderTrio.Order.Quantity >= qty)
                            {
                                ...
                                //if ((htred == 0 && !((orderTrios.Count == 1 && htvalue >= _baseInstrumentPrice - 5) || (orderTrios.Count > 1 && (_baseInstrumentPrice - orderTrios[0].BaseInstrumentStopLoss < 300)))) || (
                                //        (htred == 1 && !((orderTrios.Count == 1 && htvalue <= _baseInstrumentPrice + 5) || (orderTrios.Count > 1 && (orderTrios[0].BaseInstrumentStopLoss - _baseInstrumentPrice < 300))))
                                //        ))
                                //{
                                ...
                                //else
                                //{
                                //    orderTrio.Order.AveragePrice -= FIRST_TARGET;
                                //    continue;
                                //}
