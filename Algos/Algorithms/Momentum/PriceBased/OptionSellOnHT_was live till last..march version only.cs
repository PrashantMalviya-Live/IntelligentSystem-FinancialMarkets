...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
                    ...
                    lock (tradeLock)
                    {
                        ...
                        if (SubscriptionTokens.Contains(token))
                        {
                            ...
                            if (OptionUniverse != null)
                            {
                               ...
                               && BookStopLoss(_putorderTrios, htrend == 1, ref _peCurrentProfit)
                               )
                                    {
                               ...
                               && BookStopLoss(_callorderTrios, htrend == 0, ref _ceCurrentProfit)
                               )
                                    {
                                ...
                                else
                                {
                                    ...
                                    ref _thirdPETargetActive, ref _peCurrentProfit,ref _firsttargetmovedPE, ref _secondtargetmovedPE);
                                    _callProfitBookedQty += BookPartialProfit(_callorderTrios, tick, htrend == 0,
                                        ...
                                        ref _thirdCETargetActive, ref _ceCurrentProfit, ref _firsttargetmovedCE, ref _secondtargetmovedCE);
                                }
                            }
#if BACKTEST
                        }
                        ...
                        TrailStopLoss(_putorderTrios, ref _peCurrentProfit, ref _putTriggered);
                        TrailStopLoss(_callorderTrios, ref _ceCurrentProfit, ref _callTrigerred);


                        //TrailOptions(_callorderTrios, currentTime, _baseInstrumentPrice, InstrumentType.CE, ref _ceCurrentProfit);
                        //TrailOptions(_putorderTrios, currentTime, _baseInstrumentPrice, InstrumentType.PE, ref _peCurrentProfit);
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (suspensionWindow && previoushtred == 1 && htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (OrderTrio ot in _putorderTrios)
                                {
                                    //ot.BaseInstrumentStopLoss = _downtrendlowswingindexvalue;// - SL_BUFFER;
                                 
                                    //SL SHIFTED TO LOW OF PREVIOUS CANDLE
                                    ot.BaseInstrumentStopLoss = _peSL != 0 ? _peSL : _downtrendlowswingindexvalue;
                                }
                                ...
                                //int qty = orderTrio.Order.Quanutity;
                                ...
                                //        //buyback existing option, and buy at new swing low
                                ...
                                //    orderTrio.Option = _atmOption;
                                //    orderTrio.BaseInstrumentStopLoss = _downtrendlowswingindexvalue - SL_BUFFER;
                        ...
                        else if (suspensionWindow && previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (OrderTrio ot in _callorderTrios)
                                {
                                    //ot.BaseInstrumentStopLoss = _uptrendhighswingindexvalue;// + SL_BUFFER;

                                    ot.BaseInstrumentStopLoss = _ceSL != 0 ? _ceSL : _uptrendhighswingindexvalue;
                                }
                                //    orderTrio.BaseInstrumentStopLoss = _uptrendhighswingindexvalue + SL_BUFFER;
                        ...
                        else
                        {
                            ...
                            //{
                            //    if (_orderTrios[0].Order.TransactionType.ToLower() == "buy")
                            //    {
                        ...
                        if (_callorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _callorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy" && _bCandle.ClosePrice > orderTrio.BaseInstrumentStopLoss && htred == 0)
                                {
                                    Instrument option = orderTrio.Option;

                                    Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, "pe", option.LastPrice,
                                       option.KToken, !_optionBuy, orderTrio.Order.Quantity, algoIndex, _bCandle.CloseTime,
                                       Tag: "",
                                       product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                       broker: Constants.KOTAK,
                                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                                    ...
                                    _pnl += order.Quantity * order.AveragePrice * (!_optionBuy ? -1 : 1);
                        ...
                        if (_putorderTrios.Count != 0)
                        {
                            for (int i = 0; i < _putorderTrios.Count; i++)
                            {
                                ...
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy" && _bCandle.ClosePrice < orderTrio.BaseInstrumentStopLoss && htred == 1)
                                {
                                    Instrument option = orderTrio.Option;
                ...
                )
            {
                ...
                if (htred == 0 && (!_peSLHit || (htvalue > _previousUpswingValue && _peSLHit)))//&& _putTriggered)
                {
                    ...
                    if (_putTriggered && _putorderTrios.Count == 0 && (htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5? 31: 11) ))
                    {
                        ...
                        //stopLoss = _downtrendlowswingindexvalue - SL_BUFFER;
                    ...
                    else if (_putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].BaseInstrumentStopLoss < 300))
                    {
                        ...
                        OrderTrio previousOrderTrio = _putorderTrios.Last<OrderTrio>();
                        tradeQty = 0;
                        if (previousOrderTrio.Option.LastPrice > previousOrderTrio.Order.AveragePrice + 20)
                        {
                            tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize),
                                _putProfitBookedQty, _thirdPETargetActive) - _putorderTrios.Sum(x => x.Order.Quantity);
                    ...
                    if (tradeQty > 0 && _baseInstrumentPrice > stopLoss)
                    {
                        ...
                        //Instrument _atmOption = GetOptions(currentTime, _baseInstrumentPrice, InstrumentType.PE, _putorderTrios, stopLoss);

                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, "ce", _atmOption.LastPrice,
                           _atmOption.KToken, _optionBuy, qty, algoIndex, currentTime,
                           Tag: "",
                           product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                        ...
                        _pnl += orderTrio.Order.Quantity * orderTrio.Order.AveragePrice * (_optionBuy ? -1 : 1);
                ...
                if (htred == 1 && (!_ceSLHit || (_ceSLHit &&  htvalue < _previousDownswingValue)))// && _callTrigerred)
                {
                    ...
                    if (_callorderTrios.Count == 0 && _callTrigerred && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes == 5 ? 31 : 11)))
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, 
                            _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive);

                        //NO POSITION increase after initial trade. YES
                        if (tradeQty > 0)
                        {
                            _callTrigerred = false;
                        }
                    ...
                    else if (_callorderTrios.Count != 0 && (_callorderTrios[0].BaseInstrumentStopLoss - _baseInstrumentPrice < 300))
                    {
                        ...
                        OrderTrio previousOrderTrio = _callorderTrios.Last<OrderTrio>();
                        tradeQty = 0;

                        if (previousOrderTrio.Option.LastPrice > previousOrderTrio.Order.AveragePrice + 20)
                        {
                            tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss,
                                _tradeQty * Convert.ToInt32(_lotSize), _callProfitBookedQty,
                                _thirdCETargetActive) - _callorderTrios.Sum(x => x.Order.Quantity);
                        }
                    ...
                    if (tradeQty > 0 && _baseInstrumentPrice < stopLoss)
                    {
                        ...
                        //Instrument _atmOption = GetOptions(currentTime, _baseInstrumentPrice, InstrumentType.CE, _callorderTrios, stopLoss);
        ...
        private void MoveOption(List<OrderTrio> orderTrios, Instrument newOption)
        {
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice,
                   option.KToken, !_optionBuy, orderTrio.Order.Quantity, algoIndex, _bCandle.CloseTime, Tag: "",
            ...
            orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, newOption.TradingSymbol, newOption.InstrumentType, newOption.LastPrice,
               newOption.KToken, _optionBuy, qty, algoIndex, _bCandle.CloseTime,
               Tag: "",
               product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
               broker: Constants.KOTAK,
               httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        private decimal GetPEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
            {
                strike = Math.Floor((Math.Min(stoploss, bPrice) - 25 * (_optionBuy ? -1 : 1)) / 50) * 50;
            }
            else
            {
                strike = !_optionBuy? Math.Floor((Math.Min(stoploss, bPrice) - 100) / 100) * 100 : Math.Floor((bPrice - 100) / 100) * 100;
            }
        ...
        private decimal GetCEStrike(decimal bPrice, decimal stoploss)
        {
            ...
            if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
            {
                strike = Math.Ceiling((Math.Max(stoploss, bPrice) + 25 * (_optionBuy ? -1 : 1)) / 50) * 50;
            }
            else
            {
                //strike = Math.Ceiling((Math.Max(stoploss, bPrice) + 100 * (_optionBuy ? -1: 1)) / 100) * 100;
                strike = !_optionBuy ? Math.Ceiling((Math.Max(stoploss, bPrice) + 100) / 100) * 100 : Math.Ceiling((bPrice + 100) / 100) * 100;
    ...
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit, ref bool firstTargetMoved, ref bool secondTargetMoved)
        {
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                {
                    if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy")
                    //&&
                       ...
                       // else
                        ...
                        if (qty > 0)
                        {
                            ...
                            if (orderTrio.Order.Quantity >= qty)
                            {
                                quantityTraded = qty;
                                Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                                   option.KToken, !_optionBuy, qty, algoIndex, _bCandle.CloseTime, Tag: "",
                                   product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                   broker: Constants.KOTAK,
                                   httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        //                        i--;
        //                    }
        //                    else if (orderTrio.Order.TransactionType.ToLower() == "buy" && (orderTrio.TPFlag))
        //                    {
        //                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
        ...
        //                        i--;
        //                    }
        //                    else if (orderTrio.Order.TransactionType.ToLower() == "buy" && ((lastPrice > orderTrio.StopLoss || closeAll) || (pCandle != null && (lastPrice > pCandle.HighPrice) && orderTrio.TPFlag)))
        //                    {
        //                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
        //                       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo1", product: Constants.KPRODUCT_NRML,
        //                       broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
        ...
        //                        i--;
        //                    }
        //                    else if (orderTrio.Order.TransactionType.ToLower() == "buy" && (lastPrice > _indexEMAValue.GetValue<decimal>() || closeAll))
        //                    {
        //                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
