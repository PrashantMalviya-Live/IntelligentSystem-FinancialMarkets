...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        Dictionary<uint, HalfTrend> _stockTokenHalfTrendSL;
        decimal _downtrendlowswingindexvalue = 0;
        decimal _uptrendhighswingindexvalue = 0;
        decimal _trailingHighSL = 0;
        decimal _trailingLowSL = 0;
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            if(_candleTimeSpan.TotalMinutes == 5)
            {
                ...
                STOP_LOSS_POINTS_GAMMA = 500;
            }
            _candleTimeSpanSL = new TimeSpan(0, 1, 0);
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            _stockTokenHalfTrendSL = new Dictionary<uint, HalfTrend>();
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    if (_stockTokenHalfTrendSL.ContainsKey(token) && _stockTokenHalfTrend.ContainsKey(token))
                    {
                        decimal previoushtredSL = _stockTokenHalfTrendSL[token].GetTrend(); // 0 is up trend
                        _stockTokenHalfTrendSL[token].Process(e);
                        decimal htredSL = _stockTokenHalfTrendSL[token].GetTrend(); // 0 is up trend
                        
                        decimal htred = _stockTokenHalfTrend[token].GetTrend(); // 0 is up trend
                        decimal htvalue = _stockTokenHalfTrend[token].GetValue(); // 0 is up trend

                        if (TimeCandles[token].Count == 1)
                        {
                            _trailingLowSL = e.LowPrice;
                            _trailingHighSL = e.HighPrice;
                        }

                        if (_callorderTrios.Count != 0 && previoushtredSL == 0 && htredSL == 1 && _trailingHighSL != 0 && _trailingHighSL > htvalue)
                        {
                            for (int i = 0; i < _callorderTrios.Count; i++)
                            {
                                var orderTrio = _callorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && orderTrio.StopLoss < _trailingHighSL + 100)
                                {
                                    orderTrio.StopLoss = _trailingHighSL;
                                }
                            }
                        }
                        if (_putorderTrios.Count != 0 && previoushtredSL == 1 && htredSL == 0 && _trailingLowSL != 0 && _trailingLowSL < htvalue)
                        {
                            for (int i = 0; i < _putorderTrios.Count; i++)
                            {
                                var orderTrio = _putorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && orderTrio.StopLoss + 100 < _trailingLowSL)
                                {
                                    orderTrio.StopLoss = _trailingLowSL;
                                }
                            }
                        }

                        if (htredSL == 0 && e.HighPrice > _trailingHighSL)// && htred == 1)
                        {
                            _trailingHighSL = e.HighPrice;
                            _trailingLowSL = 0;
                        }
                        else if (htredSL == 1 && (_trailingLowSL > e.LowPrice  || _trailingLowSL == 0))// && htred == 0)
                        {
                            _trailingLowSL = e.LowPrice;
                            _trailingHighSL = 0;
                        }
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(token) && (e.CloseTime.Minute - MARKET_START_TIME.Minutes) % _candleTimeSpan.Minutes == 0)
                    { 
                        _bCandle = GetLastCandle(_candleTimeSpan.Minutes / _candleTimeSpanSL.Minutes, e.CloseTime);
                        if(_bCandle == null)
                        {
                            return;
                        }
                        ...
                        //_uptrendhighswingindexvalue = _uptrendhighswingindexvalue != 0 ? _uptrendhighswingindexvalue : _bCandle.HighPrice;
                        //_downtrendlowswingindexvalue = _downtrendlowswingindexvalue != 0 ? _downtrendlowswingindexvalue : _bCandle.LowPrice;

                        if (TimeCandles[_bCandle.InstrumentToken].Count == _candleTimeSpan.Minutes)
                        {
                       ...
                       // }
                        if(_callorderTrios.Count != 0)
                        {
                            ...
                            {
                                var orderTrio = _callorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _baseInstrumentPrice > orderTrio.StopLoss + 10 && htred == 0)
                                {
                                    Instrument option = orderTrio.Option;
                        ...
                        if (_putorderTrios.Count != 0)
                        {
                            ...
                            {
                                var orderTrio = _putorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "sell" && _baseInstrumentPrice < orderTrio.StopLoss - 10 && htred == 1)
                                {
                                    Instrument option = orderTrio.Option;
                    ...
                    if (_intraday || _expiryDate.Value.Date == e.CloseTime.Date)
                    {
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5? 30:15) && _putTriggered)
                    {
                    ...
                    decimal optionStrike = _putorderTrios.Count == 0 ? GetPEStrike(_baseInstrumentPrice, stopLoss) : _putorderTrios[0].Option.Strike;
                    ...
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][optionStrike]; //Change -  Take position 50 points farther to avoid gamma loss
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice +  (_candleTimeSpan.Minutes == 5? 30:15)) && _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
                    ...
                    decimal optionStrike = _callorderTrios.Count == 0? GetCEStrike(_baseInstrumentPrice, stopLoss) : _callorderTrios[0].Option.Strike;

                    OrderTrio orderTrio = new OrderTrio();
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][optionStrike]; //Change -  Take position 50 points farther to avoid gamma loss
    ...
    ref bool thirdTradeActive)
                ...
                if (orderTrio.Option.InstrumentToken == tick.InstrumentToken)
                {
                    ...
                    //))
                    {
                       ...
                       // else
                        ...
                        else if ((orderTrio.Order.AveragePrice > tick.LastPrice + THIRD_TARGET - 585)
                        && totalTradedQty >= (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                        && thirdTradeActive)
                        {
        ...
        {
            LoadHistoricalCandlesForHT(tokenList, candlesCount, lastCandleEndTime, _candleTimeSpan, _stockTokenHalfTrend);
            LoadHistoricalCandlesForHT(tokenList, candlesCount, lastCandleEndTime, _candleTimeSpanSL, _stockTokenHalfTrendSL);
            //LoadHistoricalCandlesForADX(tokenList, candlesCount, lastCandleEndTime, new TimeSpan(0, 15, 0));
        }
        
        private void LoadHistoricalCandlesForHT(List<uint> tokenList, int candlesCount, DateTime lastCandleEndTime, TimeSpan timeSpan, Dictionary<uint, HalfTrend> tokenHalfTrend)
        {
            ...
            lock (tokenHalfTrend)
            {
                ...
                foreach (uint token in tokenList)
                {
                    ...
                    foreach (var price in historicals)
                    {
                        ...
                        if (tokenHalfTrend.ContainsKey(tc.InstrumentToken))
                        {
                            tokenHalfTrend[tc.InstrumentToken].Process(tc);
                        }
                        else
                        {
                            ht = new HalfTrend(2, 2);
                            ht.Process(tc);
                            tokenHalfTrend.TryAdd(tc.InstrumentToken, ht);
                        }
                    }
                }
            }
        ...
        private void MonitorCandles(Tick tick, DateTime currentTime)
        {
            try
            {
                ...
                {
                    candleManger.StreamingTimeFrameCandle(tick, token, _candleTimeSpanSL, true); // TODO: USING LOCAL VERSION RIGHT NOW
                }
                else
                {
                    DateTime lastCandleEndTime;
                    ...
                    if (candleStartTime.HasValue)
                    {
                        ...
                        candleManger.StreamingTimeFrameCandle(tick, token, _candleTimeSpanSL, true, candleStartTime); // TODO: USING LOCAL VERSION
