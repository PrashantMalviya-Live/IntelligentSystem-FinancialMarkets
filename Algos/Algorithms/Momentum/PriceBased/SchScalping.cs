...
{
    public class SchScalping : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(SchScalping source);
        [field: NonSerialized]
        ...
        public SchScalping(TimeSpan candleTimeSpan, uint baseInstrumentToken,
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (e.InstrumentToken == _baseInstrumentToken)
            {
                ...
                if (_indexSch.IsFormed && _indexSch.GetValue<decimal>(0) > SCH_UPPER_THRESHOLD)
                {
                    int qty;
                    ...
                    if (qty > 0)
                    {
                        ...
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice, //e.ClosePrice,
                           _activeFuture.KToken, true, qty, algoIndex, e.CloseTime, Tag: "Algo3",
                        ...
                        OnCriticalEvents(e.CloseTime.ToShortTimeString(), String.Format("{0} @ {1}",  "Bought ", Math.Round(orderTrio.Order.AveragePrice, 2)));
#endif
                    }
                }
                else if (_indexSch.IsFormed && _indexSch.GetValue<decimal>(0) < SCH_LOWER_THRESHOLD)
                    {
                    int qty;
                    if (_orderTrios.Count > 0)
                    {
                    ...
                    if (qty > 0)
                    {
                        ...
                        OrderTrio orderTrio = new OrderTrio();
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice, //e.ClosePrice,
                           _activeFuture.KToken, false, qty, algoIndex, e.CloseTime, Tag: "Algo3",
                           broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
...
#if !BACKTEST
                        OnTradeEntry(orderTrio.Order);
                        OnCriticalEvents(e.CloseTime.ToShortTimeString(), String.Format("{0} @ {1}","Sold ", Math.Round(orderTrio.Order.AveragePrice, 2)));
#endif
                    }
                }
        ...
        //{
        //    DataLogic dl = new DataLogic();
        //    DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.SchScalping, Convert.ToDateTime("2021-11-30"), Convert.ToDateTime("2021-12-30"));
