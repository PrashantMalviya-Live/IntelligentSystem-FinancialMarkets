                ...
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType == "buy")
                    {
                        ...
                        //OnCriticalEvents(currentTime.ToShortTimeString(), String.Format("{0} @ {1}", "Closed", Math.Round(tradePrice, 2)));
                            ...
                            if ((_activeFuture.LastPrice < orderTrio.StopLoss) && sl)
                            {
                                decimal target = orderTrio.StopLoss - (orderTrio.TargetProfit - orderTrio.StopLoss) / 1.768m;
                                decimal stoploss = orderTrio.StopLoss + (orderTrio.TargetProfit - orderTrio.StopLoss) / 1.768m;
                                //_brokeBands.Add(new Band()
                                //{
                                //    lastBreakoutTime = currentTime,
                                //    sl = orderTrio.StopLoss + (orderTrio.TargetProfit - orderTrio.StopLoss)/1.768m,
                                //    tp = orderTrio.StopLoss - (orderTrio.TargetProfit - orderTrio.StopLoss) / 1.768m,
                                //    tradePrice = orderTrio.StopLoss,
                                //    up = false
                                //});

                                TakeTrade(false, currentTime, target, stoploss);
                            }
                    ...
                    else if (orderTrio.Order.TransactionType == "sell")
                    {
                        ...
                        //OnCriticalEvents(currentTime.ToShortTimeString(), String.Format("{0} @ {1}", "Closed", Math.Round(tradePrice, 2)));
                            ...
                            if ((_activeFuture.LastPrice > orderTrio.StopLoss) && sl)
                            {
                                ////_brokeBands.Add(new Band() { lastBreakoutTime = currentTime, sl = sl, tp = target, tradePrice = orderTrio.StopLoss, up = true });
                                ...
                                TakeTrade(true, currentTime, target, stoploss);
                            }
                        }
                    }
