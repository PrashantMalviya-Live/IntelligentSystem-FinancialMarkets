...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private bool _firstPEprofitBooked = false;
        private bool _firstCEprofitBooked = false;
       ...
       // private const decimal FIRST_TARGET = 50;
        ...
        private decimal _tp1, _tp2, _tp3, _tp4;
        private decimal _stopLoss;
        private decimal _sl1, _sl2, _sl3, _sl4;
        ...
        public OptionSellonHT(TimeSpan candleTimeSpan, uint baseInstrumentToken,
            ...
            bool intraday, decimal pnl,decimal firstTargetProfit, decimal secondTargetProfit, decimal thirdTargetProfit, decimal fourthTargetProfit,
            decimal firstStopLoss, decimal secondStopLoss, decimal thirdStopLoss, decimal fourthStopLoss,
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            _tp1 = firstTargetProfit;
            _tp2 = secondTargetProfit;
            _tp3 = thirdTargetProfit;
            _tp4 = fourthTargetProfit;
        ...
        /// <summary>
        /// Change: Any candle that closes on the oppsoite direction, close everything...till candle comes back
        /// </summary>
                           ...
                           && BookStopLoss(_callorderTrios, htrend == 1)
                            ...
                            else
                            {
                                ...
                                /*ref _preFirstPETargetActive,*/ ref _firstPETargetActive, ref _secondPETargetActive, ref _thirdPETargetActive);
                                    _firstPEprofitBooked = _putProfitBookedQty > 0;
                                _callProfitBookedQty += BookPartialProfit(_callorderTrios, tick, htrend == 0,
                                    ...
                                    _firstCEprofitBooked = _callProfitBookedQty > 0;
                                }
                        }
#if BACKTEST
                        }
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (/*suspensionWindow && previoushtred == 1 &&*/ htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                int newqty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _firstPEprofitBooked);
                                ...
                                if (option.Strike != atmStrike && newqty > 0)
                                {
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _firstPEprofitBooked);
                                ...
                                else
                                {
                                    ...
                                    int shouldbeQty = GetTradedQuantity(_baseInstrumentPrice, _downtrendlowswingindexvalue, 
                                        _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _firstPEprofitBooked);
                        ...
                        else if (/*suspensionWindow && previoushtred == 0 && */htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                int qty;// = orderTrio.Order.Quantity;
                                int newQty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _firstCEprofitBooked);
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    DataLogic dl = new DataLogic();
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _firstCEprofitBooked);
                                ...
                                else
                                {
                                    ...
                                    int shouldbeQty = GetTradedQuantity(_baseInstrumentPrice, _uptrendhighswingindexvalue, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _firstCEprofitBooked);
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5? 30:5) && _putTriggered)
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdPETargetActive, _firstPEprofitBooked);
                    ...
                    else if (_putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].StopLoss < 300))
                    {
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), 
                            _putProfitBookedQty, _thirdPETargetActive, _firstPEprofitBooked) - _putorderTrios.Sum(x => x.Order.Quantity);
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice +  (_candleTimeSpan.Minutes == 5? 30:5)) && _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
                        ...
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), 0, _thirdCETargetActive, _firstCEprofitBooked);
                    }
                    else if (_callorderTrios.Count != 0 && (_callorderTrios[0].StopLoss - _baseInstrumentPrice < 300))
                    {
                        //TODO: COMMENTED FOR NOT INCREASING QTY LATER
                        stopLoss = _callorderTrios[0].StopLoss;
                        tradeQty = GetTradedQuantity(_baseInstrumentPrice, stopLoss, _tradeQty * Convert.ToInt32(_lotSize), _callProfitBookedQty, _thirdCETargetActive, _firstCEprofitBooked)
                            - _callorderTrios.Sum(x => x.Order.Quantity);
        ...
        private bool BookStopLoss(List<OrderTrio> orderTrios, bool supportingTrend)
        {
            ...
            decimal maxLoss = _tradeQty * Convert.ToInt32(_lotSize) * _sl1;// (supportingTrend ? STOP_LOSS_POINTS_GAMMA : STOP_LOSS_POINTS);
    ...
    ref bool thirdTradeActive)
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                if (orderTrio.Option.InstrumentToken == tick.InstrumentToken)
                {
                    ...
                    //))
                    {
                       ...
                       // }
                       // else
                        if ((orderTrio.Order.AveragePrice > tick.LastPrice + _tp1)//FIRST_TARGET)
                       // && totalTradedQty > (_tradeQty * Convert.ToInt32(_lotSize) * 2 / 4)
                        && firstTradeActive)
                        {
                        ...
                        else if ((orderTrio.Order.AveragePrice > tick.LastPrice + _tp2) //SECOND_TARGET)
                       // && totalTradedQty > (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                        && secondTradeActive)
                        {
                        ...
                        else if ((orderTrio.Order.AveragePrice > tick.LastPrice + _tp3)
                       // && totalTradedQty >= (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                        && thirdTradeActive)
                        {
                       ...
                       // else if ((orderTrio.Order.AveragePrice > tick.LastPrice + _tp4)
                       //// && totalTradedQty >= (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                       //&& thirdTradeActive)
                       // {
                       //     //thirdTradeActive = false;
                       //     qty = Convert.ToInt32(_ * totalQty);
                       //     thirdTradeActive = false;
                       // }
                        // Trend change , book 1/4 provided it is in profit
                        else if ((orderTrio.Order.AveragePrice > tick.LastPrice + _tp1 && trendChanged)
                        && totalTradedQty == (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4))
                        {
                            qty = Convert.ToInt32(_thirdTargetQty * totalQty);
                        }
        ...
        //}
        private int GetTradedQuantity(decimal triggerLevel, decimal SL, int totalQuantity, int bookedQty, bool thirdTargetActive, bool firstProfitBooked)
        {
            int tradedQuantity = 0;
            ...
            if (_candleTimeSpan.Minutes == 3)
            {
                if (Math.Abs(triggerLevel - SL) < 15)
                {
                    tradedQuantity = totalQuantity;
                    //tradedQuantity = firstProfitBooked ? totalQuantity : totalQuantity / 2;
                    tradedQuantity -= tradedQuantity % 25;
                }
                //if (Math.Abs(triggerLevel - SL) < 50)
                //{
            ...
            else if (_candleTimeSpan.Minutes == 5)
            {
                if (Math.Abs(triggerLevel - SL) < 30)
                {
                    //tradedQuantity = firstProfitBooked ? totalQuantity : totalQuantity / 2;
