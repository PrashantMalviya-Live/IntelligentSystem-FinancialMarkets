...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        //        decimal currentProfit = order.AveragePrice - AllOptions[orderTrio.Option.InstrumentToken].LastPrice;

        //        for (int i = 1; i < 100; i++)
        //        {
        //            if (currentProfit > SL_TRAIL_LIMIT * i && currentProfit < SL_TRAIL_LIMIT * (i + 1))
        //            {
        //                orderTrio.StopLoss -= SL_TRAIL_LIMIT;
        //            }
        //        }
        //        //_pnl += order.Quantity * AllOptions[orderTrio.Option.InstrumentToken].LastPrice * -1;
        //    }
        ...
        //    if (_pnl/ _tradeQty * Convert.ToInt32(_lotSize) > FIRST_SL_TRAIL_LIMIT)
        //    {
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - (_candleTimeSpan.Minutes == 5? 31: 11) && _putTriggered)
                    {
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + (_candleTimeSpan.Minutes == 5 ? 31 : 11)) && _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
