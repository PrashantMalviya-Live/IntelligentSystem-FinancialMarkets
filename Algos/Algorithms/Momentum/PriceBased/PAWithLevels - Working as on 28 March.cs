        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (e.InstrumentToken == _baseInstrumentToken)
            {
                ...
                if (_firstCandleOutsideRange.GetValueOrDefault(false))
                {
                        ...
                        && _orderTriosFromEMATrades.Count == 0)
                    {
                        ...
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice, //e.ClosePrice,
                           _activeFuture.KToken, buy, qty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, e.CloseTime, Tag: "Algo2",
                ...
                if (e.CloseTime.TimeOfDay.Minutes % 15 == 0)
                {
                    ...
                    if (ValidateRiskReward(e, out targetLevel, out stopLevel, out s2r2Level) && ValidateCandleSize(e, s2r2Level))
                    {
                        //PostOrderInKotak(_activeFuture, e.CloseTime, _tradeQty, e.ClosePrice > e.OpenPrice);

                        if (_orderTrios.Count > 0
                            && ((_orderTrios[0].Order.TransactionType.ToLower() == "buy" && e.ClosePrice > e.OpenPrice)
                            || (_orderTrios[0].Order.TransactionType.ToLower() == "sell" && e.ClosePrice < e.OpenPrice)))
                        {
                        }
                        else
                        {
                ...
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    if ((orderTrio.Order.TransactionType.ToLower() == "buy") && ((lastPrice < orderTrio.StopLoss || closeAll) || (pCandle != null && (lastPrice < pCandle.LowPrice) && orderTrio.TPFlag)))
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                            _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo1",
                            broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                    ...
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo1",
                       broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
        ...
        private void CheckSLForEMABasedOrders(DateTime currentTime, decimal lastPrice, Candle pCandle, bool closeAll = false)
        {
            if (_orderTriosFromEMATrades.Count > 0)
            {
                for (int i = 0; i < _orderTriosFromEMATrades.Count; i++)
                {
                    ...
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                            _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag:"Algo2",
                            broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                    ...
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo2",
                       broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
