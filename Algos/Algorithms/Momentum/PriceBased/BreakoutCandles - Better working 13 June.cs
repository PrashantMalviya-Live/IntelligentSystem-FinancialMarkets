...
namespace Algorithms.Algorithms
{
    public class BreakoutCandles : IZMQ, IObserver<Tick>//, IKConsumer
    {
            ...
            if (e.InstrumentToken == _referenceIndex.InstrumentToken)
            {
                ...
                if(_orderTrios.Count > 0)
                {
                    for(int i = 0;i < _orderTrios.Count; i++)
                    {
                        ...
                        if(orderTrio.Order.TransactionType.ToLower() == "buy" && e.ClosePrice < orderTrio.StopLoss)
                        {
                            Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, false, orderTrio.Order.Quantity, algoIndex, e.CloseTime, Tag: Convert.ToString(orderTrio.Order.AveragePrice),
        ...
        private void TradeTP(DateTime currentTime)
        {
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType.ToLower() == "buy" && _referenceIndex.LastPrice > orderTrio.TargetProfit)
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
        ...
        private void CheckSL(DateTime currentTime)
        {
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, false, orderTrio.Order.Quantity, algoIndex, currentTime, Tag: Convert.ToString(Guid.NewGuid()),
                       product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
