       ...
       // decimal _previoushtred;
        ...
        private void CheckTargetProfit()
        {
            if (_orderTrios.Count > 0 
                && _orderTrios[0].TargetProfit != 0 
                && (
                (_orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell" && _activeFuture.LastPrice < _orderTrios[0].TargetProfit)
                || (_orderTrios[0].Order.TransactionType.ToLower().Trim() == "buy" && _activeFuture.LastPrice > _orderTrios[0].TargetProfit)
                ))
            {
                _orderTrios[0].TargetProfit = 0;
                bool buyOrder = _orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell";
                //Book half quantity at 1:1 R&R
                int qty = _orderTrios[0].Order.Quantity/ 2;
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                   _activeFuture.KToken, buyOrder, qty, algoIndex, _bCandle.CloseTime,
                   Tag: String.Format("Half qty {0} at 1:1 R&R at {1}", qty, _activeFuture.LastPrice),
                   product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                ...
                _orderTrios[0].Order.Quantity = _orderTrios[0].Order.Quantity - qty;
            }
        }
        private void TakeTrade(DateTime currentTime)
        {
            ...
            if (htred == 0 && _orderTrios.Count == 0
                   && htvalue >= _baseInstrumentPrice - 10
                   && _putTriggered)
            {
                OrderTrio orderTrio = new OrderTrio();
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            ...
            if (_baseCandleLoaded)//&& _referenceCandleLoaded)
            {
                ...
                if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                {
                    ...
                    //if ((_bCandle.CloseTime.TimeOfDay.Minutes - MARKET_START_TIME.Minutes) % 30 == 0)
                    //{
                    //    Candle c = GetLastCandle(_bCandle.InstrumentToken, 6);
                    //    if (c != null)
                    //    {
                    //        _stockTokenHalfTrendLong[_bCandle.InstrumentToken].Process(c);
                    //    }
                    //}
                    ...
                    //decimal htredLong = _stockTokenHalfTrendLong[_bCandle.InstrumentToken].GetTrend(); // 0 is up trend
                    ...
                    //if (_bCandle.ClosePrice > htvalue && _bCandle.OpenPrice < htvalue)// && dplus > dminus)


                   // int qty = _orderTrios.Count == 0 ? suspensionWindow ? 0 : _activeFuture.LastPrice - _swingLowPrice < SL_THRESHOLD ? _tradeQty : 0 : 0;

                    if (_orderTrios.Count == 0 && !suspensionWindow && _activeFuture.LastPrice - _swingLowPrice < SL_THRESHOLD)
                    {
                        _slTrace = false;
                        switch (htred)
                        {
                    ...
                    {
                        if (previoushtred == 1 && htred == 0)// && htredLong == 1)
                        {
                            //int qty = _orderTrios.Count == 0 ? suspensionWindow ? 0 : _tradeQty : _orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell" ? suspensionWindow ? _tradeQty : 2 * _tradeQty : 0;
                            _slTrace = false;
                            LastSwingHighlow(e.CloseTime, _activeFuture.InstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        ...
                        //else if (_bCandle.ClosePrice < htvalue && _bCandle.OpenPrice > htvalue)// && dplus < dminus)  //down 
                        else if (previoushtred == 0 && htred == 1)// && htredLong == 0)
                        {
                            //int qty = _orderTrios.Count == 0 ? suspensionWindow ? 0 : _tradeQty : _orderTrios[0].Order.TransactionType.ToLower().Trim() == "buy" ? suspensionWindow ? _tradeQty : 2 * _tradeQty : 0;
                            _slTrace = false;
                            LastSwingHighlow(e.CloseTime, _activeFuture.InstrumentToken, out _swingHighPrice, out _swingLowPrice);

                           /* int qty = _orderTrios.Count == 0 ? suspensionWindow ? 0 : _swingHighPrice - _activeFuture.LastPrice < SL_THRESHOLD ? _tradeQty : 0 : 0;

                            if (qty > 0)
                            {
                                _callTrigerred = true;
                            }
                            else */if (_orderTrios.Count != 0 && _orderTrios[0].Order.TransactionType.ToLower() == "sell")
                            {
                        ...
                        else
                        {
                            if (_orderTrios.Count != 0)
                            {
                                if (_orderTrios[0].Order.TransactionType.ToLower() == "sell" && htred == 1)
                                {
                                    if (e.HighPrice > htvalue)
                                    {
                                        _slTrace = true;
                                    }
                                    else if (_slTrace && e.LowPrice < htvalue)
                                    {
                                        LastSwingHighlow(e.CloseTime, _activeFuture.InstrumentToken, out _swingHighPrice, out _swingLowPrice, doubleCandles: false);

                                        _orderTrios[0].StopLoss = Math.Min(_orderTrios[0].StopLoss, _swingHighPrice);
                                        _slTrace = false;
                                    }
                                }
                                else if (_orderTrios[0].Order.TransactionType.ToLower() == "buy" && htred == 0)
                                {
                                    if (e.LowPrice < htvalue)
                                    {
                                        _slTrace = true;
                                    }
                                    else if (_slTrace && e.HighPrice > htvalue)
                                    {
                                        LastSwingHighlow(e.CloseTime, _activeFuture.InstrumentToken, out _swingHighPrice, out _swingLowPrice, doubleCandles: false);
                    ...
                    if (_orderTrios.Count > 0 && ((_orderTrios[0].Order.TransactionType.ToLower().Trim() == "sell" && _activeFuture.LastPrice > _orderTrios[0].StopLoss && htred == 0)
                        || (_orderTrios[0].Order.TransactionType.ToLower().Trim() == "buy" && _activeFuture.LastPrice < _orderTrios[0].StopLoss && htred == 1)
                        ))
                    {
                        ...
                        LastSwingHighlow(e.CloseTime, _activeFuture.InstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        //if ((buyOrder && _activeFuture.LastPrice - _swingLowPrice > SL_THRESHOLD) || (!buyOrder && _swingHighPrice - _activeFuture.LastPrice > SL_THRESHOLD))
                        //{
                        //    qty = _tradeQty;
                        //}
                        if ((buyOrder && _activeFuture.LastPrice - _swingLowPrice < SL_THRESHOLD) || (!buyOrder && _swingHighPrice - _activeFuture.LastPrice < SL_THRESHOLD))
                    ...
                    //if ((_lastOrderTransactionType == "sell" && _activeFuture.LastPrice > _initialStopLoss)
                    //    || (_lastOrderTransactionType == "buy" && _activeFuture.LastPrice < _initialStopLoss)
                    //    )
                    //{

                    //    bool buyOrder = _lastOrderTransactionType == "sell";
                    //    //int qty = 2 * _tradeQty;
                    ...
                    //        OnTradeEntry(orderTrio.Order);
                    //        orderTrio.Option = _activeFuture;
                    //        _pnl += orderTrio.Order.Quantity * orderTrio.Order.AveragePrice * (_lastOrderTransactionType == "sell" ? -1 : 1);
        ...
        private void LastSwingHighlow(DateTime currentTime, uint token, out decimal swingHigh, out decimal swingLow, int pullCandles = 0, bool doubleCandles = true)
        {
            ...
            List<Candle> dcandles;
            if (doubleCandles)
            {
                //do the analysis with double candle size
                dcandles = new List<Candle>();
            ...
            foreach (var ohlc in dcandles.OrderByDescending(x => x.CloseTime))
            {
                ...
                if (ohlc.HighPrice > lastmaxPrice && ohlc.HighPrice > secondlastmaxprice && highCounter < 1)
                {
                ...
                if (ohlc.LowPrice < lastminPrice && ohlc.LowPrice < secondlastminprice && lowCounter < 1)
                {
                ...
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    if ((orderTrio.Order.TransactionType.ToLower() == "buy") && (orderTrio.TPFlag))
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                            _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo1", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                            broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                    ...
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo1", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML, 
                       broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
        ...
        private void CheckSLForEMABasedOrders(DateTime currentTime, decimal lastPrice, Candle pCandle, bool closeAll = false)
        {
            if (_orderTriosFromEMATrades.Count > 0)
            {
                for (int i = 0; i < _orderTriosFromEMATrades.Count; i++)
                {
                    ...
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                            _activeFuture.KToken, false, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo2", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                            broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
                    ...
                    {
                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, _tradeQty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "Algo2", product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                       broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
