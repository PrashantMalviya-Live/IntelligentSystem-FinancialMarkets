...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private readonly int STOP_LOSS_POINTS = 150;//50;//30; //40 points max loss on an average 
        private readonly int STOP_LOSS_POINTS_GAMMA = 150;//50;//30;
        ...
        private const decimal SL_TRAIL_LIMIT_TRAIL = 30;
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            _candleTimeSpanLong = new TimeSpan(1,0, 0, 0);
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    //    {
                    //        Candle bc = GetLastCandle(_candleTimeSpanLong.Minutes / _candleTimeSpan.Minutes, e.CloseTime);
                    //        _stockTokenHalfTrendLong[bc.InstrumentToken].Process(bc);
                    //    }
                    //}
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
               ...
               // decimal htredLong = _stockTokenHalfTrendLong[_baseInstrumentToken].GetTrend(); // 0 is up trend

                int tradeQty = 0;
                if (true || /*htredLong == 0 &&*/ (!_peSLHit || (htvalue > _previousUpswingValue && _peSLHit)))//&& _putTriggered)
                {
                    decimal stopLoss = 0;
                ...
                if (true || /*htredLong == 1 &&*/ (!_ceSLHit || (_ceSLHit &&  htvalue < _previousDownswingValue)))// && _callTrigerred)
                {
                    decimal stopLoss = 0;
    ...
    ref bool thirdTradeActive, ref decimal tradeCurrentProfit, ref bool firstTargetMoved, ref bool secondTargetMoved)
            ...
            int bookedQty, bool thirdTargetActive)
        {
            ...
            //else if (Math.Abs(triggerLevel - SL) < 270)// 150) //wednesday :90
            //{
            //    //tradedQuantity = bookedQty != 0 ? totalQuantity * 2 / 4 : totalQuantity * 3 / 4;
            //    tradedQuantity = totalQuantity * 2 / 4;
            //    tradedQuantity -= tradedQuantity % 25;
        ...
        private void LoadHistoricalCandlesForHT(List<uint> tokenList, int candlesCount, DateTime lastCandleEndTime)
        {
            ...
            //LoadHistoricalCandlesForHTLong(tokenList, candlesCount, lastCandleEndTime, new TimeSpan(0, 15, 0));
        }
