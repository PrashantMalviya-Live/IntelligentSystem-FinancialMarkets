    ...
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
            ...
            try
            {
                ...
                if (_baseCandleLoaded)// && _referenceCandleLoaded)
                {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        bool suspensionWindow = true;// _bCandle.CloseTime.TimeOfDay > SUSPENSION_WINDOW_START && _bCandle.CloseTime.TimeOfDay < SUSPENSION_WINDOW_END;
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - 30 && _putTriggered)
                    {
                    ...
                    else if (_putorderTrios.Count != 0 && (_baseInstrumentPrice - _putorderTrios[0].StopLoss < 300))
                    {
                        ...
                        if (true || _putorderTrios.Sum(x => x.Order.Quantity) == _tradeQty * Convert.ToInt32(_lotSize))
                        {
                            _putTriggered = false;
                            return;
                        }
                ...
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + 30) && _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
                    ...
                    else if (_callorderTrios.Count != 0 && (_callorderTrios[0].StopLoss - _baseInstrumentPrice < 300))
                    {
                        ...
                        if (true || _callorderTrios.Sum(x => x.Order.Quantity) == _tradeQty * Convert.ToInt32(_lotSize))
                        {
                            _callTrigerred = false;
                            return;
                        }
