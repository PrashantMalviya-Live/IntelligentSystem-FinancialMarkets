...
{
    public class EMAScalpingKB : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(EMAScalpingKB source);
        [field: NonSerialized]
        ...
        public const AlgoIndex algoIndex = AlgoIndex.EMAScalpingKB;
        ...
        private SortedDictionary<int, decimal> _criticalLevels;
        ...
        public EMAScalpingKB(TimeSpan candleTimeSpan, uint baseInstrumentToken,
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    foreach (var emaItem in _indexEMAs)
                    {
                        //Check if this updating the values correctly within the dictionary
                        emaItem.Value.Process(e.ClosePrice, isFinal: true);
                        _criticalLevels[emaItem.Key] = Math.Round( emaItem.Value.GetValue<Decimal>(0), 2);
                    }
...
//            {
        ...
        private void LoadCriticalLevels(uint token, DateTime currentTime)
        {
            try
            {
                ...
                //_criticalLevels = new SortedDictionary<int, decimal>();
                _criticalLevels.TryAdd(0, _previousDayOHLC.Close);
                _criticalLevels.TryAdd(1, _previousDayOHLC.High);
                _criticalLevels.TryAdd(2, _previousDayOHLC.Low);
                _criticalLevels.TryAdd(4, _cpr.Prices[(int)PivotLevel.CPR]);
                _criticalLevels.TryAdd(5, _cpr.Prices[(int)PivotLevel.R1]);
                _criticalLevels.TryAdd(6, _cpr.Prices[(int)PivotLevel.R2]);
                _criticalLevels.TryAdd(7, _cpr.Prices[(int)PivotLevel.R3]);
                _criticalLevels.TryAdd(8, _cpr.Prices[(int)PivotLevel.S1]);
                _criticalLevels.TryAdd(9, _cpr.Prices[(int)PivotLevel.S2]);
                _criticalLevels.TryAdd(10, _cpr.Prices[(int)PivotLevel.S3]);
...
//        }
        ...
        //{
        //    DataLogic dl = new DataLogic();
        //    DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.EMAScalpingKB, Convert.ToDateTime("2021-11-30"), Convert.ToDateTime("2021-12-30"));
