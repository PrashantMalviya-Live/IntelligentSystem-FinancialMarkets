...
namespace Algorithms.Algorithms
{
    public class RangeBreakoutCandle : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private PriceRange _currentPriceRange;
        private PriceRange _previousPriceRange;
        ...
        public void LoadActiveOrders(PriceActionInput paInput)
        {
            ...
            if (activeOrderTrios != null)
            {
                ...
                _pnl = paInput.PnL;
                _currentPriceRange = paInput.CPR;
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    if (SubscriptionTokens.Contains(token))
                    {
                        ...
                        if (!_HTLoaded)// && _currentPriceRange == null)
                        {
        ...
        private void TradeTP(DateTime currentTime, List<OrderTrio> orderTrios)
        {
            ...
            if (orderTrios.Count > 0)
            {
                for (int i = 0; i < orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType.ToLower() == "buy"
                        && ((option.InstrumentType.ToLower() == "ce" && _referenceIndex.LastPrice > orderTrio.TargetProfit)
                        || (option.InstrumentType.ToLower() == "pe" && _referenceIndex.LastPrice < orderTrio.TargetProfit))
                        )
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                                   option.KToken, false, orderTrio.Order.Quantity, algoIndex, currentTime, Tag: Convert.ToString(orderTrio.Order.AveragePrice),
        ...
        private void TradeSL(DateTime CurrentTime, List<OrderTrio> orderTrios, decimal lastPrice)
        {
            if (orderTrios.Count > 0)
            {
                for (int i = 0; i < orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType.ToLower() == "buy"
                         && ((option.InstrumentType.ToLower() == "ce" && lastPrice < orderTrio.StopLoss)
                        || (option.InstrumentType.ToLower() == "pe" && lastPrice > orderTrio.StopLoss))
                        )
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                   option.KToken, false, orderTrio.Order.Quantity, algoIndex, CurrentTime, Tag: Convert.ToString(orderTrio.Order.AveragePrice),
                   ...
                   httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                        ...
                        if (option.InstrumentType.ToLower() == "ce")
                        {
                            ...
                            //_shortTrigerred.Add(newTarget, newsl);


                            Trigger trigger = new Trigger() { tradePrice = orderTrio.StopLoss, sl = newsl, tp = newTarget, slOrder = true, up = false };
                            _shortTrigerred ??= new List<Trigger>();
                            _shortTrigerred.Add(trigger);
                        ...
                        else
                        {
                            ...
                            Trigger trigger = new Trigger() { tradePrice = orderTrio.StopLoss, sl = newsl, tp = newTarget, slOrder = true, up = true };
                            _longTrigerred ??= new List<Trigger>();
                            _longTrigerred.Add(trigger);
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
            ...
            if (_currentBreakOutMode == Breakout.UP)
            {
                ...
                decimal targetProfit = 2 * _previousPriceRange.Upper - _currentPriceRange.Lower;
                decimal sl = _currentPriceRange.Lower;

                //1:2 RR
                if (targetProfit - tick.LastPrice >= 2 * (tick.LastPrice - sl))
                {
                    _putTriggered = true;
            ...
            {
                //check if future has already reached the target;
                decimal targetProfit = 2 * _previousPriceRange.Lower - _currentPriceRange.Upper;
                decimal sl = _currentPriceRange.Upper;

                //1:2 RR
                if ((tick.LastPrice - targetProfit) >= 2 * (sl - tick.LastPrice))
                {
                    _callTrigered = true;
                    _currentBreakOutMode = Breakout.NONE;
                }
            ...
            if (_putTriggered)
            {
                ...
                //check if future has already reached the target;
                decimal targetProfit = 2 * _currentPriceRange.Upper - _currentPriceRange.Lower;
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                  _activeFuture.KToken, true, qty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "",
                  product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                  httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                ...
                orderTrio.BaseInstrumentStopLoss = _currentPriceRange.Lower;
                orderTrio.StopLoss = _currentPriceRange.Lower;
                orderTrio.EntryTradeTime = currentTime;
                ////first target is 1:1 R&R
                //orderTrio.TargetProfit = 2 * _previousPriceRange.Upper - _previousPriceRange.Lower;

                //first target is 1:1 R&R
                orderTrio.TargetProfit = 2 * _previousPriceRange.Upper - _currentPriceRange.Lower;
            ...
            else if(_callTrigered)
            {
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                  _activeFuture.KToken, false, qty * Convert.ToInt32(_activeFuture.LotSize), algoIndex, currentTime, Tag: "",
                  product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                  httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                ...
                //SL SHIFTED TO HIGH OF PREVIOUS CANDLE
                //orderTrio.BaseInstrumentStopLoss = stopLoss;
                orderTrio.BaseInstrumentStopLoss = _currentPriceRange.Upper;
                orderTrio.EntryTradeTime = currentTime;

                //first target is 1:1 R&R
                //orderTrio.TargetProfit = 2 * _previousPriceRange.Lower - _previousPriceRange.Upper;
                //_pnl += orderTrio.Order.Quantity * orderTrio.Order.AveragePrice * 1;
                orderTrio.TargetProfit = 2 * _currentPriceRange.Lower - _currentPriceRange.Upper;
                _pnl += orderTrio.Order.Quantity * orderTrio.Order.AveragePrice * 1;
        ...
        private Breakout UpdatePriceRange(Candle tc, int currentCandleIndex, uint instrumentToken)
        {
            ...
            _currentPriceRange ??= new PriceRange();
            _previousPriceRange??= new PriceRange();
            if (_currentPriceRange.Upper == 0)
            {
                _currentPriceRange.Upper = tc.HighPrice;
                _currentPriceRange.UpperCandleIndex = currentCandleIndex;
                breakout = Breakout.NONE;
            }
            else if (tc.ClosePrice > _currentPriceRange.Upper)
            {
                CopyPriceRange(_currentPriceRange, _previousPriceRange);
                _currentPriceRange.Upper = tc.HighPrice;
                //iterate through last breakout and this breakout to find lower range;
                if (currentCandleIndex != _currentPriceRange.UpperCandleIndex + 1)
                {
                    _currentPriceRange.Lower = GetLowerRange(_currentPriceRange.UpperCandleIndex, currentCandleIndex, instrumentToken);
                }
                _currentPriceRange.UpperCandleIndex = currentCandleIndex;
                breakout = Breakout.UP;

                DataLogic dl = new DataLogic();
                dl.UpdateAlgoParamaters(_algoInstance, arg3: _currentPriceRange.Upper, arg4: _currentPriceRange.Lower, arg5: Convert.ToInt32(breakout));
            }
            else if (tc.HighPrice > _currentPriceRange.Upper)
            {
                //CopyPriceRange(_currentPriceRange, _previousPriceRange);
                _currentPriceRange.Upper = tc.HighPrice;
                //_currentPriceRange.UpperCandleIndex = currentCandleIndex;
            ...
            if (_currentPriceRange.Lower == 0)
            {
                _currentPriceRange.Lower = tc.LowPrice;
                _currentPriceRange.LowerCandleIndex = currentCandleIndex;
                breakout = Breakout.NONE;
            }
            else if (tc.ClosePrice < _currentPriceRange.Lower)
            {
                CopyPriceRange(_currentPriceRange, _previousPriceRange);
                _currentPriceRange.Lower = tc.LowPrice;
                //iterate through last breakout and this breakout to find lower range;
                if (currentCandleIndex != _currentPriceRange.LowerCandleIndex + 1)
                {
                    _currentPriceRange.Upper = GetUpperRange(_currentPriceRange.LowerCandleIndex, currentCandleIndex, instrumentToken);
                }
                _currentPriceRange.LowerCandleIndex = currentCandleIndex;
            ...
            else if (tc.LowPrice < _currentPriceRange.Lower)
            {
                //CopyPriceRange(_currentPriceRange, _previousPriceRange);
                _currentPriceRange.Lower = tc.LowPrice;
                //_currentPriceRange.LowerCandleIndex = currentCandleIndex;
        ...
        private void CopyPriceRange(PriceRange fromPriceRange, PriceRange toPriceRange)
        {
            ...
            //    else if(tc.ClosePrice > _currentPriceRange.Upper)
            //    {
            //        _previousPriceRange = _currentPriceRange;
            //        _currentPriceRange.Upper = tc.HighPrice;
            //        _currentPriceRange.Lower = _currentPriceRange.nextLower; //_currentPriceRange.nextLower == 0 || _currentPriceRange.nextLower < tc.LowPrice ? tc.LowPrice : _currentPriceRange.nextLower;
            //        breakout = Breakout.UP;
            //    }
            //    else if (tc.ClosePrice > _currentPriceRange.nextUpper && _currentPriceRange.nextUpper != 0)
            //    {
            //        _currentPriceRange.nextUpper = tc.HighPrice;
            //    }
            //    if (_currentPriceRange.Lower == 0)
            //    {
            //        _currentPriceRange.Lower = tc.LowPrice;
            ...
            //        _previousPriceRange = _currentPriceRange;
            //        _currentPriceRange.Lower = tc.LowPrice;
            //        //_currentPriceRange.Upper = _currentPriceRange.nextUpper;// _currentPriceRange.nextUpper == 0 || _currentPriceRange.nextUpper > tc.HighPrice ? tc.HighPrice : _currentPriceRange.nextUpper;
            //        _currentPriceRange.Upper = _currentPriceRange.nextUpper != 0 && _currentPriceRange.nextUpper < tc.HighPrice ? tc.HighPrice : _currentPriceRange.nextUpper;
            //        breakout = Breakout.DOWN;
            //    }
            //    else if (tc.ClosePrice < _currentPriceRange.nextLower && _currentPriceRange.nextLower != 0)
            //    {
            //        _currentPriceRange.nextLower = tc.LowPrice;
            //    }


            //    if ((_currentPriceRange.nextUpper == 0 && _currentPriceRange.Upper != tc.HighPrice) || (_currentPriceRange.Upper == _currentPriceRange.nextUpper))
            //    {
            //        _currentPriceRange.nextUpper = tc.HighPrice;
            //    }
            //    if ((_currentPriceRange.nextLower == 0 && _currentPriceRange.Lower != tc.LowPrice)||(_currentPriceRange.nextLower == _currentPriceRange.Lower))
            //    {
            //        _currentPriceRange.nextLower = tc.LowPrice;
            //    }
            //    //if (tc.ClosePrice > _currentPriceRange.nextUpper && _currentPriceRange.nextUpper != 0)
            //    //{
            //    //    _currentPriceRange.nextUpper = tc.HighPrice;
            //    //}
            //    //if (tc.ClosePrice < _currentPriceRange.nextLower && _currentPriceRange.nextLower != 0)
            //    //{
            //    //    _currentPriceRange.nextLower = tc.LowPrice;
            //    //}

            //    //if (tc.HighPrice < _currentPriceRange.Upper && tc.LowPrice > _currentPriceRange.Lower)
            //    //{
            //    //    if(_currentPriceRange.nextUpper == 0 && _currentPriceRange.Upper != tc.HighPrice) {

            //    //        _currentPriceRange.nextUpper = tc.HighPrice;
            //    //    }
            //    //    if (_currentPriceRange.nextLower == 0 && _currentPriceRange.Lower != tc.LowPrice)
            //    //    {
            //    //        _currentPriceRange.nextLower = tc.LowPrice;
            //    //    }
            //    //    if (tc.ClosePrice > _currentPriceRange.nextUpper)
            //    //    {
            //    //        _currentPriceRange.nextUpper = tc.HighPrice;
            //    //    }
            //    //    if(tc.ClosePrice <  _currentPriceRange.nextLower)
            //    //    {
            //    //        _currentPriceRange.nextLower = tc.LowPrice;
            //    //    }
            //    //}
        ...
        private void TriggerEODPositionClose(DateTime currentTime)
        {
            if (currentTime.TimeOfDay > new TimeSpan(15, 00, 00))
            {
                ...
                //    CheckSL(currentTime, _orderTrios, null, closeAll: true);
                    ...
                    dl.UpdateAlgoParamaters(_algoInstance, arg3:_currentPriceRange.Upper, arg4:_currentPriceRange.Lower, arg5: Convert.ToInt32(_currentBreakOutMode));
                    if(_orderTrios ==null || _orderTrios.Count == 0)
                    {
            ...
            if (orderTrios != null && orderTrios.Count > 0)
            {
                for (int i = 0; i < orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType == "buy" && (lastPrice > orderTrio.TargetProfit))
                    {
                        ...
                        Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                       option.KToken, false, Math.Abs(_netQty) /*orderTrio.Order.Quantity*/, algoIndex, currentTime, Tag: Convert.ToString(orderTrio.Order.OrderId),
                       product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                    ...
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, Math.Abs(_netQty) /*orderTrio.Order.Quantity*/, algoIndex, currentTime, Tag: Convert.ToString(orderTrio.Order.OrderId),
                       product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
            ...
            if (orderTrios != null && orderTrios.Count > 0)
            {
                for (int i = 0; i < orderTrios.Count; i++)
                {
                    ...
                    if (orderTrio.Order.TransactionType == "sell" && (closeAll || c.ClosePrice > orderTrio.BaseInstrumentStopLoss))
                    {
                        int qty = closeAll ? orderTrio.Order.Quantity : 2 * orderTrio.Order.Quantity;
                        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeFuture.TradingSymbol, "fut", _activeFuture.LastPrice,
                       _activeFuture.KToken, true, 2* orderTrio.Order.Quantity, algoIndex, currentTime, Tag: Convert.ToString(orderTrio.Order.OrderId),
                       product: Constants.KPRODUCT_NRML, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        {
            DataLogic dl = new DataLogic();
            DateTime previousTradingDate = dl.GetPreviousTradingDate(lastCandleEndTime, 10);
