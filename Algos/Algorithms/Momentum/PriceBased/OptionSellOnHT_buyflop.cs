...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        private readonly int STOP_LOSS_POINTS = 200;//50;//30; //40 points max loss on an average 
        private readonly int STOP_LOSS_POINTS_GAMMA = 200;//50;//30;
        ...
        // 1 min: 30,50,70
        //25% quantity booked in each target
        private const decimal FIRST_TARGET = 30;
        ...
        private const decimal THIRD_TARGET = 70;
            ...
            try
            {
                ...
                if (_baseCandleLoaded)// && _referenceCandleLoaded)
                {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (suspensionWindow && previoushtred == 0 && htred == 1)
                        {
                            _callTrigerred = false;
                            _putTriggered = true;
                            //_uptrendhighswingindexvalue = _bCandle.HighPrice;
                            ...
                            else
                            {
                                ...
                                if (option.Strike != atmStrike && newQty > 0)
                                {
                                    ...
                                    for (int i = 0; i < _putorderTrios.Count; i++)
                                    {
                                        ...
                                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, "pe", option.LastPrice,
                                           option.KToken, false, qty, algoIndex, _bCandle.CloseTime,
                                           Tag: "",
                                           product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                    ...
                                    //buyback existing option, and sell at new swing low
                                    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, _atmOption.TradingSymbol, "pe", _atmOption.LastPrice,
                                       _atmOption.KToken, true, qty, algoIndex, _bCandle.CloseTime,
                                       Tag: "",
                                       product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                       broker: Constants.KOTAK,
                                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                                ...
                                else
                                {
                                    ...
                                    if (shouldbeQty > totalTradedQty)
                                    {
                                        ...
                                        //sell addtionalqty
                                        orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, "pe", option.LastPrice,
                                           option.KToken, true, addtionalQty, algoIndex, _bCandle.CloseTime,
                                           Tag: "",
                                           product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                           broker: Constants.KOTAK,
                                           httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                                        ...
                                        orderTrio.Option = option;
                                        orderTrio.BaseInstrumentStopLoss = _putorderTrios[0].StopLoss;
                        ...
                        else if (suspensionWindow && previoushtred == 1 && htred == 0)
                        {
                            _putTriggered = false;
                            _callTrigerred = true;
                            //_downtrendlowswingindexvalue = _bCandle.LowPrice;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (shouldbeQty > totalTradedQty)
                                    {
                                        ...
                                        orderTrio.Option = option;
                                        orderTrio.BaseInstrumentStopLoss = _callorderTrios[0].StopLoss;
                                        _initialStopLoss = orderTrio.BaseInstrumentStopLoss;
                                        orderTrio.EntryTradeTime = orderTrio.Order.OrderTimestamp.Value;
                                        //first target is 1:1 R&R
                                        orderTrio.TargetProfit = 2 * orderTrio.Order.AveragePrice - _uptrendhighswingindexvalue;
                       ...
                       // }
                        if(_callorderTrios.Count != 0)
                        {
                            ...
                            {
                                var orderTrio = _callorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy" && _baseInstrumentPrice < orderTrio.BaseInstrumentStopLoss && htred == 0)
                                {
                                    Instrument option = orderTrio.Option;
                                    ...
                                    _pnl += order.Quantity * order.AveragePrice;
                        ...
                        if (_putorderTrios.Count != 0)
                        {
                            ...
                            {
                                var orderTrio = _putorderTrios[i];
                                if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy" && _baseInstrumentPrice > orderTrio.BaseInstrumentStopLoss && htred == 1)
                                {
                                    Instrument option = orderTrio.Option;
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                int tradeQty = 0;
                if (htred == 1 )//&& _putTriggered)
                {
                    decimal stopLoss = 0;
                    if (_putorderTrios.Count == 0 && _baseInstrumentPrice >= htvalue - (_candleTimeSpan.Minutes == 5? 31: 11) && _putTriggered)
                    {
                        ...
                        stopLoss = _uptrendhighswingindexvalue;// _downtrendlowswingindexvalue;
                    ...
                    else if (_putorderTrios.Count != 0 && (_putorderTrios[0].StopLoss - _baseInstrumentPrice < 300))
                    {
                ...
                else if (htred == 0)// && _callTrigerred)
                {
                    decimal stopLoss = 0;
                    if (_callorderTrios.Count == 0 && (_baseInstrumentPrice <= htvalue + (_candleTimeSpan.Minutes == 5 ? 31 : 11)) && _callTrigerred)
                    {
                        //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
                        _firstCETargetActive = true;
                        _secondCETargetActive= true;
                        ...
                        stopLoss = _downtrendlowswingindexvalue;// _uptrendhighswingindexvalue;
                    ...
                    else if (_callorderTrios.Count != 0 && (_baseInstrumentPrice - _callorderTrios[0].StopLoss < 300))
                    {
                        //TODO: COMMENTED FOR NOT INCREASING QTY LATER
            ...
            if (currentLoss > maxLoss || closeAll)
            {
                for (int i = 0; i < orderTrios.Count; i++)
                {
                    ...
                    Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                       option.KToken, false, orderTrio.Order.Quantity, algoIndex, _bCandle.CloseTime, Tag: "",
                       product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                       broker: Constants.KOTAK,
                       httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                if (orderTrio.Option.InstrumentToken == tick.InstrumentToken)
                {
                    ...
                    //))
                    {
                       ...
                       // }
                       // else
                        if ((tick.LastPrice > orderTrio.Order.AveragePrice + FIRST_TARGET)
                        && totalTradedQty > (_tradeQty * Convert.ToInt32(_lotSize) * 2 / 4)
                        && firstTradeActive)
                        {
                        ...
                        else if ((tick.LastPrice > orderTrio.Order.AveragePrice + SECOND_TARGET)
                        && totalTradedQty > (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                        && secondTradeActive)
                        {
                        ...
                        else if ((tick.LastPrice > orderTrio.Order.AveragePrice + THIRD_TARGET)
                        && totalTradedQty >= (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4)
                        && thirdTradeActive)
                        {
                        ...
                        // Trend change , book 1/4 provided it is in profit
                        else if ((tick.LastPrice > orderTrio.Order.AveragePrice + FIRST_TARGET && trendChanged)
                        && totalTradedQty == (_tradeQty * Convert.ToInt32(_lotSize) * 1 / 4))
                        {
                            qty = Convert.ToInt32(_thirdTargetQty * totalQty);
                        }
