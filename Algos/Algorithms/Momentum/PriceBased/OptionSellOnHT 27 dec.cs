...
namespace Algorithms.Algorithms
{
    public class OptionSellonHT : IZMQ, IObserver<Tick>//, IKConsumer
    {
        ...
        decimal downtrendlowswingindexvalue = 0;
        decimal uptrendhighswingindexvalue = 0;
        ...
        private const int STOP_LOSS_POINTS = 40; //40 points max loss on an average 
            ...
            try
            {
                ...
                if (_baseCandleLoaded)// && _referenceCandleLoaded)
                {
                    ...
                    if (_stockTokenHalfTrend.ContainsKey(_bCandle.InstrumentToken))
                    {
                        ...
                        if (htred = 1 && _bCandle.LowPrice < _lowtrendlowswingindexvalue && _lowtrendlowswingindexvalue > 0)
                        {
                            _lowtrendlowswingindexvalue = _bCandle.LowPrice;
                        }
                        ...
                        if (previoushtred == 1 && htred == 0)
                        {
                            ...
                            else
                            {
                                ...
                                decimal atmStrike = Math.Floor((_swingLowPrice - 50) / 100) * 100;// Math.Floor(_swingLowPrice / 100) * 100;

                                if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
                                {
                                    atmStrike = Math.Floor((_swingLowPrice - 25) / 50) * 50;// Math.Floor(_swingLowPrice / 50) * 50; 
                                }
                                ...
                                if (option.Strike != atmStrike)
                                {
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _swingLowPrice, _tradeQty * Convert.ToInt32(_lotSize), 0);
                        ...
                        else if (previoushtred == 0 && htred == 1)
                        {
                            ...
                            else
                            {
                                ...
                                decimal atmStrike = Math.Ceiling((_swingHighPrice + 50) / 100) * 100;// Math.Ceiling(_swingHighPrice / 100) * 100;
                                if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
                                {
                                    atmStrike = Math.Ceiling((_swingHighPrice + 25) / 50) * 50;// Math.Ceiling(_swingHighPrice / 50) * 50;
                                }
                                ...
                                if (option.Strike != atmStrike)
                                {
                                    ...
                                    qty = GetTradedQuantity(_baseInstrumentPrice, _swingHighPrice, _tradeQty * Convert.ToInt32(_lotSize), 0);
        ...
        private void ExecuteTrade(Tick tick, DateTime currentTime)
        {
                ...
                )
            {
                ...
                if (htred == 0 && _putTriggered)
                {
                    ...
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][Math.Floor((stopLoss - 50) / 100) * 100]; //Change -  Take position 50 points farther to avoid gamma loss
                ...
                else if (htred == 1 && _callTrigerred)
                {
                    ...
                    OrderTrio orderTrio = new OrderTrio();
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((stopLoss + 50) / 100) * 100]; //Change -  Take position 50 points farther to avoid gamma loss

                    if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
                    {
                        _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((stopLoss + 25) / 50) * 50]; //Change -  Take position 50 points farther to avoid gamma loss
                    }
        ...
        private bool BookStopLoss(List<OrderTrio> orderTrios)
        {
        ...
        private int GetTradedQuantity(decimal triggerLevel, decimal SL, int totalQuantity, int bookedQty)
        {
            ...
            if (Math.Abs(triggerLevel - SL) < 90)// && bookedQty != 0)
            {
        ...
        private void LoadHistoricalCandlesForHT(List<uint> tokenList, int candlesCount, DateTime lastCandleEndTime, TimeSpan timeSpan)
        {
            ...
            lock (_stockTokenHalfTrend)
            {
                ...
                DataLogic dl = new DataLogic();
                DateTime previousTradingDate = dl.GetPreviousTradingDate(lastCandleEndTime, timeSpan.Minutes == 5 ? 1 : 5);
        ...
        private void TakeTrade(DateTime currentTime)
        {
                ...
                )
            {
                    ...
                    && _putTriggered)
                {
                    ...
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.PE][Math.Floor((_swingLowPrice - 50) / 100) * 100]; //Change -  Take position 50 points farther to avoid gamma loss

                    if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
                    {
                        _atmOption = OptionUniverse[(int)InstrumentType.PE][Math.Floor((_swingLowPrice - 25) / 50) * 50]; //Change -  Take position 50 points farther to avoid gamma loss
                    }
                    ...
                    && _callTrigerred)
                {
                    ...
                    OrderTrio orderTrio = new OrderTrio();
                    Instrument _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((_swingHighPrice + 50) / 100) * 100]; //Change -  Take position 50 points farther to avoid gamma loss

                    if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
                    {
                        _atmOption = OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((_swingHighPrice + 25) / 50) * 50]; //Change -  Take position 50 points farther to avoid gamma loss
                    }
