...
{
    public class BC : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(BC source);
        [field: NonSerialized]
        ...
        //token and minute of loading
        private Dictionary<uint, int> _optionLoaded;
        ...
        private const int STOP_LOSS_POINTS = 30;//30; //40 points max loss on an average 
        private const int STOP_LOSS_POINTS_GAMMA = 30;//30;
        private const int DAY_STOP_LOSS_PERCENT = 1; //40 points max loss on an average 


        private const decimal CANDLE_WICK_SIZE_PERCENT_OF_BODY = 0.075m;//45;//10000;//45;//35
        private const decimal CANDLE_BODY_MIN = 5;
        private const decimal CANDLE_BODY_PERCENT_OF_PRICE = 0.00154m;//20;//40; //25
        private const decimal OPTION_CANDLE_BODY_PERCENT_OF_PRICE = 0.10m;//20;//40; //25
        ...
        private const decimal RSI_THRESHOLD = 60;
        ...
        //Dictionary<uint, bool> _firstCandleOpenPriceNeeded2;
        public const AlgoIndex algoIndex = AlgoIndex.BC2;
        ...
        //CandleManger candleManger2;
        Dictionary<uint, List<Candle>> TimeCandles;
        //Dictionary<uint, List<Candle>> TimeCandles2;
        ...
        Dictionary<uint, ExponentialMovingAverage> _tokenEMA;
        Dictionary<uint, ExponentialMovingAverage> _tokenShortEMA;
        Dictionary<uint, RelativeStrengthIndex> _tokenRSI;
        Dictionary<uint, ExponentialMovingAverage> _tokenRSIEMA;

        private const int _emaShortLength = 5;
        private const int _emaLength = 9;
        private const int _rsiLength = 14;
        private const int _rsiEMALength = 20;
        ...
        public BC(TimeSpan candleTimeSpan, uint baseInstrumentToken,
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            //_candleShortTimeSpan = new TimeSpan(0, 5, 0);
            _baseInstrumentToken = baseInstrumentToken;
            _optionLoaded = new Dictionary<uint, int>();
            _stopTrade = true;
            ...
            _tokenRSI = new Dictionary<uint, RelativeStrengthIndex>();
            _tokenEMA = new Dictionary<uint, ExponentialMovingAverage>();
            _tokenShortEMA = new Dictionary<uint, ExponentialMovingAverage>();
            _tokenRSIEMA = new Dictionary<uint, ExponentialMovingAverage>();
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            //_firstCandleOpenPriceNeeded2 = new Dictionary<uint, bool>();
            TimeCandles = new Dictionary<uint, List<Candle>>();
            //TimeCandles2 = new Dictionary<uint, List<Candle>>();
            
            candleManger = new CandleManger(TimeCandles, CandleType.Time);
            candleManger.TimeCandleFinished += CandleManger_TimeCandleFinished;

            //candleManger2 = new CandleManger(TimeCandles2, CandleType.Time);
            //candleManger2.TimeCandleFinished += CandleManger_TimeCandleFinished2;
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                ...
                _optionLoaded[e.InstrumentToken] = e.CloseTime.Minute;

                if (_tokenRSI.ContainsKey(e.InstrumentToken))
                {
                    _tokenRSI[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    _tokenEMA[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    _tokenShortEMA[e.InstrumentToken].Process(e.ClosePrice, isFinal: true);
                    _tokenRSIEMA[e.InstrumentToken].Process(_tokenRSI[e.InstrumentToken].GetCurrentValue<decimal>(), isFinal: true);
                }

                //Buy Logic: Bullish Candle, closing near the top. Both in options and index. RSI above 60
                // Both index and option should cross 9ema and rsi should be above rsi ema
                if (_optionLoaded[_baseInstrumentToken] == e.CloseTime.Minute)
                {
                    if ( ((_bCandle.ClosePrice > Math.Max(_bCandle.OpenPrice, _tokenEMA[_baseInstrumentToken].GetCurrentValue<decimal>()) && (_bCandle.OpenPrice  < _tokenEMA[_baseInstrumentToken].GetCurrentValue<decimal>())) ||
                        (_bCandle.ClosePrice < Math.Min(_bCandle.OpenPrice, _tokenEMA[_baseInstrumentToken].GetCurrentValue<decimal>()) && (_bCandle.OpenPrice > _tokenEMA[_baseInstrumentToken].GetCurrentValue<decimal>())))
                       // && Math.Abs(_baseInstrumentPrice - _tokenEMA[_baseInstrumentToken].GetCurrentValue<decimal>()) < 80
                        )
                    {
                        //if (_bCandle.ClosePrice > _bCandle.OpenPrice)
                        //{
                        Instrument atmOption = _bCandle.ClosePrice > _bCandle.OpenPrice ? OptionUniverse[(int)InstrumentType.CE][GetCEStrike(_baseInstrumentPrice)]
                                : OptionUniverse[(int)InstrumentType.PE][GetPEStrike(_baseInstrumentPrice)];

                        

                        if ((_lastTradeTime == null || _lastTradeTime != e.CloseTime)
                        && _optionLoaded[atmOption.InstrumentToken] == e.CloseTime.Minute
                        //&& (true || ValidateCandleSize(TimeCandles[atmOption.InstrumentToken].Last(x=>x.State== CandleStates.Finished), option:true) )
                        && _tokenRSI[atmOption.InstrumentToken].IsFormed
                       // && _tokenRSI[atmOption.InstrumentToken].GetCurrentValue<decimal>() > RSI_THRESHOLD
                        && _tokenRSI[atmOption.InstrumentToken].GetCurrentValue<decimal>() > _tokenRSIEMA[atmOption.InstrumentToken].GetCurrentValue<decimal>()
                        && !_orderTrios.Any(x => x.Option.InstrumentType == atmOption.InstrumentType)
                        //&& e.CloseTime.TimeOfDay >= new TimeSpan(13,0,0)
                        )
                        {
                            decimal closePrice = TimeCandles[atmOption.InstrumentToken].Last(x => x.State == CandleStates.Finished).ClosePrice;
                            if (closePrice > _tokenEMA[atmOption.InstrumentToken].GetCurrentValue<decimal>())
                            {
                                DataLogic dl = new DataLogic();

                                if (_orderTrios.Count > 0 && _orderTrios[0].Option.InstrumentType != atmOption.InstrumentType)
                                {
                                    Instrument option = _orderTrios[0].Option;
                                    Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice,
                                  option.KToken, false, _orderTrios[0].Order.Quantity, algoIndex, e.CloseTime, Tag: Math.Round(_tokenRSI[atmOption.InstrumentToken].GetCurrentValue<decimal>(), 1).ToString(),
                                  ...
                                  httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                                    ...
                                    dl.UpdateOrderTrio(_orderTrios[0], _algoInstance);
                                ...
                                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, atmOption.TradingSymbol, atmOption.InstrumentType, atmOption.LastPrice,
                                   atmOption.KToken, true, _tradeQty * Convert.ToInt32(atmOption.LotSize), algoIndex, e.CloseTime,
                                   Tag: Math.Round(_tokenRSI[atmOption.InstrumentToken].GetCurrentValue<decimal>(), 1).ToString(),
                                   product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                   broker: Constants.KOTAK, HsServerId: _user.HsServerId, httpClient: _httpClientFactory == null ? null
                                   : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);

                                OnTradeEntry(orderTrio.Order);
                                orderTrio.Option = atmOption;

                                decimal swingHigh, swingLow;
                                LastSwingHighlow(e.CloseTime, atmOption.InstrumentToken, out swingHigh, out swingLow);
                                orderTrio.StopLoss = swingLow;

                                //orderTrio.StopLoss = TimeCandles[atmOption.InstrumentToken].Last(x => x.State == CandleStates.Finished).LowPrice;

                                orderTrio.EntryTradeTime = e.CloseTime;
                                _lastTradeTime = e.CloseTime;
                                orderTrio.TargetProfit = 2 * _tokenEMA[atmOption.InstrumentToken].GetCurrentValue<decimal>() - swingLow;  // 2 * orderTrio.Order.AveragePrice - orderTrio.StopLoss;
                                orderTrio.Order.Quantity = _tradeQty * Convert.ToInt32(atmOption.LotSize);
                    ...
                    if (_orderTrios.Count != 0)
                    {
                        for (int i = 0; i < _orderTrios.Count; i++)
                        {
                            ...
                            decimal closePrice = TimeCandles[option.InstrumentToken].Last(x => x.State == CandleStates.Finished).ClosePrice;

                            var candles = TimeCandles[option.InstrumentToken].SkipWhile(x => x.CloseTime <= orderTrio.EntryTradeTime);
                            if(candles.Count() < 2)
                            {
                                return;
                            }    
                            Candle optionLastCandle = candles.Last(x => x.State == CandleStates.Finished);
                            Candle optionPreviousCandle = candles.SkipLast(1).Last(x => x.State == CandleStates.Finished);
                            if (orderTrio.Order.TransactionType.ToLower().Trim() == "buy" && (closePrice < _tokenShortEMA[option.InstrumentToken].GetCurrentValue<decimal>())
                                //(
                                //(option.InstrumentType.ToLower() == "ce" &&  closePrice < _tokenShortEMA[option.InstrumentToken].GetCurrentValue<decimal>()) ||
                                //(option.InstrumentType.ToLower() == "pe" && closePrice > _tokenShortEMA[option.InstrumentToken].GetCurrentValue<decimal>())

                                //)// || optionClosePrice < orderTrio.StopLoss)// || optionClosePrice > orderTrio.TargetProfit)
                                || ((optionLastCandle.ClosePrice < optionPreviousCandle.ClosePrice)
                                && (optionLastCandle.ClosePrice < optionLastCandle.OpenPrice) 
                                && (optionPreviousCandle.ClosePrice < optionPreviousCandle.OpenPrice))
                                //|| (orderTrio.Order.AveragePrice - 10 < option.LastPrice)
                                )
                            {
                                Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice,
                                   option.KToken, false, orderTrio.Order.Quantity, algoIndex, e.CloseTime,
                                   Tag: Math.Round(_tokenRSI[option.InstrumentToken].GetCurrentValue<decimal>(), 1).ToString(),
                                   product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                                   broker: Constants.KOTAK,
                                   httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                ...
                //_optionLoaded[e.InstrumentToken] = false;
            }
            catch (Exception ex)
            {
        ...
        private void CheckExit(DateTime currentTime)
        {
            if (_orderTrios.Count != 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    //{
                    //    lastPrice = OptionUniverse[(int)InstrumentType.CE][option.Strike].LastPrice;
                    //}
                    //else
                    //{
                    //    lastPrice = OptionUniverse[(int)InstrumentType.PE][option.Strike].LastPrice;
                    //}

                    if (orderTrio.Order.AveragePrice - 10 > option.LastPrice)
                    {
                        Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice,
                           option.KToken, false, orderTrio.Order.Quantity, algoIndex, currentTime,
                           Tag: Math.Round(_tokenRSI[option.InstrumentToken].GetCurrentValue<decimal>(), 1).ToString(),
                           product: _intraday ? Constants.PRODUCT_MIS : Constants.KPRODUCT_NRML,
                           broker: Constants.KOTAK,
                           httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        private void LoadHistoricalHT(DateTime currentTime)
        {
            try
            {
                ...
                var tokens = SubscriptionTokens.Where(x => !_HTLoaded.Contains(x));
                ...
                foreach (uint t in tokens)
                {
                    ...
                    if (!_tokenRSI.ContainsKey(t) && !_SQLLoadingHT.Contains(t))
                    {
                        newTokens.Add(t);
                        sb.AppendFormat("{0},", t);
                        _SQLLoadingHT.Add(t);
...
#if BACKTEST
                    LoadHistoricalCandlesForHT(newTokens, _rsiEMALength * 2 + _rsiLength, lastCandleEndTime);
                    //LoadHistoricalCandlesFromDB(tokenList, _rsiEMALength * 2 + _rsiLength, lastCandleEndTime);
                ...
                foreach (uint tkn in tokens)
                {
                    ...
                    if (TimeCandles.ContainsKey(tkn) && _tokenRSI.ContainsKey(tkn))
                    {
                        if (_firstCandleOpenPriceNeeded[tkn])
                        {
                            ...
                            _tokenRSI[tkn].Process(TimeCandles[tkn].First().ClosePrice, isFinal:true);
                            _tokenEMA[tkn].Process(TimeCandles[tkn].First().ClosePrice, isFinal: true);
                            _tokenShortEMA[tkn].Process(TimeCandles[tkn].First().ClosePrice, isFinal: true);
                            _tokenRSIEMA[tkn].Process(_tokenRSI[tkn].GetCurrentValue<decimal>(), isFinal: true);
                        ...
                        if (TimeCandles[tkn].Count > 1)
                        {
                            ...
                            {
                                _tokenRSI[tkn].Process(price.ClosePrice, isFinal: true);
                                _tokenEMA[tkn].Process(price.ClosePrice, isFinal: true);
                                _tokenShortEMA[tkn].Process(price.ClosePrice, isFinal: true);
                                _tokenRSIEMA[tkn].Process(_tokenRSI[tkn].GetCurrentValue<decimal>(), isFinal: true);
                    ...
                    if ((firstCandleFormed == 1 || !_firstCandleOpenPriceNeeded[tkn]) && _tokenRSI.ContainsKey(tkn))
                    {
                        _HTLoaded.Add(tkn);
        ...
        private void LoadHistoricalCandlesForHT(List<uint> tokenList, int candlesCount, DateTime lastCandleEndTime, TimeSpan timeSpan)
        {
            lock (_tokenRSIEMA)
            {
                ...
                DataLogic dl = new DataLogic();
                DateTime previousTradingDate = dl.GetPreviousTradingDate(lastCandleEndTime, 2);// timeSpan.Minutes == 5 ? 1 : timeSpan.Minutes);


                ExponentialMovingAverage lema, optionema, sema; //.Process(candle.ClosePrice, isFinal: true)
                ...
                foreach (uint token in tokenList)
                {
                    ...
                    optionema = new ExponentialMovingAverage(length: _emaLength);
                    sema = new ExponentialMovingAverage(length: _emaShortLength);
                    foreach (var price in historicals)
                    {
                        rsi.Process(price.Close, isFinal: true);
                        optionema.Process(price.Close, isFinal: true);
                    }
                    _tokenRSI.Add(token, rsi);
                    _tokenEMA.Add(token, optionema);

                    //if (token == _baseInstrumentToken)
                    //{
                    foreach (var price in historicals)
                        {
                            sema.Process(price.Close, isFinal: true);
                        }
                        _tokenShortEMA.Add(token, sema);
                    //}
                   

                    lema = new ExponentialMovingAverage(length: _rsiEMALength);
                    for (int i = _rsiEMALength * 2 - 1; i >= 0; i--)
                    {
                        lema.Process(_tokenRSI[token].GetValue<decimal>(i), isFinal: true);
                    }

                    _tokenRSIEMA.Add(token, lema);
                }
            }
        }
        ...
        //        {
        //            decimal stopLoss = 0;
        //            if (_putorderTrios.Count == 0 && htvalue >= _baseInstrumentPrice - 11 && _putTriggered)
        //            {
        ...
        //                //Instrument atmOption = OptionUniverse[(int)InstrumentType.PE][GetPEStrike(_baseInstrumentPrice, stopLoss)]; //Change -  Take position 50 points farther to avoid gamma loss

        //                //if (_putorderTrios.Last().Order.AveragePrice + 15 > _putorderTrios.Last().Option.LastPrice)
        //                //{
        //                //    return;
        //                //}
        //            }
        ...
        //        {
        //            decimal stopLoss = 0;
        //            if (_callorderTrios.Count == 0 && (htvalue <= _baseInstrumentPrice + 11) && _callTrigerred)
        //            {
        //                //LastSwingHighlow(currentTime, _baseInstrumentToken, out _swingHighPrice, out _swingLowPrice);
        ...
        //                //Instrument atmOption = OptionUniverse[(int)InstrumentType.CE][GetCEStrike(_baseInstrumentPrice, stopLoss)]; //Change -  Take position 50 points farther to avoid gamma loss

        //                //if (_callorderTrios.Last().Order.AveragePrice + 15 > _callorderTrios.Last().Option.LastPrice)
        //                //{
        //                //    return;
        //                //}
        //            }
        ...
        private void LoadHistoricalCandlesFromDB(string tokenList, int candlesCount, DateTime lastCandleEndTime)
        {
            try
            {
                lock (_tokenRSIEMA)
                {
                    ...
                    ExponentialMovingAverage lema, oema, sema; //.Process(candle.ClosePrice, isFinal: true)
                    RelativeStrengthIndex rsi;

                    foreach (uint t in historicalCandlePrices.Keys)
                    {
                        rsi = new RelativeStrengthIndex();
                        oema = new ExponentialMovingAverage(length: _emaLength);
                        sema = new ExponentialMovingAverage(length: _emaShortLength);

                        foreach (var price in historicalCandlePrices[t])
                        {
                            rsi.Process(price, isFinal: true);
                            oema.Process(price, isFinal: true);
                        }
                        _tokenRSI.TryAdd(t, rsi);
                        _tokenEMA.TryAdd(t, oema);

                        //if (t != _baseInstrumentToken)
                        //{
                        foreach (var price in historicalCandlePrices[t])
                        {
                            sema.Process(price, isFinal: true);
                        }
                        _tokenShortEMA.TryAdd(t, sema);
                        //}

                        lema = new ExponentialMovingAverage(length: _rsiEMALength);
                        for (int i = _rsiEMALength * 2 - 1; i >= 0; i--)
                        {
                            lema.Process(_tokenRSI[t].GetValue<decimal>(i), isFinal: true);
                        }

                        _tokenRSIEMA.TryAdd(t, lema);
        ...
        private decimal GetPEStrike(decimal bPrice)
        {
            decimal strike;

            if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
            {
                strike = Math.Floor((bPrice - 25) / 50) * 50;
            }
            else
            {
                strike = Math.Floor((bPrice) / 100) * 100;
            }

            return strike;
        }
        private decimal GetCEStrike(decimal bPrice)
        {
            decimal strike;

            if (_baseInstrumentToken == Convert.ToUInt32(Constants.NIFTY_TOKEN))
            {
                strike = Math.Ceiling((bPrice + 25) / 50) * 50;
            }
            else
            {
                strike = Math.Ceiling((bPrice) / 100) * 100;
        ...
        private bool ValidateCandleSize(Candle c, bool option)
        {
            bool validCandle = false;

            decimal percentThreshold = option ? OPTION_CANDLE_BODY_PERCENT_OF_PRICE : CANDLE_BODY_PERCENT_OF_PRICE;


            if (Math.Abs(c.ClosePrice - c.OpenPrice)/c.OpenPrice > percentThreshold)
            {
                validCandle = (c.ClosePrice - c.OpenPrice) > 0 ? (((c.OpenPrice - c.LowPrice) >= (c.HighPrice - c.ClosePrice) * 0.9m && ((c.OpenPrice - c.LowPrice)/(c.ClosePrice-c.OpenPrice) < CANDLE_WICK_SIZE_PERCENT_OF_BODY))
                ...
                (((c.ClosePrice - c.LowPrice) * 0.9m <= (c.HighPrice - c.OpenPrice) && ((c.HighPrice - c.OpenPrice)/(c.ClosePrice-c.OpenPrice) < CANDLE_WICK_SIZE_PERCENT_OF_BODY))
        ...
        private void LoadOptionsToTrade(DateTime currentTime)
        {
            try
            {
                    ...
                    )
                {
                    ...
                    foreach (var key in mTokens.Keys)
                    {
                       // _tokenRSI.TryAdd(key, new RelativeStrengthIndex());
                        _optionLoaded.TryAdd(key, -1);
                    }
                    _optionLoaded.TryAdd(_baseInstrumentToken, -1);
