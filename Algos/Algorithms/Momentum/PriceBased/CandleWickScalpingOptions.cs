...
{
    public class CandleWickScalpingOptions : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(CandleWickScalpingOptions source);
        [field: NonSerialized]
        ...
        public const AlgoIndex algoIndex = AlgoIndex.CandleWickScalping;
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                ...
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    ...
                    {
                        //breakout
                        TakeTrade("ce", e.CloseTime, RR_BREAKOUT * (e.ClosePrice - _resistancePriceRange.Upper), e.ClosePrice - _resistancePriceRange.Upper);
                    }
                    ...
                    {
                        TakeTrade("ce", e.CloseTime, RR_BREAKDOWN * (e.ClosePrice - _supportPriceRange.Lower), e.ClosePrice - _supportPriceRange.Lower);
                    }
                    ...
                    {
                        //breakout
                        TakeTrade("pe", e.CloseTime, RR_BREAKOUT * (_supportPriceRange.Lower - e.ClosePrice), _supportPriceRange.Lower - e.ClosePrice);
                    }
        ...
        private void TakeTrade(string instrumentType, DateTime currentTime, decimal targetLevel, decimal stopLevel)
        {
            int qty = _tradeQty;
            ...
            if (qty > 0)
            {
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, atmOption.TradingSymbol, atmOption.InstrumentType.ToLower(), atmOption.LastPrice, //e.ClosePrice,
                   atmOption.KToken, true, _tradeQty * Convert.ToInt32(atmOption.LotSize), algoIndex, currentTime, Tag: "Algo4",
        ...
        private decimal CloseTrade(DateTime currentTime, Instrument option)
        {
            ...
            if (_orderTrios.Count > 0)
            {
                ...
                _pnl += (orderTrio.Order.AveragePrice - _orderTrios[0].Order.AveragePrice) * _tradeQty * Convert.ToInt32(option.LotSize);
                //orderTrio.TPFlag = false;
                ////atmOption.InstrumentToken = _baseInstrumentToken;
            ...
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                        ...
                        || ((AllOptions[tradedOption.InstrumentToken].LastPrice < orderTrio.StopLoss) && sl))
                    {
                        ...
                        //_pnl += (tradePrice - orderTrio.Order.AveragePrice) * _tradeQty * Convert.ToInt32(tradedOption.LotSize);
                        _orderTrios.Remove(orderTrio);
        ...
        private void CheckSL(DateTime currentTime)
        {
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    _pnl += (orderTrio.Order.AveragePrice - oldPrice) * _tradeQty* orderTrio.Option.LotSize;
