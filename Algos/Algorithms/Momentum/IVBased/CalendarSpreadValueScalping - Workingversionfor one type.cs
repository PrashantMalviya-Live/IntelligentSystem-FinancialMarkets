...
namespace Algorithms.Algorithms
{
    public class CalendarSpreadValueScalping : IZMQ
    {
        ...
        public Dictionary<decimal, decimal> HistoricalSpread { get; set; }
        ...
        public CalendarSpreadValueScalping(DateTime endTime, uint baseInstrumentToken,
            DateTime? nearExpiry, DateTime? farExpiry, int quantity, int maxQty, int stepQty, decimal targetProfit, decimal stopLoss,
            int algoInstance = 0, bool positionSizing = false, decimal maxLossPerTrade = 0)
        {
            ...
            HistoricalSpread = new Dictionary<decimal, decimal>();
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    else if (tick.LastTradeTime.HasValue)
                    {
                        ...
                        Option nearOption = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.Expiry.Value.Date == _nearExpiry.Value.Date);
                        Option farOption = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.Expiry.Value.Date == _farExpiry.Value.Date);


                        if (option.Expiry.Value.Date == _nearExpiry.Value.Date)
                        {
                            CurrentSpread[option.Strike][NEAR_EXPIRY] = tick.LastPrice;
                            nearOption.LastPrice = tick.LastPrice;
                        }
                        else
                        {
                            CurrentSpread[option.Strike][FAR_EXPIRY] = tick.LastPrice;
                            farOption.LastPrice = tick.LastPrice;
                        }

                        decimal currentSpread = CurrentSpread[option.Strike][NEAR_EXPIRY] - CurrentSpread[option.Strike][FAR_EXPIRY];

                        if (CurrentSpread[option.Strike][NEAR_EXPIRY] != 0 && CurrentSpread[option.Strike][FAR_EXPIRY] != 0)
                        {
                            Console.WriteLine("Strike:{0}, Spread:{1}, Historical:{2} ", option.Strike, currentSpread, HistoricalSpread[option.Strike]);
                            ...
                            if ( (currentSpread -  (TradedSpread[option.Strike] == 0 ? HistoricalSpread[option.Strike] : TradedSpread[option.Strike])) > 20
                                && _tradedQty < _maxQty - _stepQty
                                && (_activeOrders == null || _activeOrders[0].InstrumentToken == nearOption.InstrumentToken
                                || _activeOrders[1].InstrumentToken == farOption.InstrumentToken))
                            {
                                Order order = MarketOrders.PlaceOrder(_algoInstance, farOption.TradingSymbol, farOption.InstrumentType, farOption.LastPrice,
                                ...
                                order = MarketOrders.PlaceOrder(_algoInstance, nearOption.TradingSymbol, nearOption.InstrumentType, nearOption.LastPrice,
                                ...
                                TradedSpread[option.Strike] = currentSpread;
                            }

                            HistoricalSpread[option.Strike] = HistoricalSpread[option.Strike] == 0 ? currentSpread : (HistoricalSpread[option.Strike] * 300 + currentSpread) / 301;
                            CurrentSpread[option.Strike][NEAR_EXPIRY] = 0;
                            CurrentSpread[option.Strike][FAR_EXPIRY] = 0;


                            if (_tradedQty > 0 && HistoricalSpread[option.Strike] >= currentSpread && TradedSpread[option.Strike] != 0)
                            {
                                ...
                                Order order = MarketOrders.PlaceOrder(_algoInstance, nearOption.TradingSymbol, nearOption.InstrumentType, nearOption.LastPrice,
                                    nearOption.InstrumentToken, true, _stepQty * Convert.ToInt32(nearOption.LotSize),
                                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET);

                                order = MarketOrders.PlaceOrder(_algoInstance, farOption.TradingSymbol, farOption.InstrumentType, farOption.LastPrice,
                                   farOption.InstrumentToken, false, _stepQty * Convert.ToInt32(farOption.LotSize),
                                   algoIndex, currentTime, Constants.ORDER_TYPE_MARKET);

                                _tradedQty = _tradedQty - _stepQty;
                                TradedSpread[option.Strike] = 0;
                            }
                        }
                       // }
        ...
        private void LoadOptionsToTrade(DateTime currentTime)
        {
            try
            {
                if (SpreadUniverse == null)
                {
                    ...
                    {
                        CurrentSpread.TryAdd(strike, new decimal[] { 0, 0 });
                        TradedSpread.TryAdd(strike,  0);
                        HistoricalSpread.TryAdd(strike, 0);
                    }
