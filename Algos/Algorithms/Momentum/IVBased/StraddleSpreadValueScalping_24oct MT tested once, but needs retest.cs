    ...
    public class StraddleSpreadValueScalping : IZMQ
    {
        ...
        private async Task ActiveTradeIntraday(Tick tick)
        {
               ...
               // lock (lockObject)
                //{
                    ...
                    if (tick.LastTradeTime.HasValue)
                    {
                       ...
                       //decimal optionStrike = OptionUniverse[token].Strike;
                        UpdateStraddleValue(tick, OptionUniverse[tick.InstrumentToken]);


                        OptionUniverse[tick.InstrumentToken].TradeStrike = GetTradeStrike(OptionUniverse[tick.InstrumentToken].Strike);
                        //bool currentUpTrade = OptionUniverse[tick.InstrumentToken].TradeStrike > _baseInstrumentPrice;

                        BuySideLogic(OptionUniverse[tick.InstrumentToken], OptionUniverse[tick.InstrumentToken].Strike, OptionUniverse[tick.InstrumentToken].TradeStrike > _baseInstrumentPrice, tick.LastTradeTime.Value);
                        SellSideLogic(OptionUniverse[tick.InstrumentToken].Strike, OptionUniverse[tick.InstrumentToken].TradeStrike, OptionUniverse[tick.InstrumentToken].TradeStrike > _baseInstrumentPrice, tick.LastTradeTime.Value);
                        

                        //Close all Positions at 3:15
                        TriggerEODPositionClose(tick.LastTradeTime.Value);
                    }
                //}
            ...
            catch (Exception ex)
            {
                ...
                LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Error, tick.LastTradeTime.Value,
        ...
        private void SellSideLogic(decimal optionStrike, decimal tradeStrike, bool currentUpTrade, DateTime currentTime)
        {
            ...
            if (CurrentSellStraddleValue[optionStrike][(int)InstrumentType.CE] * CurrentSellStraddleValue[optionStrike][(int)InstrumentType.PE] != 0)
            {
                ...
                if (isEntryTriggered(optionStrike, tradeStrike, currentTime))
                {
                    ...
                    //await Task.WhenAll(puttask, calltask);
                    ...
                    _tradedQty = Interlocked.Add(ref _tradedQty, _stepQty);
                    ...
                    {
                        Task<Order> putbuytask = new Task<Order>(() => TradeEntry(marginPutBuyInstrument, currentTime, _buyQty, optionStrike, true));
                        Task<Order> callbuytask = new Task<Order>(() => TradeEntry(marginCallBuyInstrument, currentTime, _buyQty, optionStrike, true));

                        putbuytask.Start();
                        callbuytask.Start();

                        Task.WaitAll(putbuytask, callbuytask);
        ...
        void PlaceCloseOrder(decimal tradeStrike, int qtyTobeExited, DateTime currentTime, bool currentUpTrade)//, List<decimal> triggerStrikes)
        {
            ...
            if (puttask.IsCompleted && calltask.IsCompleted)
            {
                ...
                _tradedQty = Interlocked.Add(ref _tradedQty, -1* Convert.ToInt32(qtyTobeExited));
                //_tradedQty = _tradedQty + _stepQty;
        ...
        public async Task<bool> OnNext(Tick tick)
        {
            try
            {
                ...
                await ActiveTradeIntraday(tick);
