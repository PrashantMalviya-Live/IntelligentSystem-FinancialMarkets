            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    if (tick.LastTradeTime.HasValue)
                    {
                        ...
                        if (
                            (CurrentSellStraddleValue[optionStrike][(int)InstrumentType.CE] 
                            * CurrentSellStraddleValue[optionStrike][(int)InstrumentType.PE]
                            * CurrentBuyStraddleValue[optionStrike][(int)InstrumentType.CE]
                            * CurrentBuyStraddleValue[optionStrike][(int)InstrumentType.PE] != 0)
                            && StraddleUniverse.ContainsKey(tradeStrike)
                            )
                        {
                                ...
                                )
                            {
                                ...
                                //ReferenceStraddleValue[tradeStrike].Add(straddleValue);
                            ...
                            HistoricalStraddleAverage[tradeStrike].Enqueue(straddleValue);
                            ...
                            //    && TradedStraddleValue[tradeStrike].Count > 0
        ...
        void PlaceCloseOrder(decimal tradeStrike, DateTime currentTime, string currentUpTrade)
        {
            ...
            if (qty > 0)
            {
                Order callBuyOrder = MarketOrders.PlaceOrder(_algoInstance, call.TradingSymbol, call.InstrumentType, call.LastPrice,
                    ...
                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, Tag: currentUpTrade, broker: Constants.KOTAK);
