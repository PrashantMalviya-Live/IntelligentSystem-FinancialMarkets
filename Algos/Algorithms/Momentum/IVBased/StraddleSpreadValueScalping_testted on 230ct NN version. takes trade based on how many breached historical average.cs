...
namespace Algorithms.Algorithms
{
    public class StraddleSpreadValueScalping : IZMQ
    {
        ...
        {
            decimal tradeStrike = GetTradeStrike();
            if(tradeStrike == 0)
            {
                return;
            }
        ...
        private int QuantityToBeTraded(decimal optionStrike)
        {
            ...
            if (optionStrike == GetTradeStrike())
            {
                quantityTobeTraded += _stepQty;
            }
        ...
        //    return tradeStrike;
        //}
        private decimal GetTradeStrike()
        {
            //scan through all the option strikes within the range and see how many are breaking out and depending on that take the trade.

            decimal delta, upDelta = 0, downDelta = 0; ;
            decimal currentSellStraddleValueForOptionStrike;

            decimal upmove, downmove, averageUpMove = 0, averageDownMove = 0, counter = 0; ;
            for (decimal strike = _baseInstrumentPrice - _strikePriceRange; strike <= _baseInstrumentPrice; strike += 100)
            {
                strike = Math.Round(strike / 100, 0) * 100;
                upmove = CurrentSellStraddleValue[strike].Sum() - HistoricalStraddleAverage[strike].Value;
                if (upmove >= _openSpread)
                {
                    upDelta += 50;
                }
                counter++;
                averageUpMove += upmove;
            }
            averageUpMove /= counter;
            counter = 0;
            for (decimal strike = _baseInstrumentPrice; strike <= _baseInstrumentPrice + _strikePriceRange; strike += 100)
            {
                strike = Math.Round(strike / 100, 0) * 100;
                downmove = CurrentSellStraddleValue[strike].Sum() - HistoricalStraddleAverage[strike].Value;
                if (downmove >= _openSpread)
                {
                    downDelta -= 50;
                }
                counter++;
                averageDownMove += downmove;
            }
            averageDownMove /= counter;

            delta = averageUpMove > averageDownMove ? upDelta : downDelta;
