                    ...
                    else if (tick.LastTradeTime.HasValue)
                    {
                        ...
                        if (CurrentSpread[option.Strike][optionType][NEAR_EXPIRY] != 0 && CurrentSpread[option.Strike][optionType][FAR_EXPIRY] != 0)
                        {
                            Console.WriteLine("{5} {0}{3}, Spread:{1}, Historical:{2}, GAP:{4} ", option.Strike, 
                                Math.Round(currentSpread, 0), Math.Round(HistoricalSpread[option.Strike][optionType], 0), optionType == 1 ? "PE" : "CE", 
                                Math.Round(currentSpread, 0) - Math.Round(HistoricalSpread[option.Strike][optionType], 0), currentTime.ToString("HH:mm:ss"));
                            ...
                            if ( Math.Abs(currentSpread -  (TradedSpread[option.Strike][optionType] == 0 ? HistoricalSpread[option.Strike][optionType] : 
                                TradedSpread[option.Strike][optionType])) > 20
                                && HistoricalSpread[option.Strike][optionType] !=0
                                && _tradedQty < _maxQty - _stepQty
                                && (_activeOrders == null || _activeOrders[0].InstrumentToken == nearOption.InstrumentToken
                                || _activeOrders[1].InstrumentToken == farOption.InstrumentToken))
                            {
                                ...
                                TradedSpread[option.Strike][optionType] = currentSpread;
                            }

                            HistoricalSpread[option.Strike][optionType] = HistoricalSpread[option.Strike][optionType] == 0 ? currentSpread : (HistoricalSpread[option.Strike][optionType] * 1800 + currentSpread) / 1801;
                            CurrentSpread[option.Strike][optionType][NEAR_EXPIRY] = 0;
                            CurrentSpread[option.Strike][optionType][FAR_EXPIRY] = 0;


                            if (_tradedQty > 0 && ( Math.Abs(HistoricalSpread[option.Strike][optionType]) + 2 >= Math.Abs(currentSpread)
                                && Math.Abs(HistoricalSpread[option.Strike][optionType]) -2 <= Math.Abs(currentSpread)) && TradedSpread[option.Strike][optionType] != 0)
                            {
            ...
            try
            {
                if (SpreadUniverse == null)
                {
                    ...
                    foreach (decimal strike in SpreadUniverse.Keys)
                    {
                        ...
                        HistoricalSpread.TryAdd(strike, new decimal[] { 0, 0 });
                    }
