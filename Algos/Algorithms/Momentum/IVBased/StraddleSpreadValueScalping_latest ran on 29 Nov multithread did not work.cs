...
namespace Algorithms.Algorithms
{
    public class StraddleSpreadValueScalping : IZMQ
    {
                            ...
                            )
                        {
                            ...
                            if (
                                ...
                                && ReferenceStraddleValue[option.Strike].Count() <= 5
                                  ...
                                  && (option.Strike <= _baseInstrumentPrice + 400 && option.Strike >= _baseInstrumentPrice - 400)
                                ...
                                && !_optionStrikeCovered[option.Strike]
                                )
                            {
                                ...
                                _optionStrikeCovered[option.Strike] = true;

                                CallEntryTrades(option, tradeStrike, currentSellStraddleValue, currentTime, currentUpTrade);
        ...
        private async Task CallEntryTrades(Instrument option, decimal tradeStrike, decimal currentSellStraddleValue, DateTime currentTime, bool currentUpTrade)
        {
            await ExecuteOrderEntry(option, tradeStrike, currentSellStraddleValue, currentTime, currentUpTrade);
        }
        private async Task ExecuteOrderEntry(Instrument option, decimal tradeStrike, decimal currentSellStraddleValue, DateTime currentTime, bool currentUpTrade)
        {
            ...
            if (currentUpTrade)
            {
                ...
                //putOrderId = TradeEntry(put, currentTime, _stepQty, currentUpTrade, false);
                //callOrderStart = putOrderEnd = DateTime.Now;
                //callOrderId = TradeEntry(call, currentTime, _stepQty, currentUpTrade, false);
            ...
            Task.WaitAll(callSellOrderTask, putSellOrderTask);
            ...
            //if (callSellOrderTask.IsCompletedSuccessfully && putSellOrderTask.IsCompletedSuccessfully)
            //{
                callSellOrder = callSellOrderTask.Result;
                putSellOrder = putSellOrderTask.Result;
                ...
                _optionStrikeCovered[option.Strike] = false;
        ...
        private async Task CallOrderExit(decimal tradeStrike, DateTime currentTime, bool currentUpTrade)
        {
            await PlaceCloseOrder(tradeStrike, currentTime, currentUpTrade);
        }

        async Task PlaceCloseOrder(decimal tradeStrike, DateTime currentTime, bool currentUpTrade)
        {
            ...
            {
                //callOrderStart = DateTime.Now;
                //callOrderId = TradeEntry(call, currentTime, qty, currentUpTrade, true);
                //putOrderStart = callOrderEnd = DateTime.Now;
                //putOrderId = TradeEntry(put, currentTime, qty, currentUpTrade, true);
                //putOrderEnd = DateTime.Now;
                //putBuyOrder = GetOrderDetails(putOrderId, put, currentTime, qty, currentUpTrade, true, putOrderEnd - putOrderStart);
                //callBuyOrder = GetOrderDetails(callOrderId, call, currentTime, qty, currentUpTrade, true, callOrderEnd - callOrderStart);
        ...
        //    return optionOrder;
        //}
        private string TradeEntry(Instrument option, DateTime currentTime, int qtyInlots, bool currentUpTrade, bool buyOrder)
        {
            Task<string> orderIdTask = MarketOrders.PlaceInstantKotakOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice,
               MappedTokens[option.InstrumentToken], buyOrder, qtyInlots * Convert.ToInt32(option.LotSize),
                algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, Tag: currentUpTrade.ToString(), httpClient: _httpClient);

            orderIdTask.Wait();

            return orderIdTask.Result;
        }

        private async Task<Order> TradeEntryAsync(Instrument option, DateTime currentTime, int qtyInlots, bool currentUpTrade, bool buyOrder)
        {
        ...
        private Order GetOrderDetails (string orderId, Instrument option, DateTime currentTime, int qtyInlots, bool currentUpTrade, bool buyOrder, TimeSpan orderExecutionTime)
        {
            return MarketOrders.GetOrderDetails(_algoInstance, option.TradingSymbol, algoIndex, orderId, currentTime, option.LastPrice,
                MappedTokens[option.InstrumentToken], buyOrder, Constants.ORDER_TYPE_MARKET, qtyInlots * Convert.ToInt32(option.LotSize), Tag: orderExecutionTime.TotalMilliseconds.ToString());
        }
