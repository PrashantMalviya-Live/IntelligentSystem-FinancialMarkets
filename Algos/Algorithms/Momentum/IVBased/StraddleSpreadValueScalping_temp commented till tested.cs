...
namespace Algorithms.Algorithms
{
    public class StraddleSpreadValueScalping : IZMQ
    {
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    else if (tick.LastTradeTime.HasValue)
                    {
                        ...
                        decimal tradeStrike = _baseInstrumentPrice + ((_baseInstrumentPrice - optionStrike) > 100 ? ((_baseInstrumentPrice - optionStrike) - 100) : ((_baseInstrumentPrice - optionStrike) + 100));
                            ...
                            )
                        {
                            ...
                            if (//false &&
                                ((currentSellStraddleValue - (ReferenceStraddleValue[option.Strike].Count == 0 ? HistoricalStraddleAverage[option.Strike].Value :
                                (ReferenceStraddleValue[option.Strike].Last() + 10))) >= _openSpread)
                                && ReferenceStraddleValue[option.Strike].Count() <= 5
                                  ...
                                  && (option.Strike <= _baseInstrumentPrice + 300 && option.Strike >= _baseInstrumentPrice - 300)

                                && (HistoricalStraddleAverage[option.Strike].Value != 0)
                                && (_tradedQty < _maxQty - _stepQty)
                                && (currentTime.TimeOfDay <= new TimeSpan(14, 45, 00))
                            ...
                            {
                                //if (_upTrade != currentUpTrade)
                                //{
                                //    //CloseAllTradesExcept(tradeStrike, currentTime);
                                //}
                                //_upTrade = currentUpTrade;
                                ...
                                Order callSellOrder = MarketOrders.PlaceOrder(_algoInstance, call.TradingSymbol, call.InstrumentType, call.LastPrice,
                                   MappedTokens[call.InstrumentToken], false, _stepQty * Convert.ToInt32(call.LotSize),
                                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, product:Constants.PRODUCT_SM, Tag: currentUpTrade.ToString(), triggerPrice: call.LastPrice * 3, broker: Constants.KOTAK);

                                //Order callSellOrder = MarketOrders.PlaceOrder(_algoInstance, call.TradingSymbol, call.InstrumentType, call.LastPrice,
                                //   call.InstrumentToken, false, _stepQty * Convert.ToInt32(call.LotSize),
                                ...
                                Order putSellOrder = MarketOrders.PlaceOrder(_algoInstance, put.TradingSymbol, put.InstrumentType, put.LastPrice,
                                     MappedTokens[put.InstrumentToken], false, _stepQty * Convert.ToInt32(put.LotSize),
                                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, product: Constants.PRODUCT_SM, Tag: currentUpTrade.ToString(), triggerPrice: put.LastPrice * 3, broker: Constants.KOTAK);

                                putSellOrder.OrderTimestamp = DateTime.Now;
                                putSellOrder.ExchangeTimestamp = DateTime.Now;
                                //Order putSellOrder = MarketOrders.PlaceOrder(_algoInstance, put.TradingSymbol, put.InstrumentType, put.LastPrice,
                                //     put.InstrumentToken, false, _stepQty * Convert.ToInt32(put.LotSize),
                                //    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, Tag: currentUpTrade.ToString(), broker: Constants.ZERODHA);
                                ...
                                CallOrdersTradeStrike.TryAdd(callSellOrder, tradeStrike);
                                PutOrdersTradeStrike.TryAdd(putSellOrder, tradeStrike);
                            ...
                            {
                                PlaceCloseOrder(tradeStrike, currentTime, currentUpTrade.ToString());
                            }
                        }
        ...
        public void CloseAllTradesExcept(decimal currentTradeStrike, DateTime currentTime)
        {
            var localQty = new Dictionary<decimal, decimal[]>(TradedQuantity);
            foreach (var element in localQty)
            {
                //PlaceCloseOrder(element.Key, currentTime, 0);
                if (element.Key != currentTradeStrike)
                {
                    decimal qty = element.Value[0];
                    Instrument call = OptionUniverse.Values.First(x => x.Strike == element.Key && x.InstrumentType.Trim(' ').ToLower() == "ce");
                    Instrument put = OptionUniverse.Values.First(x => x.Strike == element.Key && x.InstrumentType.Trim(' ').ToLower() == "pe");
                    ...
                    if (qty > 0)
                    {
                        ...
                        LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, currentTime, string.Format("Closed Opposite side Straddle: CE: {0}, PE: {1}, Total: {2}",
                    ...
                    TradedStraddleValue[element.Key] = new List<decimal>();
                    TradedQuantity[element.Key][0] = 0;
                    TradedQuantity[element.Key][1] = -1;
                    CurrentBuyStraddleValue[element.Key][(int)InstrumentType.CE] = 0;
                    CurrentBuyStraddleValue[element.Key][(int)InstrumentType.PE] = 0;
            ...
            //foreach (decimal strike in strikes)
            //{
            //    if ((uptrade && strike > _baseInstrumentPrice)
            //        || (!uptrade && strike < _baseInstrumentPrice)
            //        )
            //    {
            //        ReferenceStraddleValue[strike] = new List<decimal>();
            //    }
        ...
        //public void CloseAllTrades(bool uptrade, DateTime currentTime)
        //{
        //    if (_tradedQty > 0)
        //    {
        //        var localQty = new Dictionary<decimal, decimal[]>(TradedQuantity);
        //        foreach (var element in localQty.Where(x => x.Value[0] > 0 && uptrade? x.Value[1] == 0 : x.Value[1] == 1))
        //        {
        //            PlaceCloseOrder(element.Key, currentTime, 0);
        //        }
        ...
        //}
        void PlaceCloseOrder(decimal tradeStrike, DateTime currentTime, string currentUpTrade)
        {
            decimal qty = TradedQuantity[tradeStrike][0];
            Instrument call = OptionUniverse.Values.First(x => x.Strike == tradeStrike && x.InstrumentType.Trim(' ').ToLower() == "ce");
            Instrument put = OptionUniverse.Values.First(x => x.Strike == tradeStrike && x.InstrumentType.Trim(' ').ToLower() == "pe");
            ...
            if (qty > 0)
            {
                Order callBuyOrder = MarketOrders.PlaceOrder(_algoInstance, call.TradingSymbol, call.InstrumentType, call.LastPrice,
                    ...
                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, product: Constants.PRODUCT_SM, Tag: currentUpTrade, triggerPrice: call.LastPrice * 3, broker: Constants.KOTAK);
                ...
                Order putBuyOrder = MarketOrders.PlaceOrder(_algoInstance, put.TradingSymbol, put.InstrumentType, put.LastPrice,
                   ...
                   algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, product: Constants.PRODUCT_SM, Tag: currentUpTrade, triggerPrice: put.LastPrice * 3, broker: Constants.KOTAK);
                ...
                {
                    Order cancelOrder = MarketOrders.CancelKotakOrder(_algoInstance, algoIndex, order, currentTime, product: Constants.PRODUCT_SM);
                    //CancelKotakOrder(int algoInstance, AlgoIndex algoIndex, string orderId, DateTime currentTime, string tradingSymbol)
        ...
        private void TriggerEODPositionClose(DateTime currentTime)
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 15, 00))
            {
                if (_tradedQty > 0)
                {
                    ...
                    {
                        PlaceCloseOrder(element.Key, currentTime, "Blanket");
                    }
                }
