...
namespace Algorithms.Algorithms
{
    public class StraddleSpreadValueScalping : IZMQ
    {
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    else if (tick.LastTradeTime.HasValue)
                    {
...
#if market
                        CurrentSellStraddleValue [option.Strike][optionType] = tick.Bids[0].Price;
                        CurrentBuyStraddleValue [option.Strike][optionType] = tick.Offers[0].Price;
#elif local
                        ...
                        decimal tradeStrike = _baseInstrumentPrice + (_baseInstrumentPrice - option.Strike);
                        ...
                        if ((CurrentSellStraddleValue[option.Strike][(int)InstrumentType.CE] != 0 
                            ...
                            if (
                                ...
                                && ReferenceStraddleValue[option.Strike].Count() <= 5
                                  ...
                                  && (option.Strike <= _baseInstrumentPrice + 300 && option.Strike >= _baseInstrumentPrice - 300m)
                            ...
                            //Console.WriteLine("{5} {0}{3}, Current:{1}, Historical:{2}, GAP:{4} ", option.Strike,
                            //   Math.Round(currentSellStraddleValue, 0), Math.Round(HistoricalStraddleAverage[option.Strike].Value, 0), optionType == 1 ? "PE" : "CE",
                            //   Math.Round(currentSellStraddleValue, 0) - Math.Round(HistoricalStraddleAverage[option.Strike].Value, 0), currentTime.ToString("HH:mm:ss"));


                        //}

                        //if (CurrentBuyStraddleValue[tradeStrike][(int)InstrumentType.CE] != 0 && CurrentBuyStraddleValue[tradeStrike][(int)InstrumentType.PE] != 0)
                        //{
                            decimal currentBuyStraddleValue = CurrentBuyStraddleValue[tradeStrike].Sum();

                            if (_tradedQty > 0 && ((HistoricalStraddleAverage.ContainsKey(tradeStrike) 
                                ...
                                //|| (currentBuyStraddleValue - TradedStraddleValue[option.Strike] > _stopLoss))
                                )
                            {
                                PlaceCloseOrder(tradeStrike, currentTime, option.Strike);
                            }
                        }
       ...
       void PlaceCloseOrder(decimal tradeStrike, DateTime currentTime, decimal triggerStrike)
        {
            ...
            if (triggerStrike != 0)
            {
                ...
                CurrentBuyStraddleValue[triggerStrike][(int)InstrumentType.CE] = 0;
                CurrentBuyStraddleValue[triggerStrike][(int)InstrumentType.PE] = 0;
            }
        ...
        private void LoadOptionsToTrade(DateTime currentTime)
        {
            try
            {
                if (StraddleUniverse == null)
                {
                    ...
                    StraddleUniverse = dl.LoadCloseByStraddleOptions(_expiry, _baseInstrumentToken, _baseInstrumentPrice, maxDistanceFromBInstrument:700, out allOptions);
