...
namespace Algorithms.Algorithms
{
    public class StraddleSpreadValueScalping : IZMQ
    {
        ...
        public Dictionary<decimal, decimal> TradedStraddleValue { get; set; }
            ...
            int algoInstance = 0, bool positionSizing = false, decimal maxLossPerTrade = 0)
        {
            ...
            TradedStraddleValue = new Dictionary<decimal, decimal>();
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    else if (tick.LastTradeTime.HasValue)
                    {
                        ...
                        if (CurrentSellStraddleValue[option.Strike][(int)InstrumentType.CE] != 0 && CurrentSellStraddleValue[option.Strike][(int)InstrumentType.PE] != 0 )
                        {
                            ...
                            if (
                                ((currentSellStraddleValue - (TradedStraddleValue[option.Strike] == 0 ? HistoricalStraddleAverage[option.Strike].Value :
                                (TradedStraddleValue[option.Strike]+10))) >= _openSpread)
                                ...
                                //: option.Strike >= _baseInstrumentPrice - 100 && option.Strike <= _baseInstrumentPrice + 100m)
                                  && (option.Strike <= _baseInstrumentPrice + 120 && option.Strike >= _baseInstrumentPrice - 120m)
                            ...
                            {
                                Instrument call = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.InstrumentType.Trim(' ').ToLower() == "ce");
                                Instrument put = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.InstrumentType.Trim(' ').ToLower() == "pe");
                                ...
                                TradedStraddleValue[option.Strike] = straddleValue;
                                TradedQuantity[option.Strike] += _stepQty;
                            ...
                            if (_tradedQty > 0 && ((HistoricalStraddleAverage[option.Strike].Value > currentBuyStraddleValue + 2
                                && TradedStraddleValue[option.Strike] > currentBuyStraddleValue + _closeSpread) ) //|| (currentBuyStraddleValue - TradedStraddleValue[option.Strike] > _stopLoss))
                                && TradedQuantity[option.Strike] > 0)
                            {
                                decimal qty = TradedQuantity[option.Strike];
                                Instrument call = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.InstrumentType.Trim(' ').ToLower() == "ce");
                                Instrument put = OptionUniverse.Values.First(x => x.Strike == option.Strike && x.InstrumentType.Trim(' ').ToLower() == "pe");
                                ...
                                TradedStraddleValue[option.Strike] = 0;
                                TradedQuantity[option.Strike] = 0;
        ...
        private void LoadOptionsToTrade(DateTime currentTime)
        {
            try
            {
                if (StraddleUniverse == null)
                {
                    ...
                    StraddleUniverse = dl.LoadCloseByStraddleOptions(_expiry, _baseInstrumentToken, _baseInstrumentPrice, maxDistanceFromBInstrument:500, out allOptions);
                    ...
                    foreach (decimal strike in StraddleUniverse.Keys)
                    {
                        ...
                        TradedStraddleValue.TryAdd(strike, 0);
                        TradedQuantity.TryAdd(strike, 0);
