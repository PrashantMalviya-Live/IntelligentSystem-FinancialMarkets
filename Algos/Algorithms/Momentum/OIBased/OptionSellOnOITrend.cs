...
{
    public class OptionSellOnOITrend : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(OptionSellOnOITrend source);
        [field: NonSerialized]
        ...
        private const int STOP_LOSS_POINTS = 40;//30; //40 points max loss on an average 
        private const int STOP_LOSS_POINTS_GAMMA = 40;//30;
        ...
        private ExponentialMovingAverage _peEMA, _ceEMA;
        private decimal _cumPEOI = 0, _cumCEOI = 0;
        ...
        public const AlgoIndex algoIndex = AlgoIndex.OptionSellOnOITrend;
        ...
        public OptionSellOnOITrend(TimeSpan candleTimeSpan, uint baseInstrumentToken,
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            _tradeQty = quantity;
            _totalQty = quantity*25;
            _stepQty = _totalQty / 3;
        ...
        private void SetUpInitialData(DateTime? expiry, int algoInstance = 0)
        {
            ...
            _maxDistanceFromBInstrument = 2000;
                        ...
                        if (SubscriptionTokens.Contains(token))
                        {
                           ...
                           // decimal htrend = _stockTokenHalfTrend[_baseInstrumentToken].GetTrend();
                            if (tick.InstrumentToken == _baseInstrumentToken)
                            {
                                ...
                                //if (_putorderTrios.Count > 0 && BookStopLoss(_putorderTrios, htrend == 0))
                                //{
                                //    _putProfitBookedQty = 0;
                                //    _putTriggered = false;
                                //}
                                ////if (BookStopLoss(_callorderTrios))
                                //if (_callorderTrios.Count > 0 && BookStopLoss(_callorderTrios, htrend == 1))
                                //{
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                ...
                {
                    #region Index candle (1 mins), 
                    ///1) Update OI and Calculate change in OI
                    ///2) update 20 EMA for each putside and callside
                    ///3) if one goes above and other comes down, and also overall supporting trend, then take trade
                    ///if both are up, then wait for cross over.

                    _peEMA ??= new ExponentialMovingAverage(20);
                    _ceEMA ??= new ExponentialMovingAverage(20);

                    _cumCEOI = CalculateCumOI(OptionUniverse[(int)InstrumentType.CE]) - _cumCEOI;
                    _cumPEOI = CalculateCumOI(OptionUniverse[(int)InstrumentType.PE]) - _cumPEOI;

                    _peEMA.Process(_cumPEOI, true);
                    _ceEMA.Process(_cumCEOI, true);

                    if (_peEMA.IsFormed && _ceEMA.IsFormed)
                    {
                        if (_cumCEOI > _cumPEOI)
                        {
                            if (_cumPEOI < _peEMA.GetValue<decimal>(0) && _cumCEOI > _ceEMA.GetValue<decimal>(0) && _putorderTrios.Sum(x=>x.Order.Quantity) <= _totalQty)
                            {
                                //sell CE in 3 lots
                                _putorderTrios.Add(ExecuteTrade(OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((e.ClosePrice + 50) / 100) * 100], e.CloseTime));
                            }
                        }
                        else
                        {
                            if (_cumCEOI < _ceEMA.GetValue<decimal>(0) && _cumPEOI > _peEMA.GetValue<decimal>(0) && _callorderTrios.Sum(x => x.Order.Quantity) <= _totalQty)
                            {
                                //sell PE in 3 lots
                                _callorderTrios.Add(ExecuteTrade(OptionUniverse[(int)InstrumentType.PE][Math.Floor((e.ClosePrice - 50) / 100) * 100], e.CloseTime));
                            }
                        }
                    }
        ...
        private decimal CalculateCumOI(SortedList<decimal, Instrument> optionList)
        {
            decimal cumOI = 0;
            if(optionList !=null)
            {
                foreach(Instrument o in optionList.Values)
                {
                    cumOI += o.OI;
                }
            }
        ...
        private OrderTrio ExecuteTrade(Instrument option, DateTime currentTime)
        {
            ...
            orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, "pe", option.LastPrice,
                       option.KToken, false, _stepQty, algoIndex, currentTime,
            ...
            orderTrio.TargetProfit = orderTrio.Order.AveragePrice - FIRST_TARGET;
            _lastOrderTransactionType = "sell";
            orderTrio.Order.Quantity = _stepQty;
    ...
    ref bool thirdTradeActive)
            ...
            for (int i = 0; i < orderTrios.Count; i++)
            {
                ...
                if (orderTrio.Option.InstrumentToken == tick.InstrumentToken)
                {
                    ...
                    //))
                    {
                       ...
                       // }
                       // else
                        if ((orderTrio.Order.AveragePrice > tick.LastPrice + FIRST_TARGET - 20)
                        && totalTradedQty > (_tradeQty * Convert.ToInt32(_lotSize) * 2 / 4)
                        && firstTradeActive)
                        {
        ...
        private void UpdateOptionPriceAndOI(Tick tick)
        {
        ...
        private void LoadOptionsToOI(DateTime currentTime)
        {
            try
            {
                if (OptionUniverse == null)
