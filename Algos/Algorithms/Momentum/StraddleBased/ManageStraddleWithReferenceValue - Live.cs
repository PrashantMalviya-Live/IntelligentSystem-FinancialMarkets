...
namespace Algorithms.Algorithms
{
    public class ManageStraddleWithReferenceValue : IZMQ
    {
        ...
        private void ActiveTradeIntraday(Tick tick)
        {
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    //Live Logic
                    if (_referenceStraddleValue != 0 && (currentTime - _referenceStraddleTime).TotalMinutes > 1)
                    {
                        ...
                        if (callLoaded && putLoaded)
                        {
                                ...
                                if (!_buyMode)
                                {
                                    ...
                                    if (callPriceDelta >= putPriceDelta)
                                    {
                                        if (_soloCallOrderTrio == null)
                                        {
                                            ...
                                            {
                                                _soloCallOrderTrio = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, 2 * _tradeQty, true);
                                                _straddleCallOrderTrio = null;
                                            }
                                            else
                                            {
                                                _soloCallOrderTrio = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, _tradeQty, true);
                                            }
                                        }
                                    ...
                                    else
                                    {
                                        if (_soloPutOrderTrio == null)
                                        {
                                            if (_straddlePutOrderTrio != null)
                                            {
                                                _soloPutOrderTrio = TradeEntry(_activePut, currentTime, _activePut.LastPrice, 2 * _tradeQty, true);
                                                _straddlePutOrderTrio = null;
                                            }
                                            else
                                            {
                                                _soloPutOrderTrio = TradeEntry(_activePut, currentTime, _activePut.LastPrice, _tradeQty, true);
                                            }
                                        }
                                        //if (_straddleCallOrderTrio != null)
                                        //{
                            ...
                            else
                            {
                                ...
                                if (_straddleCallOrderTrio == null)
                                {
                                    ...
                                    {
                                        _straddleCallOrderTrio = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, 2 * _tradeQty, false);
                                ...
                                //{
                                //    TradeEntry(_referenceActiveCall, currentTime, _referenceActiveCall.LastPrice, _tradeQty, false);
                                //    _soloCallOrderTrio = null;
                                //}
                                if (_straddlePutOrderTrio == null)
                                {
                                    ...
                                    {
                                        _straddlePutOrderTrio = TradeEntry(_activePut, currentTime, _activePut.LastPrice, 2 * _tradeQty, false);
                                ...
                                //{
                                //    TradeEntry(_referenceActivePut, currentTime, _referenceActivePut.LastPrice, _tradeQty, false);
                                //    _soloPutOrderTrio = null;
                                //}
                            }
                        }
                    ...
                    else if (_referenceStraddleValue == 0 && _activeCall != null && _activePut != null)
                    {
                        if (_activeCall.InstrumentToken == token)
                        {
                            _activeCall.LastPrice = tick.LastPrice;
        ...
        {
            if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(15, 25, 00) && _referenceStraddleValue != 0)
            {
