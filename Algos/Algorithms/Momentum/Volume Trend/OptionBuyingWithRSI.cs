...
{
    public class OptionBuyingWithRSI : IZMQ
    {
        ...
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(OptionBuyingWithRSI source);
        [field: NonSerialized]
        ...
        public const int RSI_MID_POINT = 50;
        ...
        public OptionBuyingWithRSI(DateTime endTime, TimeSpan candleTimeSpan, uint baseInstrumentToken, 
            DateTime? expiry, int quantity, decimal minDistanceFromBInstrument, decimal maxDistanceFromBInstrument,
            decimal rsiLowerLimitForEntry, decimal rsiUpperLimitForExit, decimal targetProfit, 
            int emaLength, int algoInstance = 0, bool positionSizing = false, decimal maxLossPerTrade = 0 )
        {
            ...
            try
            {
                ...
                lock (TimeCandles)
                {
                    ...
                    if (token == _baseInstrumentToken)
                    {
                        ...
                        {
                            _bEMA.Process(tick.LastPrice, isFinal: false);
                        }
                    }
                    else if (token != _baseInstrumentToken && tick.LastTradeTime.HasValue)
                    {
                        tokenRSI[token].Process(tick.LastPrice, isFinal: false);
                        lTokenEMA[token].Process(tokenRSI[token].GetValue<decimal>(0), isFinal: false);
                        ...
                        if (ActiveOptions.Any(x => x.InstrumentToken == token))
                        {
                            ...
                            if (orderTrio == null)
                            {
                                orderTrio = TradeEntry(token, tick.LastPrice, currentTime);
                            }
                            else
                            {
                                bool orderExited = TradeExit(token, currentTime, tick.LastPrice, orderTrio);
             ...
             decimal lastPrice, OrderTrio orderTrio)
        {
            if (orderTrio.Option != null && orderTrio.Option.InstrumentToken == token)
            {
                ...
                if (isOptionCall && option.Strike < _baseInstrumentPrice - _maxDistanceFromBInstrument
                    && CheckEntryCriteria(activeCE.Value.InstrumentToken, currentTime, out rsi))
                {
                   
                    newOrderTrio = PlaceTrailingOrder(option, activeCE.Value, lastPrice, currentTime, orderTrio, rsi);
                    newOrderTrio.EntryRSI = rsi;
                    ...
                    && CheckEntryCriteria(activePE.Value.InstrumentToken, currentTime, out rsi))
                {
                    newOrderTrio = PlaceTrailingOrder(option, activePE.Value, lastPrice, currentTime, orderTrio, rsi);
                    newOrderTrio.EntryRSI = rsi;
        ...
        private OrderTrio TradeEntry(uint token, decimal lastPrice, DateTime currentTime)
        {
            ...
            try
            {
                ...
                decimal entryRSI = 0;
                if (option != null && CheckEntryCriteria(token, currentTime, out entryRSI))
                {
                    decimal stopLoss = 0, targetProfit = 0;
                    GetCriticalLevels(lastPrice, out stopLoss, out targetProfit);
        ...
        /// <returns></returns>
        private bool CheckEntryCriteria(uint token, DateTime currentTime, out decimal entryRSI)
        {
            entryRSI = 0;
            try
            {
                ...
                if (tokenRSI[token].IsFormed && lTokenEMA[token].IsFormed
                    && rsi >= ema && rsi >= _rsiLowerLimit && ema >= _rsiLowerLimit
                    && rsi <= RSI_MID_POINT && ema <= RSI_MID_POINT)
                {
                    return true;
                }
            }
        ...
        private bool CheckExitCriteria(uint token, decimal lastPrice, DateTime currentTime, 
            OrderTrio orderTrio, out bool tpHit)
        {
            /// Exit Criteria:
            ...
            try
            {
                ...
                if ( tokenRSI[token].IsFormed && lTokenEMA[token].IsFormed 
                    && rsi < ema && rsi <= _rsiUpperLimit && ema <= _rsiUpperLimit)
                {
                    return true;
                }
            }
        ...
        private bool TradeExit(uint token, DateTime currentTime, decimal lastPrice, OrderTrio orderTrio)
        {
            try
            {
                if (orderTrio.Option != null && orderTrio.Option.InstrumentToken == token)
                {
                    ...
                    if(CheckExitCriteria(token, lastPrice, currentTime, orderTrio, out tpHit))
                    {
                        if (tpHit)
                        {
                            orderTrio.TPOrder = UpdateOrder(orderTrio.TPOrder, lastPrice, currentTime);
                            LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, currentTime,
                            string.Format("Target profit Triggered. Exited the trade @ {0}", orderTrio.TPOrder.AveragePrice), "TradeExit");
