...
namespace Algorithms.Algorithms
{
    public class FutureBuyWithRSICandle : IZMQ
    {
        ...
        public FutureBuyWithRSICandle(DateTime endTime, TimeSpan candleTimeSpan, uint baseInstrumentToken, 
            ...
            int emaLength, decimal decisiveCandleHighLow, int algoInstance = 0, bool entryatCross = false, bool positionSizing = false, decimal maxLossPerTrade = 0 )
        {
            ...
            _algoInstance = algoInstance != 0 ? algoInstance :
                ...
                Arg3: _futureBuyLevel, Arg6: _futureSellLevel, Arg7: _decisiveCandleHighLow);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_bCandle != null && _fLoaded)
                    {
                        ...
                        {
                            ExecuteTrade(currentTime, rsi, ema, _fCandle.ClosePrice, _fCandle.InstrumentToken, true);
                        }
                        else if (GetCandleFormation(_bCandle) == CandleFormation.Bearish)
                        {
                            ExecuteTrade(currentTime, rsi, ema, _fCandle.ClosePrice, _fCandle.InstrumentToken, false);
                        }
        ...
        private void ExecuteTrade(DateTime currentTime, IIndicatorValue rsi, IIndicatorValue ema, decimal lastPrice, uint token, bool longTrade)
        {
            //orderTrio.Option = option;
            if (_orderTrio == null)
            {
                _orderTrio = TradeEntry(token, lastPrice, currentTime, rsi, ema, _bEMAValue, longTrade);
            }
            else
            {
        ...
        private OrderTrio TradeEntry(uint token, decimal lastPrice, DateTime currentTime, 
            IIndicatorValue rsi, IIndicatorValue ema, IIndicatorValue bema, bool longTrade)
        {
            OrderTrio orderTrio = null;
            try
            {
                decimal entryRSI = 0;
                Trade trade = CheckEntryCriteria(token, currentTime, rsi, ema, bema, out entryRSI);
        ...
        private OrderTrio TradeExit(uint token, decimal lastPrice, DateTime currentTime,
            IIndicatorValue rsi, IIndicatorValue ema, IIndicatorValue bema, bool longTrade)
        {
            OrderTrio orderTrio = null;
            try
        ...
        private Trade CheckEntryCriteria(uint token, DateTime currentTime, IIndicatorValue rsi, IIndicatorValue ema,
                    IIndicatorValue bema, out decimal entryRSI)
        {
            Trade trade = Trade.DoNotTrade;
            entryRSI = 0;
            try
            {
                if (!tokenRSI.ContainsKey(token) || !_belowEMA.ContainsKey(token) || !rsi.IsFormed || !ema.IsFormed)
                {
