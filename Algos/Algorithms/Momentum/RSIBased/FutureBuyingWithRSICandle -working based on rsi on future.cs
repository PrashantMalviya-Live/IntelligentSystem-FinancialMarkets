...
namespace Algorithms.Algorithms
{
    public class FutureBuyWithRSICandle : IZMQ
    {
            ...
            try
            {
                ...
                if (_bCandle != null && _fLoaded)
                {
                    ...
                    if (_orderTrio == null)
                    {
                        ...
                        {
                            TradeEntry(fToken, lastPrice, _fCandle.OpenPrice, currentTime, rsi, ema, _bEMAValue, TradeZone.Open);
                        }
                    }
                    else
                    {
                        ...
                        if (TradeExit(token, _fCandle.ClosePrice, currentTime, rsi, ema, _bEMAValue, candleEnd:true))
                        {
                            _orderTrio = null;
                        }
                    }
        ...
        private bool TradeExit(uint token, decimal lastPrice, DateTime currentTime,
            IIndicatorValue rsi, IIndicatorValue ema, IIndicatorValue bema, bool candleEnd)
        {
            bool exit = false;
            try
            {
                ...
                if(CheckExitCriteria(token, lastPrice, currentTime, rsi, ema, longTrade, candleEnd, out tpHit))
                {
                    //This method should also check if there is any current position on opposite side, then include that quantity in current trade.
        ...
        private bool CheckExitCriteria(uint token, decimal lastPrice, DateTime currentTime, 
            IIndicatorValue rsi, IIndicatorValue ema, bool longTrade, bool candleEnd, out bool tpHit)
        {
            /// Exit Criteria:
            ...
            try
            {
                ...
                else
                {
                    ...
                    if((!longTrade && ((lastPrice >= _orderTrio.StopLoss) || ((rsiValue > emaValue && (candleEnd))))) || (longTrade && ((lastPrice <= _orderTrio.StopLoss) || ((rsiValue < emaValue && (candleEnd))))))
                    {
                        tpHit = false;
                        exit = true;
                    }
