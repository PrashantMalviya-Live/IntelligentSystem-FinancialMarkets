...
namespace Algorithms.Algorithms
{
    public class OptionBuyWithRSICandle : IZMQ
    {
        ...
        private decimal _decisiveCandleHighLow;
        ...
        public OptionBuyWithRSICandle(DateTime endTime, TimeSpan candleTimeSpan, uint baseInstrumentToken, 
            ...
            int emaLength, decimal decisiveCandleHighLow, int algoInstance = 0, bool entryatCross = false, 
            bool positionSizing = false, decimal maxLossPerTrade = 0)
        {
            ...
            _decisiveCandleHighLow = decisiveCandleHighLow;
            ...
            _algoInstance = algoInstance != 0 ? algoInstance :
                ...
                Arg3: _lowerLimitForCEBuy, Arg6: _upperLimitForPEBuy, Arg7: _decisiveCandleHighLow);
        ...
        private void LoadHistoricalEMAs(DateTime currentTime)
        {
            ...
            try
            {
                ...
                {
                    if (RSICandles.ContainsKey(tkn) && lTokenEMA.ContainsKey(tkn))
                    {
                        if (_firstCandleOpenPriceNeeded[tkn])
                        {
                            //The below EMA token input is from the candle that just started, All historical prices are already fed in.
                            tokenRSI[tkn].Process(RSICandles[tkn].First().OpenPrice, isFinal: true);

                            lTokenEMA[tkn].Process(tokenRSI[tkn].GetValue<decimal>(0), isFinal: true);
                            firstCandleFormed = 1;
                        }
                        //In case SQL loading took longer then candle time frame, this will be used to catch up
                        if (RSICandles[tkn].Count > 1)
                        {
                            foreach (var price in RSICandles[tkn])
                            {
                                tokenRSI[tkn].Process(RSICandles[tkn].First().ClosePrice, isFinal: true);
                                lTokenEMA[tkn].Process(tokenRSI[tkn].GetValue<decimal>(0), isFinal: true);
                            }
                        }
                    }
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (ActiveOptions[0] != null && ActiveOptions[0].InstrumentToken == token)
                    {
                        Instrument option = ActiveOptions[0];
                    ...
                    else if (ActiveOptions[1] != null && ActiveOptions[1].InstrumentToken == token)
                    {
                        Instrument option = ActiveOptions[1];
                        bool isOptionCall = option.InstrumentType.Trim(' ').ToLower() == "ce";
                        if (isOptionCall)
                        {
                            _ceCandle = e;
                    ...
                    if (_bCandle != null && _ceLoaded && _peLoaded)
                    {
                        ...
                        {
                            Trade(true, currentTime, rsi, ema, _ceCandle);
                        }
                        else if (GetCandleFormation(_bCandle) == CandleFormation.Bearish)
                        {
                            Trade(false, currentTime, rsi, ema, _peCandle);
                        }
                    ...
                    //if (ActiveOptions.All(x => x != null) && ActiveOptions.Any(x => x.InstrumentToken == token))
                    //{
                    //    Instrument option = ActiveOptions.Find(x => x.InstrumentToken == token);
                    ...
                    //    if (orderTrio == null)
                    //    {
                    //        orderTrio = TradeEntry(token, e.ClosePrice, currentTime, rsi, ema, _bEMAValue, isOptionCall, e);
                    //    }
                    //    else
                    //    {
                    //        bool orderExited = TradeExit(token, currentTime, e.ClosePrice, orderTrio, rsi, ema);
                    ...
                    //        else
                    //        {
                    //            orderTrio = TrailMarket(token, isOptionCall, currentTime, e.ClosePrice, _bEMAValue, orderTrio, e);
                    //        }
                    //    }
                    ...
                    //        , option.TradingSymbol, Decimal.Round(ema.GetValue<decimal>(), 2), Decimal.Round(rsi.GetValue<decimal>(), 2)), "CandleManger_TimeCandleFinished");
                    //}
                }
        ...
        private void Trade(bool isOptionCall, DateTime currentTime, IIndicatorValue rsi, IIndicatorValue ema, Candle e)
        {
            uint token = e.InstrumentToken;
            OrderTrio orderTrio = isOptionCall ? _callOrderTrio : _putOrderTrio;
            //orderTrio.Option = option;
            if (orderTrio == null)
            {
                orderTrio = TradeEntry(token, e.ClosePrice, currentTime, rsi, ema, _bEMAValue, isOptionCall);
            }
            else
            {
                bool orderExited = TradeExit(token, currentTime, e.ClosePrice, orderTrio, rsi, ema);
        ...
        private CandleFormation GetCandleFormation(Candle candle)
        {
            ...
            if (candleSize >= _decisiveCandleHighLow)
            {
                if (candleBody > 0 
                ...
                else if (candleBody < 0 && (candleBody >= CANDLE_DECISIVE_BODY_FRACTION * candleSize ||
