...
namespace AdvanceAlgos.Algorithms1
{
    public class FrequentBuySellWithNodemovementBothSide : IObserver<Tick[]>
    {
        ...
        private async Task ReviewStrangle(Tick[] ticks, InstrumentLinkedList optionList)
        {
             ...
             int strangleId = 0)
        {
            if (currentPE.LastPrice * currentCE.LastPrice != 0)
            {
                ...
                if (strangleId == 0)
                {
                    ...
                    //ZObjects.kite.PlaceOrder(Constants.EXCHANGE_NFO, Symbol,
