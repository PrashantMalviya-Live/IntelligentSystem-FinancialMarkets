...
namespace Algorithms.Algorithms
{
    public class ManagedStrangleDelta : IZMQ //, IObserver<Tick[]>
    {
        ...
        {
            Order order = MarketOrders.PlaceOrder(_algoInstance, "BANKNIFTY22D0841500PE", "pe",
                         0, 12, false, 25, algoIndex: _algoIndex,
                         tickTime: DateTime.Now, broker: Constants.KOTAK, HsServerId: _user.HsServerId,
                ...
                if (StartTrade(currentTime))
                {
                    ...
                    lock (tradeLock)
                    {
                        ...
                        if (tick.LastTradeTime.HasValue && tick.InstrumentToken != _baseInstrumentToken)
                        {
                            ...
                            //Take inital trade
                            if (_activeCallOrders == null && _activePutOrders == null)
                            {
        ...
        {
            if (_activeCallOrders == null || _activeCallOrders.Count == 0)
                return;
            ...
            if (_activeCallOrders[0].InstrumentToken == e.InstrumentToken)
            {
                _callLoaded = true;
            }
            if (_activePutOrders[0].InstrumentToken == e.InstrumentToken)
            {
                _putLoaded = true;
            }
            if (_putLoaded && _callLoaded)
            {
                _callLoaded = false;
                _putLoaded = false;
                int tradedQty = _activeCallOrders.Sum(x => x.Quantity * (x.TransactionType.ToLower() == "buy" ? -1 : 1));
                _livePnl = (_activeCall.LastPrice + _activePut.LastPrice) * tradedQty - _activeCallOrders.Sum(x => x.AveragePrice * x.Quantity * (x.TransactionType.ToLower() == "buy" ? -1 : 1))
                    - _activePutOrders.Sum(x => x.AveragePrice * x.Quantity * (x.TransactionType.ToLower() == "buy" ? -1 : 1));
                //_livePnl = _activeCallOrders.Sum(x => x.AveragePrice * x.Quantity * (x.TransactionType.ToLower() == "buy" ? -1 : 1))
                //    + _activePutOrders.Sum(x => x.AveragePrice * x.Quantity * (x.TransactionType.ToLower() == "buy" ? -1 : 1)) - (_activeCall.LastPrice + _activePut.LastPrice) * tradedQty;
                int netChange = 0;
                Console.WriteLine(_livePnl);
                /// COMMENTED FOR NOW
                if (_livePnl > 10 * tradedQty && tradedQty > _initialQty)
                {
                    tradedQty -= _stepQty;
                    netChange = 1;
                }
                else
                if (_livePnl < -5 * tradedQty && (tradedQty < _maxQty))
                {
                    tradedQty += _stepQty;
                    netChange = -1;
                }
                ...
                _maxDelta = Math.Min(_maxDelta, Math.Max(cdelta, pdelta * -1) + 0.1);
                //_minDelta = Math.Min(_minDelta, _maxDelta - 0.07);
                if (cdelta > _maxDelta)
                {
                    //Close everything, and restart will happen automatically
                    //CloseStrangle(e.CloseTime);
                    //ExpandStrangle(e.CloseTime, "ce");
                    MoveFurther(e.CloseTime, "ce", _stepQty * netChange);
                    netChange = 0;
                    //return;
                }
                else if (cdelta < _minDelta)
                {
                    MoveClosure(e.CloseTime, "ce", _stepQty * netChange);
                    netChange = 0;
                }
                if (pdelta * -1 > _maxDelta)
                {
                    //Close everything, and restart will happen automatically
                    //CloseStrangle(e.CloseTime);
                    //ExpandStrangle(e.CloseTime, "pe");
                    MoveFurther(e.CloseTime, "pe", _stepQty * netChange);
                    netChange = 0;
                    //return;
                }
                if (pdelta * -1 < _minDelta)
                {
                    MoveClosure(e.CloseTime, "pe", _stepQty * netChange);
                    netChange = 0;
                }
                ...
                //    double pdelta = _activePut.UpdateDelta((double)_activePut.LastPrice, 0.1, e.CloseTime, (double)_baseInstrumentPrice);
                //    _maxDelta =  Math.Min(_maxDelta, Math.Max(delta, pdelta * -1) + 0.05);
                //    if (delta > _maxDelta)
                //    {
                //        //Close everything, and restart will happen automatically
                //        //CloseStrangle(e.CloseTime);
                ...
                //    double cdelta = _activeCall.UpdateDelta((double)_activeCall.LastPrice, 0.1, e.CloseTime, (double)_baseInstrumentPrice);
                //    _maxDelta = Math.Min(_maxDelta, Math.Max(delta * -1, cdelta) + 0.05);
        ...
        private bool TakeInitialTrade(DateTime currentTime)
        {
            ...
            {
                if ((option.Value.InstrumentType.ToLower() == "ce" && option.Value.Strike >= _baseInstrumentPrice) ||
                    (option.Value.InstrumentType.ToLower() == "pe" && option.Value.Strike <= _baseInstrumentPrice))
                {
            ...
            Order order = MarketOrders.PlaceOrder(_algoInstance, _activeCall.TradingSymbol, _activeCall.InstrumentType,
                          ...
                          tickTime: currentTime, broker: Constants.KOTAK, HsServerId: _user.HsServerId, 
                          httpClient: _httpClientFactory == null ? null :KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), 
                          user: _user);
            order.InstrumentToken = _activeCall.InstrumentToken;
            _activeCallOrders ??= new List<Order>();
            ...
            order = MarketOrders.PlaceOrder(_algoInstance, _activePut.TradingSymbol, _activePut.InstrumentType,
                          ...
                          tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), 
                          user: _user);
            order.InstrumentToken = _activePut.InstrumentToken;
            _activePutOrders ??= new List<Order>();
            ...
            try
            {
                    ...
                    )
                {
                    ...
                    Dictionary<uint, Instrument> allOptions = dl.LoadOptionsChain(_expiryDate, _baseInstrumentToken, _baseInstrumentPrice,
                        1200, out mTokens, out ceList, out peList);
        ...
        {
            if (_activePutOrders != null && _activePutOrders.Count != 0)
            {
                Order order = MarketOrders.PlaceOrder(_algoInstance, _activePut.TradingSymbol, _activePut.InstrumentType,
            _activePut.LastPrice, MappedTokens[_activePut.InstrumentToken], true, _tradedQty, algoIndex: _algoIndex,
            tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
            ...
            tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
                ...
                OnTradeExit(order);
                _activeCallOrders = null;
                _activeCall = null;
            }
        }
        private void MoveFurther(DateTime currentTime, string instrumentType, int buybackQty)
        {
            Instrument call = null, put = null;
            Instrument prevCall = null, prevPut = null;
            double? prevDelta = null;
        ...
        private void BuyBackTrade(int buyBackQty, DateTime currentTime)
        {
            Order order = MarketOrders.PlaceOrder(_algoInstance, _activeCall.TradingSymbol, _activeCall.InstrumentType,
        _activeCall.LastPrice, MappedTokens[_activeCall.InstrumentToken], buyBackQty > 0, Math.Abs(buyBackQty), algoIndex: _algoIndex,
        tickTime: currentTime, broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
        ...
        private bool StartTrade(DateTime currentTime)
        {
            if (currentTime.TimeOfDay >= new TimeSpan(09, 17, 00) )//&& currentTime.TimeOfDay <= new TimeSpan(14, 40, 00))
            {
                //LoggerCore.PublishLog(_algoInstance, _algoIndex, LogLevel.Info, currentTime,
        ...
        //        Order order = MarketOrders.PlaceOrder(_algoInstance, _activeCall.TradingSymbol, _activeCall.InstrumentType,
        //                      _activeCall.LastPrice, MappedTokens[_activeCall.InstrumentToken], false, _initialQty, algoIndex: _algoIndex,
        //                      tickTime: currentTime, httpClient: _httpClientFactory == null ? null : KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
        //        order.InstrumentToken = _activeCall.InstrumentToken;
        //        _activeCallOrders.Add(order);
        ...
        //    }
        //}
        //private void ExpandStrangle(DateTime currentTime, string instrumentType, int buyBackQty)
        //{
        //    if (instrumentType.ToLower().Trim() == "pe")
        //    {
        //        Instrument put = _putUniverse.Select(x => x.Value).Where(x => x.LastPrice < _activeCall.LastPrice).FirstOrDefault();
        ...
        //      tickTime: currentTime, httpClient: KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()));
        //        OnTradeExit(order);
        ...
        private Order PlaceOrder(Instrument option, bool buyOrder, DateTime? currentTime)
        {
            ...
            Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, 
                option.InstrumentType.Trim(' '), option.LastPrice, MappedTokens[option.InstrumentToken], buyOrder, _initialQty, _algoIndex, currentTime, broker: Constants.KOTAK,
                httpClient:KoConnect.ConfigureHttpClient(_user, _httpClientFactory.CreateClient()), user: _user);
            return order;
