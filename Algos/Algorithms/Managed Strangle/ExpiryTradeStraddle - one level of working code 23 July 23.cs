    ...
    public class StraddleExpiryTrade : IZMQ
    {
                                    ...
                                    && !_stopLossHit)
                            {
                                ...
                                if ((Math.Abs(Math.Abs(_activeCall.Delta) - Math.Abs(_activePut.Delta)) < 0.15))
                                {
...
#endif
                                    _hedgeCallOrderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, 2 * _tradeQty, true, true);
                                    _hedgeCallOrderTrio.Option = _hedgeCall;

                                    _hedgePutOrderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, 2 * _tradeQty, true, true);
                                    ...
                                    _profitableRange.LowerRange = _activePut.Strike - (_straddleCallOrderTrio.Order.AveragePrice + _straddlePutOrderTrio.Order.AveragePrice)
                                        ...
                                        + (_hedgeCallOrderTrio.Order.AveragePrice + _hedgePutOrderTrio.Order.AveragePrice) * 2;
                                    ...
                                    if (atmStrike != _activeCall.Strike) 
                                    {
                                        ...
                                        if (_activePut.LastPrice < _activeCall.LastPrice)
                                        {
                                            ...
                                            if (_hedgePut.Strike != newHedgeStrike)
                                            {
                                                ...
                                                OrderTrio hedgePutOrderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, 2 * _tradeQty, false, false);
                                                ...
                                                _hedgePutOrderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, 2 * _tradeQty, true, false);
#if !BACKTEST
                                        ...
                                        else
                                        {
                                            ...
                                            if (_hedgeCall.Strike != newHedgeStrike)
                                            {
                                                ...
                                                OrderTrio hedgeCallOrderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, 2 * _tradeQty, false, false);
                                                ...
                                                _hedgeCallOrderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, 2 * _tradeQty, true, false);
#if !BACKTEST
                                                LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, e.CloseTime,
                                        ...
                                        //_totalPnL += (_straddleCallOrderTrio.Order.AveragePrice * _straddleCallOrderTrio.Order.Quantity 
                                        //    + _straddlePutOrderTrio.Order.AveragePrice * _straddlePutOrderTrio.Order.Quantity 
                                        //    - callOrderTrio.Order.AveragePrice * callOrderTrio.Order.Quantity 
                                        //    - putOrderTrio.Order.AveragePrice * putOrderTrio.Order.Quantity);
                                        ...
                                        //_totalPnL += (_soloCallOrderTrio.Order.AveragePrice * _soloCallOrderTrio.Order.Quantity 
                                        //    + _soloPutOrderTrio.Order.AveragePrice * _soloPutOrderTrio.Order.Quantity 
                                        //    - satelliteputOrderTrio.Order.AveragePrice * satelliteputOrderTrio.Order.Quantity 
                                        //    - satellitecallOrderTrio.Order.AveragePrice * satellitecallOrderTrio.Order.Quantity);
        ...
        private void LoadReferenceStraddleValue(DateTime currentTime)
        {
            if (currentTime.TimeOfDay <= new TimeSpan(09, 15, 05) && _referenceIndexValue != 0)
            {
                ...
                {
                    decimal atmStrike = Math.Round(_referenceIndexValue / 100) * 100;
        ...
        private void TriggerEODPositionClose(DateTime currentTime)
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 10, 00))// && _referenceStraddleValue != 0)
            {
                ...
                if (_hedgeCallOrderTrio != null)
                {
                    ...
                    OrderTrio orderTrio = TradeEntry(option, currentTime, option.LastPrice, 2 * _tradeQty, false, true);
        ...
        private void HedgeStraddle(DateTime? currentTime)
        {
...
#endif
                    //Hedge Call trade
                    _hedgeCallOrderTrio = TradeEntry(callHedgeOption, currentTime.Value, callHedgeOption.LastPrice, 2 * _tradeQty, true, true);
                    _hedgePutOrderTrio = TradeEntry(putHedgeOption, currentTime.Value, putHedgeOption.LastPrice, 2 * _tradeQty, true, true);
            ...
            else if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(09, 20, 00))// && _referenceStraddleValue != 0)
            {
                if (_hedgeCallOrderTrio != null && _hedgePutOrderTrio != null)
                {
                    ...
                    //Hedge Call trade
                    TradeEntry(_hedgeCallOrderTrio.Option, currentTime.Value, _hedgeCallOrderTrio.Option.LastPrice, 2 * _tradeQty, false, false);
                    TradeEntry(_hedgePutOrderTrio.Option, currentTime.Value, _hedgePutOrderTrio.Option.LastPrice, 2 * _tradeQty, false, false);
