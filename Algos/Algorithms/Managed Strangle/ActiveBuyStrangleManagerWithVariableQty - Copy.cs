...
{
    public class ActiveBuyStrangleManagerWithVariableQty : IObserver<Tick[]>
    {
        ...
        private void ReviewStrangle(StrangleNode strangleNode, Tick[] ticks)
        {
            ...
            if(strangleNode.CurrentPosition == PositionStatus.Open) 
            {
                ...
                if (pnl > strangleNode.InitialQty * (profitPoints))
                {
                    ...
                    ShortTrade trade = PlaceOrder(callOption.TradingSymbol, false, callOption.LastPrice, strangleNode.CallTradedQty);
                    strangleNode.NetPnL = dl.UpdateTrade(strangleNode.ID, callOption.InstrumentToken, trade, AlgoIndex.ActiveTradeWithVariableQty);

                    trade = PlaceOrder(putOption.TradingSymbol, false, putOption.LastPrice, strangleNode.PutTradedQty);
                ...
                else
                {
                    ...
                    //if (callOption.LastPrice > callTrades[0].AveragePrice) //Prices have moved up => Beware this will have to be checked with Theta. May be only average price should be checked with last price
                    //{
                    int qtyNeeded = Convert.ToInt32((callOption.LastPrice / putOption.LastPrice) * strangleNode.CallTradedQty - strangleNode.PutTradedQty);
                    if (Math.Abs(qtyNeeded) >= strangleNode.StepQty)
                    {
                        if (qtyNeeded > 0) //need more puts or less calls
                        {
        ...
        private ShortTrade PlaceOrder(string tradingSymbol, bool buyOrder, decimal currentPrice, int quantity, DateTime? tickTime=null, uint token=0)
        {
            ...
            //{
            //    DataLogic dl = new DataLogic();
            //    currentPrice = dl.RetrieveLastPrice(token, tickTime);
            //}
