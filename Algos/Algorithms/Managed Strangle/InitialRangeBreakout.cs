...
{
    public class InitialRangeBreakout : IZMQ, IObserver<Tick>//, IKConsumer
    {
        private int _algoInstance;
        private IDisposable unsubscriber;
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(InitialRangeBreakout source);
        [field: NonSerialized]
        ...
        decimal _intialRangehigh = 0;
        decimal _intialRangelow = 0;
        ...
        public const AlgoIndex algoIndex = AlgoIndex.InitialRangeBreakout;
        ...
        public InitialRangeBreakout(TimeSpan candleTimeSpan, uint baseInstrumentToken,
        ...
        private void CandleManger_TimeCandleFinished(object sender, Candle e)
        {
            try
            {
                //Step1: Wait till 11 AM
                //Step 2: Take trade when index breaks one side.
                //Step3: Sell on the other side and wait till EOD
                //TEMP COMMENTED 
                //if(e.InstrumentToken == atmOption.InstrumentToken)
                //decimal candlelowbody, candlehighbody;
                if (e.InstrumentToken == _baseInstrumentToken)
                {
                    if (e.CloseTime.TimeOfDay >= new TimeSpan(09, 15, 00) && e.CloseTime.TimeOfDay <= new TimeSpan(11, 00, 00))
                    {
                        _intialRangehigh = Math.Max(_intialRangehigh, e.HighPrice);
                        _intialRangelow = _intialRangelow == 0? e.LowPrice : Math.Min(_intialRangelow, e.LowPrice);
                    }
                    else
                    {
                        if(e.ClosePrice > _intialRangehigh)
                        {
                            //Sell PE
                            decimal atmStrike = GetATMStrike(_intialRangelow, "pe");
                            TakeTrade("pe", e.CloseTime, _intialRangelow);
                        }
                        else if (e.ClosePrice < _intialRangelow)
                        {
                            //sell CE
                            decimal atmStrike = GetATMStrike(_intialRangehigh, "ce");
                            TakeTrade("ce", e.CloseTime, _intialRangehigh);
                        }
                    }
        ...
        private void TakeTrade(string instrumentType, DateTime currentTime, decimal rangehighlow)
        {
            int qty = _tradeQty;
            if (_orderTrios.Count > 0)
            {
                ...
                {
#if market
                    decimal tradePrice = CloseTrade(currentTime, _orderTrios[0].Option);
#elif local
                    decimal tradePrice = CloseTrade(currentTime, AllOptions[_orderTrios[0].Option.InstrumentToken]);
#endif
                    _pnl += (_orderTrios[0].Order.AveragePrice - tradePrice);
                }
                else
                {
                    qty = 0;
            ...
            if (qty > 0)
            {
                ...
                decimal atmStrike = GetATMStrike(rangehighlow, instrumentType);
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, atmOption.TradingSymbol, atmOption.InstrumentType.ToLower(), atmOption.LastPrice, //e.ClosePrice,
                   atmOption.KToken, false, _tradeQty * Convert.ToInt32(atmOption.LotSize), algoIndex, currentTime, Tag: "Algo5",
                ...
                //orderTrio.TargetProfit = orderTrio.Order.AveragePrice + targetLevel;
        ...
        private decimal CloseTrade(DateTime currentTime, Instrument option)
        {
            int qty;
            if (_orderTrios.Count > 0)
            {
                ...
                orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice, //e.ClosePrice,
                   option.KToken, true, _tradeQty * Convert.ToInt32(option.LotSize), algoIndex, currentTime, Tag: "Algo5",
                   broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
        ...
        private decimal GetATMStrike(decimal bPrice, string instrumentType)
        {
            ...
            return instrumentType.ToLower() == "ce" ? Math.Round(bPrice / 100) * 100 : Math.Round(bPrice / 100) * 100;
        }
        ...
        private void TriggerEODPositionClose(DateTime currentTime)
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 10, 00))
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    decimal tradePrice = CloseTrade(currentTime, orderTrio.Option);
        ...
        //                    if (((AllOptions[tradedOption.InstrumentToken].LastPrice > orderTrio.TargetProfit) && tp)
        //                        || ((AllOptions[tradedOption.InstrumentToken].LastPrice < orderTrio.StopLoss) && sl))
        //                    {
            ...
            if (_orderTrios.Count > 0)
            {
                for (int i = 0; i < _orderTrios.Count; i++)
                {
                    ...
                    orderTrio.Order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType.ToLower(), option.LastPrice,
                        option.KToken, false, _tradeQty * Convert.ToInt32(atmOption.LotSize), algoIndex, currentTime, Tag: "Algo5",
                        broker: Constants.KOTAK, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
        ...
        //{
        //DataLogic dl = new DataLogic();
        //DataSet dsPAInputs = dl.LoadAlgoInputs(AlgoIndex.InitialRangeBreakout, Convert.ToDateTime("2021-11-30"), Convert.ToDateTime("2021-12-30"));
