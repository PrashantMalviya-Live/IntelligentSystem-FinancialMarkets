...
namespace Algorithms.Algorithms
{
    public class StraddleExpiryTrade : IZMQ
    {
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            _useHedge= false;
            if(DayOfWeek.Thursday == expiry.Value.DayOfWeek)
            {
                _useHedge = true;
            }
                    ...
                    if (SubscriptionTokens.Contains(token))
                    {
                        ...
                        if (_straddleCallOrderTrio == null && _straddlePutOrderTrio == null)// && _bADX.IsFormed)
                        {
                                    ...
                                    && !_stopLossHit)
                            {
                                ...
                                if(_useHedge)
                                {
                                    ...
                                    //}
#endif
                                    _hedgeCallOrderTrios.Add(TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, _tradeQty, true, true, tag:"hedge"));
                                        _hedgeCallOrderTrios[0].Option = _hedgeCall;

                                    _hedgePutOrderTrios.Add(TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, _tradeQty, true, true, tag: "hedge"));
                                        _hedgePutOrderTrios[0].Option = _hedgePut;
                        ...
                        else
                        {
                            ...
                            if (_useHedge)
                            {
                                ...
                                int hPutQty = _hedgePutOrderTrios.Sum(x => x.Order.Quantity);
                                int hCallQty = _hedgeCallOrderTrios.Sum(x => x.Order.Quantity);

                                if (_hedgePut.LastPrice * hPutQty > _hedgeCall.LastPrice * (hCallQty + _stepQty * Convert.ToInt32(_hedgeCall.LotSize)))
                                {
                                    if (hCallQty <= (_maxQty - _stepQty) * Convert.ToInt32(_hedgeCall.LotSize))
                                    {
                                        OrderTrio orderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, _stepQty, true, false, tag: "hedge");
                                        _hedgeCallOrderTrios.Add(orderTrio);
                                    }
                                    else if (hPutQty > (_initialQty * Convert.ToInt32(_hedgePut.LotSize)))
                                    {

                                        OrderTrio orderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, _stepQty, false, false, tag: "hedge");
                                        _hedgePutOrderTrios.Remove(_hedgePutOrderTrios.Last());
                                    }
                                }
                                else if(_hedgeCall.LastPrice * hCallQty > _hedgePut.LastPrice * (hPutQty + _stepQty * Convert.ToInt32(_hedgeCall.LotSize)))
                                {
                                    if (hPutQty <= (_maxQty - _stepQty) * Convert.ToInt32(_hedgePut.LotSize))
                                    {
                                        OrderTrio orderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, _stepQty, true, false, tag: "hedge");
                                        _hedgePutOrderTrios.Add(orderTrio);
                                    }
                                    else if (hCallQty > (_initialQty * Convert.ToInt32(_hedgeCall.LotSize)))
                                    {

                                        OrderTrio orderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, _stepQty, false, false, tag: "hedge");
                                        _hedgeCallOrderTrios.Remove(_hedgeCallOrderTrios.Last());
                                    }
                                }
...
//                                        }
        ...
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 15, 00))// && _referenceStraddleValue != 0)
            {
                DataLogic dl = new DataLogic();
                ...
                if (_useHedge)
                {
                    if (_hedgeCallOrderTrios != null)
                    {
                        foreach(OrderTrio ot in _hedgeCallOrderTrios)
                        {
                            Instrument option = ot.Option;
                            TradeEntry(option, currentTime, option.LastPrice, _tradeQty, false, true, tag: "hedge");
                            dl.DeActivateOrderTrio(ot);
                        }
                        _hedgeCallOrderTrios = null;
                    }
                    if (_hedgePutOrderTrios != null)
                    {
        ...
        private void HedgeStraddle(DateTime? currentTime)
        {
            if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(15, 20, 00))// && _referenceStraddleValue != 0)
            {
                ...
                if (_straddleCallOrderTrio != null && _straddlePutOrderTrio != null)
                {
                    ...
                    putHedgeOption.LastPrice = futurePrices[0].Close;
                        ...
                        //Hedge Call trade
                        TradeEntry(callHedgeOption, currentTime.Value, callHedgeOption.LastPrice, 2 * _tradeQty, true, true, tag: "hedge");
                        TradeEntry(putHedgeOption, currentTime.Value, putHedgeOption.LastPrice, 2 * _tradeQty, true, true, tag: "hedge");
            ...
            {
                if (_hedgeCallOrderTrios != null && _hedgePutOrderTrios != null)
                {
#if BACKTEST && local
                    List<Historical> futurePrices = ZObjects.kite.GetHistoricalData(_hedgeCallOrderTrios[0].Option.InstrumentToken.ToString(), currentTime.Value, currentTime.Value, "minute");
                    _hedgeCallOrderTrios[0].Option.LastPrice = futurePrices[0].Close;
                    futurePrices = ZObjects.kite.GetHistoricalData(_hedgePutOrderTrios[0].Option.InstrumentToken.ToString(), currentTime.Value, currentTime.Value, "minute");
                    _hedgePutOrderTrios[0].Option.LastPrice = futurePrices[0].Close;

#endif
                    DataLogic dl = new DataLogic();
                    dl.DeActivateOrderTrio(_hedgeCallOrderTrios[0]);
                    dl.DeActivateOrderTrio(_hedgePutOrderTrios[0]);

                    //Hedge Call trade
                    TradeEntry(_hedgeCallOrderTrios[0].Option, currentTime.Value, _hedgeCallOrderTrios[0].Option.LastPrice, 2 * _tradeQty, false, false, tag:"hedge");
                    TradeEntry(_hedgePutOrderTrios[0].Option, currentTime.Value, _hedgePutOrderTrios[0].Option.LastPrice, 2 * _tradeQty, false, false, tag: "hedge");
        ...
        private void LoadOptionsToTrade(DateTime currentTime)
        {
            try
            {
                    ...
                    )
                {
                    ...
                    if (_hedgePutOrderTrios != null && _hedgePutOrderTrios.Count > 0)
                    {
                        foreach (OrderTrio ot in _hedgePutOrderTrios)
                        {
                            if (!OptionUniverse[(int)InstrumentType.PE].ContainsKey(ot.Option.Strike))
                            {
                                OptionUniverse[(int)InstrumentType.PE].Add(ot.Option.Strike, ot.Option);
                            }
                        }
                    }
                    if (_hedgeCallOrderTrios != null && _hedgeCallOrderTrios.Count > 0)
                    {
                        foreach (OrderTrio ot in _hedgeCallOrderTrios)
                        {
                            if(!OptionUniverse[(int)InstrumentType.CE].ContainsKey(ot.Option.Strike))
                            {
