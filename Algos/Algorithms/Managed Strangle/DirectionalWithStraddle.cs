...
{
    public class DirectionalWithStraddle : IZMQ
    {
        ...
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(DirectionalWithStraddle source);
        [field: NonSerialized]
        ...
        public DirectionalWithStraddle(DateTime endTime, TimeSpan candleTimeSpan,
            ...
            if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(15, 29, 00) && _referenceStraddleValue != 0)
            {
                ...
                if(_callOrderTrio != null)
                {
                    ...
                    Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol,
                        ...
                        _callOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "buy"? false: true,
                ...
                if (_putOrderTrio != null)
                {
                    ...
                    Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol,
                        option.InstrumentType, option.LastPrice, option.InstrumentToken, 
                        _putOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "buy" ? false : true,
                        _tradeQty * Convert.ToInt32(option.LotSize), algoIndex, currentTime);
                    OnTradeEntry(order);
                }
            }
            ...
            try
            {
                ...
                else if (_referenceStraddleValue != 0)
                {
                    ...
                    if (callLoaded && putLoaded)
                    {
                        ...
                        if (currentValue > _referenceStraddleValue)
                        {
                            if (!_buyMode)
                            {
                                _buyMode = true;
                                _sellMode = false;
                                decimal callPriceDelta = _activeCall.LastPrice - _strangleCallReferencePrice;
                                decimal putPriceDelta = _activePut.LastPrice - _stranglePutReferencePrice;
                                ...
                                if (callPriceDelta >= putPriceDelta)
                                {
                                    if (_callOrderTrio == null)
                                    {
                                        _callOrderTrio = TradeEntry(_activeCall, e.CloseTime, _activeCall.LastPrice, _tradeQty, true);
                                    }
                                    else if (_callOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "sell")
                                    {
                                        _callOrderTrio = TradeEntry(_activeCall, e.CloseTime, _activeCall.LastPrice, 2 * _tradeQty, true);
                                    }
                                    if (_putOrderTrio != null)
                                    {
                                        if (_putOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "buy")
                                        {
                                            TradeEntry(_activePut, e.CloseTime, _activePut.LastPrice, _tradeQty, false);
                                        }
                                        else
                                        {
                                ...
                                else
                                {
                                    if (_putOrderTrio == null)
                                    {
                                        _putOrderTrio = TradeEntry(_activePut, e.CloseTime, _activePut.LastPrice, _tradeQty, true);
                                    }
                                    else if (_putOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "sell")
                                    {
                                        _putOrderTrio = TradeEntry(_activePut, e.CloseTime, _activePut.LastPrice, 2 * _tradeQty, true);
                                    }
                                    if (_callOrderTrio != null)
                                    {
                                        if (_callOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "buy")
                                        {
                                            TradeEntry(_activeCall, e.CloseTime, _activeCall.LastPrice, _tradeQty, false);
                                        }
                                        else
                                        {
                        ...
                        else
                        {
                            if (!_sellMode)
                            {
                                ...
                                if (_callOrderTrio == null)
                                {
                                    _callOrderTrio = TradeEntry(_activeCall, e.CloseTime, _activeCall.LastPrice, _tradeQty, false);
                                    _strangleCallReferencePrice = _activeCall.LastPrice;
                                }
                                else if (_callOrderTrio.Order.TransactionType.Trim(' ').ToLower() == "buy")
                                {
                                ...
                                if (_putOrderTrio == null)
                                {
                                    _putOrderTrio = TradeEntry(_activePut, e.CloseTime, _activePut.LastPrice, _tradeQty, false);
                                    _stranglePutReferencePrice = _activePut.LastPrice;
                                }
                                ...
                                //if (_orderTrio1 == null || _orderTrio2 == null)
                                //{
                                //    if (_orderTrio1 != null)
                                //    {
                                //        _orderTrio1 = TradeEntry(o1.InstrumentToken, e.CloseTime, o1.LastPrice, 2 * _tradeQty, false);
                                //        _orderTrio2 = TradeEntry(o2.InstrumentToken, e.CloseTime, o2.LastPrice, _tradeQty, false);
                                //    }
                                //    else if (_orderTrio2 != null)
                                //    {
                                //        _orderTrio1 = TradeEntry(o1.InstrumentToken, e.CloseTime, o1.LastPrice, _tradeQty, false);
                                //        _orderTrio2 = TradeEntry(o2.InstrumentToken, e.CloseTime, o2.LastPrice, 2 * _tradeQty, false);
                                //    }
                                //    else
                                //    {
                                //        _orderTrio1 = TradeEntry(o1.InstrumentToken, e.CloseTime, o1.LastPrice, _tradeQty, false);
        ...
        private OrderTrio TradeEntry(Instrument option, DateTime currentTime, decimal lastPrice, int tradeQty, bool buyOrder)
        {
            ...
            try
            {
                ...
                LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, order.OrderTimestamp.Value,
                   string.Format("TRADE!! {3} {0} lots of {1} @ {2}", _tradeQty,
