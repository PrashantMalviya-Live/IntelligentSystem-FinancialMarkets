...
{
    public class FrequentBuySellWithNodemovementBothSideContinuous : IZMQ// IObserver<Tick[]>
    {
        ...
        private void ReviewStrangle(Tick[] ticks)
        {
            ...
            //{
            //    ce.LastPrice = dl.RetrieveLastPrice(ce.InstrumentToken, ticks[0].LastTradeTime);
            //    pe.LastPrice = dl.RetrieveLastPrice(pe.InstrumentToken, ticks[0].LastTradeTime);
            //}
            //-----------END -------------
            ...
            if (ce.LastPrice != 0 && pe.LastPrice != 0)
            {
                if (ceNode.CurrentPosition == PositionStatus.Open && peNode.CurrentPosition == PositionStatus.Open)
                {
                    if (profitBooked)
                    {
                        ...
                        if (totalProfit + ceDelta + peDelta > TargetProfit)
                        {
                            profitBooked = false;
