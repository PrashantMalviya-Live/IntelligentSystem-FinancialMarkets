    ...
    public class ActiveBuyStrangleManagerWithVariableQty : IZMQ// IObserver<Tick[]>
    {
        ...
        private void ReviewStrangle(Tick tick)
        {
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    {
                        if ((_activeStrangle.CallOrders.Count == 0 && _activeStrangle.PutOrders.Count == 0) || secondRound)
                        {
                            secondRound = false;
                            ...
                            if (callOption.LastPrice > putOption.LastPrice)
                            {
                                ...
                                _activeStrangle.PutTradedQty = TradeEntry(putOption, _activeStrangle.PutOrders, qty, currentTime);
                            ...
                            else
                            {
                                ...
                                _activeStrangle.CallTradedQty = TradeEntry(callOption, _activeStrangle.CallOrders, qty, currentTime);
                        ...
                        if (pnl > 2500 || pnl < -5000)
                        {
                            ...
                            Order order = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", _activeStrangle.Call.LastPrice,
                                callOption.KToken, false, _activeStrangle.CallTradedQty, algoIndex, currentTime, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                            ...
                            order = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", _activeStrangle.Put.LastPrice,
                                putOption.KToken, false, _activeStrangle.PutTradedQty, algoIndex, currentTime, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                            OnTradeExit(order);
                            //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                                ...
                                if (PutBuyCallSellQty > 0)
                                {
                                    if (_activeStrangle.CallTradedQty > _activeStrangle.InitialQty)
                                    {
                                        ...
                                        Order order = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", callOption.LastPrice, callOption.KToken,
                                            false, _activeStrangle.StepQty, algoIndex, currentTime, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeExit(order);
                                    ...
                                    if (_activeStrangle.PutTradedQty < _activeStrangle.MaxQty)
                                    {
                                        ...
                                        Order order = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", putOption.LastPrice, putOption.KToken, true, 
                                            _activeStrangle.StepQty, algoIndex, currentTime, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeEntry(order);
                                        //increase trade record and quantity
                                        putOrders.Add(order);
        ...
        private int TradeEntry(Instrument option, List<Order> orders, int quantity, DateTime? currentTime)
        {
            ...
            Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice, 
                option.KToken, true, quantity, algoIndex, currentTime, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
