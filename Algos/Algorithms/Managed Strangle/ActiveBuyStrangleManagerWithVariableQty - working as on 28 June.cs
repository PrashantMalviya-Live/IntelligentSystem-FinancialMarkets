    ...
    public class ActiveBuyStrangleManagerWithVariableQty : IZMQ// IObserver<Tick[]>
    {
        ...
        private void ReviewStrangle(Tick tick)
        {
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    _activeStrangle ??= GetActiveStrangle();
                    if(_activeStrangle == null)
                    {
                        return;
                    }
                    ...
                    if (_activeStrangle.CurrentPosition == PositionStatus.Open)
                    {
                        ...
                        if ((_activeStrangle.CallTradedQty == 0 && _activeStrangle.PutTradedQty == 0) || nextRound)
                        {
                            ...
                            {
                                if ((callOption.LastPrice > putOption.LastPrice && callOption.LastPrice / putOption.LastPrice > 1.1m)
                                    || (putOption.LastPrice > callOption.LastPrice && putOption.LastPrice / callOption.LastPrice > 1.1m))
                                {
                                    return;
                                }
                            }
                            ...
                            {
                                int qty = Convert.ToInt32(Math.Round((callOption.LastPrice * _activeStrangle.InitialQty) / putOption.LastPrice / 25, 0) * 25);
                            ...
                            else
                            {
                                ...
                                int qty = Convert.ToInt32(Math.Round((putOption.LastPrice * _activeStrangle.InitialQty) / callOption.LastPrice / 25, 0) * 25);
                        ...
                        //if (pnl > _activeStrangle.InitialQty* multiplier * (_profitPoints * 0 + 10) || pnl < _activeStrangle.InitialQty*multiplier * (_profitPoints * 0 - 20))
                        if (pnl > 500 || pnl < -10000)
                        {
                            ...
                            int callTradeQty = _activeStrangle.CallTradedQty;
                            int putTradeQty = _activeStrangle.PutTradedQty;

                            //if (_baseInstrumentPrice < _activeStrangle.Call.Strike && _baseInstrumentPrice > _activeStrangle.Put.Strike)
                            if (callOption.Strike < _baseInstrumentPrice + 200 && callOption.Strike > _baseInstrumentPrice - 200)
                            {
                                if ((callOption.LastPrice > putOption.LastPrice && callOption.LastPrice / putOption.LastPrice < 1.05m)
                                                                 || (putOption.LastPrice > callOption.LastPrice && putOption.LastPrice / callOption.LastPrice < 1.05m))

                                {
                                    if (callOption.LastPrice > putOption.LastPrice)
                                    {
                                        ...
                                        callTradeQty = _activeStrangle.CallTradedQty - _activeStrangle.InitialQty;
                                        putTradeQty = _activeStrangle.PutTradedQty - qty;
                                    ...
                                    else
                                    {
                                        ...
                                        callTradeQty = _activeStrangle.CallTradedQty - qty;
                                        putTradeQty = _activeStrangle.PutTradedQty - _activeStrangle.InitialQty;
                            ...
                            if (callTradeQty > 0)
                            {
                                Order corder = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", _activeStrangle.Call.LastPrice,
                                     callOption.KToken, false, callTradeQty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                            ...
                            else if (callTradeQty < 0)
                            {
                                Order corder = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", _activeStrangle.Call.LastPrice,
                                     callOption.KToken, true, callTradeQty * -1, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                            ...
                            {
                                _activeStrangle.NetPnL += _activeStrangle.Call.LastPrice * _activeStrangle.CallTradedQty;
                            }
                            ...
                            if (putTradeQty > 0)
                            {
                                ...
                                Order porder = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", _activeStrangle.Put.LastPrice,
                                    putOption.KToken, false, putTradeQty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                OnTradeExit(porder);
                                //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                            ...
                            else if (putTradeQty < 0)
                            {
                                //trade = PlaceOrder(putOption.TradingSymbol, false, _activeStrangle.Put.Bids[1].Price, _activeStrangle.PutTradedQty);
                                //_activeStrangle.NetPnL = dl.UpdateTrade(_activeStrangle.ID, putOption.InstrumentToken, trade, AlgoIndex.ActiveTradeWithVariableQty);

                                Order porder = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", _activeStrangle.Put.LastPrice,
                                    putOption.KToken, true, putTradeQty * -1, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                OnTradeExit(porder);
                                //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                            ...
                            {
                                _activeStrangle.NetPnL += _activeStrangle.Put.LastPrice * _activeStrangle.PutTradedQty;
                            }

                            //if (_baseInstrumentPrice < _activeStrangle.Call.Strike && _baseInstrumentPrice > _activeStrangle.Put.Strike)
                            //{
                            if ((callOption.Strike < _baseInstrumentPrice + 200 && callOption.Strike > _baseInstrumentPrice - 200)
                                && ((callOption.LastPrice > putOption.LastPrice && callOption.LastPrice / putOption.LastPrice < 1.05m)
                                                             || (putOption.LastPrice > callOption.LastPrice && putOption.LastPrice / callOption.LastPrice < 1.05m)))
                            {
                                //Since this will be buy for the new strangle
                                _activeStrangle.NetPnL = _activeStrangle.NetPnL * -1;

                                //_activeStrangle.CallOrders = new List<Order>();
                                //_activeStrangle.CallOrders.Add(corder);

                                //_activeStrangle.PutOrders = new List<Order>();
                            ...
                            else
                            {
                                ...
                                multiplier = 1;
                                nextRound = false;
                                //individualLegTradeCounter = 0;
                                callLegTradeCounter = 0;
                                putLegTradeCounter = 0;
                            }
                            
                        }
                        else if (pnl < -2000 * multiplier)
                        {
                            if (callOption.LastPrice > putOption.LastPrice)
                            {
                                int qty = Convert.ToInt32(Math.Round((callOption.LastPrice * _activeStrangle.InitialQty) / putOption.LastPrice / 25, 0) * 25);
                                if (qty + _activeStrangle.PutTradedQty < _activeStrangle.MaxQty)
                                {
                                    nextRound = true;
                                    multiplier++;
                                }
                            ...
                            {
                                int qty = Convert.ToInt32(Math.Round((putOption.LastPrice * _activeStrangle.InitialQty) / callOption.LastPrice / 25, 0) * 25);
                                if (qty + _activeStrangle.CallTradedQty < _activeStrangle.MaxQty)
                                {
                                    nextRound = true;
                                    multiplier++;
                                }
                        ...
                        else if (multiplier > 1 && pnl > -2000 * (multiplier - 2))
                        {
                            multiplier--;
                            ...
                            if (callOption.LastPrice > putOption.LastPrice)
                            {
                                callQty = _activeStrangle.InitialQty;
                                putQty = Convert.ToInt32(Math.Round((callOption.LastPrice * _activeStrangle.InitialQty) / putOption.LastPrice / 25, 0) * 25);
                            }
                            else
                            {
                                putQty = _activeStrangle.InitialQty;
                                callQty = Convert.ToInt32(Math.Round((putOption.LastPrice * _activeStrangle.InitialQty) / callOption.LastPrice / 25, 0) * 25);
                            }

                            Order order = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", _activeStrangle.Call.LastPrice,
                                callOption.KToken, false, callQty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                            //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                            _activeStrangle.NetPnL += order.AveragePrice * callQty;

                            //PostOrderInKotak(callOption, currentTime, callOption.Strike, _activeStrangle.CallTradedQty, true, false);
                            callOrders.Add(order);

                            //_activeStrangle.CallTradedQty = callOrders.Sum(x => (x.TransactionType.ToLower() == "buy" ? 1 : -1) * x.Quantity);
                            _activeStrangle.CallTradedQty -= callQty;
                            ...
                            order = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", _activeStrangle.Put.LastPrice,
                                putOption.KToken, false, putQty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                            OnTradeExit(order);
                            //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);

                            //PostOrderInKotak(putOption, currentTime, putOption.Strike, _activeStrangle.PutTradedQty, true, false);

                            _activeStrangle.NetPnL += order.AveragePrice * putQty;
                            putOrders.Add(order);
                            //_activeStrangle.PutTradedQty = putOrders.Sum(x => (x.TransactionType.ToLower() == "buy" ? 1 : -1) * x.Quantity);
                            _activeStrangle.PutTradedQty -= putQty;
                        ...
                        else
                        {
                            ...
                            if (Math.Abs(PutBuyCallSellQty) >= _activeStrangle.StepQty) //need more puts or less calls
                            {
                                ...
                                {
                                    if (_activeStrangle.CallTradedQty > _activeStrangle.InitialQty && callLegTradeCounter <= _individualLegMaxTrades)
                                    {
                                        callLegTradeCounter--;
                                        ...
                                        int qty = Math.Min(PutBuyCallSellQty, _activeStrangle.CallTradedQty - _activeStrangle.InitialQty);
                                        qty = Convert.ToInt32(Math.Min(Math.Round(Convert.ToDecimal(qty / _activeStrangle.Call.LotSize), 0) * _activeStrangle.Call.LotSize, _activeStrangle.StepQty));


                                        Order order = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", callOption.LastPrice, callOption.KToken,
                                            false, qty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeExit(order);
                                        ...
                                        _activeStrangle.NetPnL += order.AveragePrice * qty; // _activeStrangle.StepQty;  //dl.UpdateOrder(_activeStrangle.ID, order);
                                    }
                                    else
                                    if (_activeStrangle.PutTradedQty < _activeStrangle.MaxQty && putLegTradeCounter <= _individualLegMaxTrades)
                                    {
                                        putLegTradeCounter++;
                                        int qty = Math.Min(PutBuyCallSellQty, _activeStrangle.MaxQty - _activeStrangle.PutTradedQty);
                                        qty = Convert.ToInt32(Math.Min(Math.Round(Convert.ToDecimal(qty / _activeStrangle.Put.LotSize), 0) * _activeStrangle.Put.LotSize, _activeStrangle.StepQty));

                                        //place order to buy Puts
                                        Order order = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", putOption.LastPrice, putOption.KToken, true,
                                            qty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeEntry(order);
                                        //increase trade record and quantity
                                        putOrders.Add(order);
                                        //_activeStrangle.PutTradedQty = putOrders.Sum(x => (x.TransactionType.ToLower() == "buy" ? 1 : -1) * x.Quantity);
                                        ...
                                        //PostOrderInKotak(putOption, currentTime, putOption.Strike, _activeStrangle.StepQty, true, true);
                                        ...
                                        _activeStrangle.NetPnL += order.AveragePrice * qty * -1;  //dl.UpdateOrder(_activeStrangle.ID, order);
                            ...
                            if (Math.Abs(PutSellCallBuyQty) >= _activeStrangle.StepQty) //need more calls or less puts
                            {
                                ...
                                {
                                    if (_activeStrangle.PutTradedQty > _activeStrangle.InitialQty && putLegTradeCounter <= _individualLegMaxTrades)
                                    {
                                        putLegTradeCounter--;
                                        int qty = Math.Min(Math.Abs(PutSellCallBuyQty), _activeStrangle.PutTradedQty - _activeStrangle.InitialQty);
                                        qty = Convert.ToInt32(Math.Min(Math.Round(Convert.ToDecimal(Math.Abs(qty) / _activeStrangle.Put.LotSize), 0) * _activeStrangle.Put.LotSize, _activeStrangle.StepQty));
                                        ...
                                        //place order to buy Puts
                                        Order order = MarketOrders.PlaceOrder(_algoInstance, putOption.TradingSymbol, "pe", putOption.LastPrice, putOption.KToken,
                                            false, qty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeExit(order);
                                        //increase trade record and quantity
                                        putOrders.Add(order);
                                        ...
                                        //_activeStrangle.NetPnL = dl.UpdateOrder(_activeStrangle.ID, order);
                                        _activeStrangle.NetPnL += order.AveragePrice * qty;  //dl.UpdateOrder(_activeStrangle.ID, order);



                                    }
                                    else if (_activeStrangle.CallTradedQty < _activeStrangle.MaxQty && callLegTradeCounter < _individualLegMaxTrades)
                                    {
                                        callLegTradeCounter++;
                                        int qty = Math.Min(Math.Abs(PutSellCallBuyQty), _activeStrangle.MaxQty - _activeStrangle.CallTradedQty);
                                        qty = Convert.ToInt32(Math.Min(Math.Round(Convert.ToDecimal(Math.Abs(qty) / _activeStrangle.Call.LotSize), 0) * _activeStrangle.Call.LotSize, _activeStrangle.StepQty));
                                        ...
                                        //place order to sell calls
                                        //ShortTrade trade = PlaceOrder(callOption.TradingSymbol, false, callOption.Bids[1].Price, _activeStrangle.StepQty);
                                        Order order = MarketOrders.PlaceOrder(_algoInstance, callOption.TradingSymbol, "ce", callOption.LastPrice, callOption.KToken, true,
                                            qty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                                        OnTradeEntry(order);
                                        //increase trade record and quantity
                                        callOrders.Add(order);
                                        ...
                                        //PostOrderInKotak(callOption, currentTime, callOption.Strike, _activeStrangle.StepQty, true, true);
                                        //_activeStrangle.CallOrders = callOrders; //This may not be required. Just check
        ...
        private void TriggerEODPositionClose(DateTime currentTime)
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 10, 00))
            {
                ...
                Order order = MarketOrders.PlaceOrder(_algoInstance, _activeStrangle.Call.TradingSymbol, "ce", _activeStrangle.Call.LastPrice,
                    _activeStrangle.Call.KToken, false, _activeStrangle.CallTradedQty, algoIndex, currentTime, broker: Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
                ...
                order = MarketOrders.PlaceOrder(_algoInstance, _activeStrangle.Put.TradingSymbol, "pe", _activeStrangle.Put.LastPrice,
                    _activeStrangle.Put.KToken, false, _activeStrangle.PutTradedQty, algoIndex, currentTime, broker: Constants.ZERODHA,  httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                OnTradeExit(order);
                //_activeStrangle.NetPnL = dl.UpdateOrder(_algoInstance, order);
        ...
        private int TradeEntry(Instrument option, List<Order> orders, int quantity, DateTime? currentTime)
        {
            ...
            Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, option.LastPrice, 
                option.KToken, true, quantity, algoIndex, currentTime, broker:Constants.ZERODHA, httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
        ...
        private StrangleNode GetActiveStrangle()
        {
            foreach (var strike in _calls.Keys)
            {
                if (strike < _baseInstrumentPrice + 200 && strike > _baseInstrumentPrice - 200)
                {
                    ...
                    if ((callOption.LastPrice * putOption.LastPrice != 0) && ((callOption.LastPrice > putOption.LastPrice && callOption.LastPrice / putOption.LastPrice < 1.05m)
                                                     || (putOption.LastPrice > callOption.LastPrice && putOption.LastPrice / callOption.LastPrice < 1.05m)))
                    {
