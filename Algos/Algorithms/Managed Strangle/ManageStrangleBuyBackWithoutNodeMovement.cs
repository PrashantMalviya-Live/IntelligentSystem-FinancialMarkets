...
{
    public class ManageStrangleBuyBackWithoutNodeMovement : IObserver<Tick[]>
    { 
        public ManageStrangleBuyBackWithoutNodeMovement()
        {
            LoadActiveData();
        }
        ...
        private void ReviewStrangle(InstrumentLinkedList optionList, Tick[] ticks)
        {
            ...
            if (strangleNode.Prices.Count > 0)
            {
                ...
                if (strangleNode.CurrentPosition == PositionStatus.Closed &&
                    CheckForReSale(strangleNode, Math.Max(Math.Abs(strangleNode.Prices.First()) * 0.3m, optionList.MaxLossPoints) / 2))
                {
                    ...
                    decimal lastOptionPrice = strangleNode.Prices.Last();  //optionNode.Prices.Skip(optionNode.Prices.Count - 2).Sum();

                    uint token = option.InstrumentToken;
                    option = strangleNode.Instrument;
                    //optionList.MaxLossPoints = Math.Max(strangleNode.Prices.Last() * 1.1m, 5);

                        DataLogic dl = new DataLogic();
                    dl.UpdateListData(optionList.listID, option.InstrumentToken, strangleNode.Index, 0, option.InstrumentType,
                       0, lastOptionPrice, 0, AlgoIndex.BBStrangleWithoutMovement);
                }
                //as there is no movement. Take full profit..no need to cut profit.
                else if (strangleNode.CurrentPosition == PositionStatus.Open && (//(option.LastPrice < strangleNode.Prices.Last() - optionList.MaxProfitPoints) ||
                    (option.LastPrice > strangleNode.Prices.First() + Math.Max(strangleNode.Prices.First() * 0.3m, optionList.MaxLossPoints) )))//+ optionList.MaxLossPoints))) if option looses 10% of its value buy back
                {
        ...
        private void LoadActiveData()
        {
            ...
            //    new InstrumentLinkedList()
            //};
            AlgoIndex algoIndex = AlgoIndex.BBStrangleWithoutMovement;
            DataLogic dl = new DataLogic();
