...
namespace Algorithms.Algorithms
{
    public class MultiStraddleDirectionalShift : IZMQ
    {
        ...
        private decimal _entryRatio = 1.1m;//1.2//1.45
                ...
                if (call.LastPrice * put.LastPrice != 0 && _callOptionLoaded[call.Strike] && _putOptionLoaded[put.Strike])
                {
                        ...
                        && ((call.LastPrice + put.LastPrice) - (_callOrderTrios[call.Strike].Order.AveragePrice + _putOrderTrios[put.Strike].Order.AveragePrice) > 50)))
                    {
                        ...
                        if (call.LastPrice > put.LastPrice && call.IsTraded)
                        {
                            ...
                            ////Do it for all the call trades
                            //foreach (var orderTrio in _callOrderTrios.Values)
                            //{
                            //    if (!_putOrderTrios.ContainsKey(orderTrio.Option.Strike))
                            //    {
                            //        //First Cancel the earlier SLM and the buy
                            //        CancelOrder(currentTime, orderTrio.SLOrder);

                            //        //buy back call
                            //        TradeEntry(orderTrio.Option, currentTime, orderTrio.Option.LastPrice, _tradeQty, true, null);
                            //    }
                            //}
                        }
                        if (put.LastPrice > call.LastPrice && put.IsTraded)
                        {
                            ...
                            ////do it for all the put trades 
                            //foreach (var orderTrio in _putOrderTrios.Values)
                            //{
                            //    if (!_callOrderTrios.ContainsKey(orderTrio.Option.Strike))
                            //    {
                            //        //First Cancel the earlier SLM and the buy
                            //        CancelOrder(currentTime, orderTrio.SLOrder);
