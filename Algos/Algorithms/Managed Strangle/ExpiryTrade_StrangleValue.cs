...
namespace Algos.TLogics
{
    ...
    public class ExpiryTrade : IZMQ //ITibcoListener , IMessageListener<Tick> , ICacheEntryEventListener<TickKey, Tick>,ITibcoListener//, IKafkaConsumer//, IObserver<Tick[]>
    {
        ...
        private const decimal OPTION_PREMIUM_MINIMUM_THRESHOLD = 8;
        ...
        private void ManageStrangle(StrangleDataStructure strangleNode, Tick[] ticks)
        {
            ...
            else if (strangleNode.BaseInstrumentPrice == 0)
            {
                return;
            }
            ...
            DateTime timeOfOrder = ticks[0].LastTradeTime.HasValue? ticks[0].LastTradeTime.Value: ticks[0].Timestamp.Value;
            ...
            ContinueTrade = ContinueTrade ? ContinueTrade : StopTrade(ref optionMatix, strangleNode, timeOfOrder, Convert.ToInt32(qtyAvailable));
            if (ContinueTrade)
            {
                return;
            }
        ...
        {
            bool stopTrade = true;
            decimal minPrice = OPTION_PREMIUM_MINIMUM_THRESHOLD;
