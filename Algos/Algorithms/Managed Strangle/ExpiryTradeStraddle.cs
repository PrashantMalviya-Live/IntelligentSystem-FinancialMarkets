...
namespace Algorithms.Algorithms
{
    public class StraddleExpiryTrade : IZMQ
    {
        ...
        public StraddleExpiryTrade(TimeSpan candleTimeSpan,
            ...
            decimal targetProfit, decimal stopLoss, bool intraday, decimal strikePriceIncrement, bool nearHedges, decimal thresholdRatio = 1.67m,
            ...
            decimal maxLossPerTrade = 0, IHttpClientFactory httpClientFactory = null)
        {
            ...
            _strikePriceIncrement = Constants.GetStrikePriceIncrement(baseInstrumentToken);
                                ...
                                if ((Math.Abs(Math.Abs(_activeCall.Delta) - Math.Abs(_activePut.Delta)) < 0.15))
                                {
                                    if (_useHedge)
                                    {
                                        if (_nearHedges)
                                        {
                                            ...
                                            _hedgeCall = OptionUniverse[(int)InstrumentType.CE][Math.Floor((_baseInstrumentPrice + _activeCall.LastPrice + _activePut.LastPrice - _strikePriceIncrement) / _strikePriceIncrement) * _strikePriceIncrement];
                                            _hedgePut = OptionUniverse[(int)InstrumentType.PE][Math.Ceiling((_baseInstrumentPrice - _activeCall.LastPrice - _activePut.LastPrice + _strikePriceIncrement) / _strikePriceIncrement) * _strikePriceIncrement];
                                        }
                                        else
                                        {
                                            //First buy hedges and then sell straddle
                                            _hedgeCall = OptionUniverse[(int)InstrumentType.CE][Math.Ceiling((_baseInstrumentPrice + _activeCall.LastPrice + _activePut.LastPrice + 2 * _strikePriceIncrement) / _strikePriceIncrement) * _strikePriceIncrement];
                                            _hedgePut = OptionUniverse[(int)InstrumentType.PE][Math.Floor((_baseInstrumentPrice - _activeCall.LastPrice - _activePut.LastPrice - 2 * _strikePriceIncrement) / _strikePriceIncrement) * _strikePriceIncrement];
                                        }
                        ...
                        if (_straddleShift)
                        {
                            if (_activeCall.LastPrice > _activePut.LastPrice * _thresholdRatio || _activePut.LastPrice > _activeCall.LastPrice * _thresholdRatio)
                            {
                                ...
                                //&& (activeCE.LastPrice < activePE.LastPrice * _thresholdRatio && activePE.LastPrice < activeCE.LastPrice * _thresholdRatio)) 
                                {
                                    ...
                                    if (_useHedge)
                                    {
                                        if (_activePut.LastPrice < _activeCall.LastPrice)
                                        {
                                            ...
                                            if (_hedgePut.Strike != newHedgeStrike)
                                            {
                                                ...
                                                int hedgeQty = _hedgePutOrderTrios.Sum(x => x.Order.Quantity) / Convert.ToInt32(newHedgePut.LotSize);

                                                OrderTrio newhedgePutOrderTrio = TradeEntry(newHedgePut, currentTime, newHedgePut.LastPrice, hedgeQty, true, false, tag: "hedge");

                                                foreach (OrderTrio ott in _hedgePutOrderTrios)
                                                {
                                                    //market has moved up, move the put hedge up
                                                    OrderTrio hedgePutOrderTrio = TradeEntry(_hedgePut, currentTime, _hedgePut.LastPrice, ott.Order.Quantity / Convert.ToInt32(_hedgePut.LotSize), false, false, tag: "hedge");
                                                    ...
                                                    dl.DeActivateOrderTrio(ott);
                                                }
                                                _hedgePutOrderTrios.Clear();
                                                _hedgePut = newHedgePut;
                                                _hedgePutOrderTrios.Add(newhedgePutOrderTrio);
#if !BACKTEST
                                                    LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, e.CloseTime,
                                                     String.Format("Put Hedge shifted from {0} to {1}. ", newhedgePutOrderTrio.Option.Strike, _hedgePut.Strike
                                        ...
                                        else
                                        {
                                            ...
                                            if (_hedgeCall.Strike != newHedgeStrike)
                                            {
...
#endif
                                                int hedgeQty = _hedgeCallOrderTrios.Sum(x => x.Order.Quantity) / Convert.ToInt32(newHedgeCall.LotSize);
                                                OrderTrio newHedgeCallOrderTrio = TradeEntry(newHedgeCall, currentTime, newHedgeCall.LastPrice, hedgeQty, true, false, tag: "hedge");

                                                foreach (OrderTrio ott in _hedgeCallOrderTrios)
                                                {
                                                    //market has moved down, move the call hedge down
                                                    OrderTrio hedgeCallOrderTrio = TradeEntry(_hedgeCall, currentTime, _hedgeCall.LastPrice, ott.Order.Quantity / Convert.ToInt32(_hedgeCall.LotSize), false, false, tag: "hedge");
                                                ...
                                                _hedgeCallOrderTrios.Add(newHedgeCallOrderTrio);
#if !BACKTEST
                                                    LoggerCore.PublishLog(_algoInstance, algoIndex, LogLevel.Info, e.CloseTime,
                                                        String.Format("Call Hedge shifted from {0} to {1}. ", newHedgeCallOrderTrio.Option.Strike, _hedgeCall.Strike), "CandleManger_TimeCandleFinished");
#endif
                                            }
                                    ...
                                    int straddleCallQty = _straddleCallOrderTrios.Sum(x => x.Order.Quantity) / Convert.ToInt32(_activeCall.LotSize);
                                    foreach (OrderTrio ott in _straddleCallOrderTrios)
                                    {
                                        OrderTrio callOrderTrio = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, ott.Order.Quantity / Convert.ToInt32(_activeCall.LotSize), true, false, tag: "straddle");
                                        dl.DeActivateOrderTrio(ott);
                                    }

                                    int straddlePutQty = _straddlePutOrderTrios.Sum(x => x.Order.Quantity) / Convert.ToInt32(_activeCall.LotSize);
                                    foreach (OrderTrio ott in _straddlePutOrderTrios)
                                    {
                                        OrderTrio putOrderTrio = TradeEntry(_activePut, currentTime, _activePut.LastPrice, ott.Order.Quantity / Convert.ToInt32(_activePut.LotSize), true, false, tag: "straddle");
                                        dl.DeActivateOrderTrio(ott);
                                    }
                                    ...
                                    OrderTrio ot = TradeEntry(_activeCall, currentTime, _activeCall.LastPrice, straddleCallQty, false, true, tag: "straddle");
                                    ot.Option = _activeCall;
                                    _straddleCallOrderTrios.Add(ot);

                                    ot = TradeEntry(_activePut, currentTime, _activePut.LastPrice, straddlePutQty, false, true, tag: "straddle");
                                    ot.Option = _activePut;
                                    _straddlePutOrderTrios.Add(ot);
        ...
        {
            if (currentTime.TimeOfDay >= new TimeSpan(15, 20, 00) || (_activeCall.LastPrice < 250/ _activeCall.LotSize || _activePut.LastPrice < 250 / _activeCall.LotSize))// && _referenceStraddleValue != 0)
            {
        ...
        private void CheckTargetProfit(DateTime currentTime)
        {
            if (_straddleCallOrderTrios.Count != 0 && _straddlePutOrderTrios.Count != 0)
            {
                //decimal costPrice = ((_straddleCallOrderTrios.Sum(x => x.Order.AveragePrice * x.Order.Quantity ))
                //        + (_straddlePutOrderTrios.Sum(x => x.Order.AveragePrice * x.Order.Quantity)))
                //-(((_hedgeCallOrderTrios.Sum(x => x.Order.AveragePrice * x.Order.Quantity))
                //        + (_hedgePutOrderTrios.Sum(x => x.Order.AveragePrice * x.Order.Quantity))));

                decimal lastOrderPrice = (_straddleCallOrderTrios.Last().Order.AveragePrice + _straddlePutOrderTrios.Last().Order.AveragePrice
                     - _hedgeCallOrderTrios.Last().Order.AveragePrice - _hedgePutOrderTrios.Last().Order.AveragePrice);


                decimal currentPrice = (_activeCall.LastPrice + _activePut.LastPrice - _hedgeCall.LastPrice - _hedgePut.LastPrice);

                if(currentPrice > lastOrderPrice * 1.1m && currentPrice - lastOrderPrice > 5 * 50/_strikePriceIncrement && _entriesCount < 4)
                { 
                //if (((currentPnl < _entriesCount * _targetProfit * -0.050m && _entriesCount < 5) || (currentPnl < _entriesCount * _targetProfit * -0.50m && _entriesCount < 8))
                //    && ((!_useHedge) || (_activeCall.Strike < _hedgeCall.Strike && _hedgePut.Strike < _activePut.Strike)))
                //{
                    _entriesCount++;
                    OrderTrio orderTrio;
                    if (_useHedge)
