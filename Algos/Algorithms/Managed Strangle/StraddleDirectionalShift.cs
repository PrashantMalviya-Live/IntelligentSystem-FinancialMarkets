...
{
    public class StraddleDirectionalShift : IZMQ
    {
        ...
        [field: NonSerialized]
        public delegate void OnOptionUniverseChangeHandler(StraddleDirectionalShift source);
        [field: NonSerialized]
        ...
        Dictionary<uint, DateTime?> InstrumentTimeofCross;
        ...
        private decimal _entryRatio = 1.4m;//1.2//1.45
        private decimal _reEntryRatio = 0.8m;//1.45
        //private decimal _entryRatioHigh = 1.4m;
        private decimal _exitRatio = 2.5m;//1.4//2.0
        ...
        public StraddleDirectionalShift(DateTime endTime, TimeSpan candleTimeSpan,
            ...
            decimal maxLossPerTrade = 0, HttpClient httpClient = null)
        {
            ...
            InstrumentTimeofCross = new Dictionary<uint, DateTime?>();
            ...
            try
            {
                ...
                lock (tradeLock)
                {
                    ...
                    if (token != _baseInstrumentToken)
                    {
                        ...
                        //if (OptionsDictionary.ContainsKey(token))
                        //{
                        ...
                        if (call.LastPrice * put.LastPrice != 0)
                        {
                            ...
                            else if (call.IsTraded && !put.IsTraded && call.LastPrice / put.LastPrice > _entryRatio)
                            {
                                TradeEntry(put, currentTime, put.LastPrice, _tradeQty, false);
                            }
                            else if (put.IsTraded && !call.IsTraded && put.LastPrice / call.LastPrice > _entryRatio)
                            {
                                TradeEntry(call, currentTime, call.LastPrice, _tradeQty, false);
                            }
                            //if (premiumRatio < _entryRatio)
                            //{
                            //    if (!call.IsTraded)
                            //    {
                            //        TradeEntry(call, currentTime, call.LastPrice, _tradeQty, false);
                            ...
                            else if (premiumRatio > _exitRatio)
                            {
                                ...
                                //if(call.IsTraded)
                                //TradeEntry(call, currentTime, call.LastPrice, _tradeQty, true);
                                //if(put.IsTraded)
                                //TradeEntry(put, currentTime, put.LastPrice, _tradeQty, true);
                                ...
                                if (call.LastPrice > put.LastPrice && call.IsTraded)
                                {
                                    ...
                                    if (InstrumentTimeofCross[call.InstrumentToken] != null && (DateTime.Now - InstrumentTimeofCross[call.InstrumentToken]) > new TimeSpan(0, 1, 0))
                                    {
                                        //buy back call
                                        TradeEntry(call, currentTime, call.LastPrice, _tradeQty, true);
                                        InstrumentTimeofCross[call.InstrumentToken] = null;
                                    }
                                    else if(InstrumentTimeofCross[call.InstrumentToken] == null)
                                    {
                                        InstrumentTimeofCross[call.InstrumentToken] = DateTime.Now;
                                    }
                                }
                                if (put.LastPrice > call.LastPrice && put.IsTraded)
                                {
                                    if (InstrumentTimeofCross[put.InstrumentToken] != null && (DateTime.Now - InstrumentTimeofCross[put.InstrumentToken]) > new TimeSpan(0, 1, 0))
                                    {
                                        //buy back put
                                        TradeEntry(put, currentTime, put.LastPrice, _tradeQty, true);
                                        InstrumentTimeofCross[put.InstrumentToken] = null;
                                    }
                                    else if (InstrumentTimeofCross[put.InstrumentToken] == null)
                                    {
                                        InstrumentTimeofCross[put.InstrumentToken] = DateTime.Now;
                                    }
                                }
                            }
                            if (premiumRatio > _exitRatio)
                            {
                                InstrumentTimeofCross[call.InstrumentToken] = null;
                                InstrumentTimeofCross[put.InstrumentToken] = null;
                            }
                            ...
                            //if (StraddleUniverse[0].All(x => x.Value != null) && OptionUniverse[0].Any(x => x.Value.InstrumentToken == token))
                            //{
                            ...
                            //        if (atmStrike != _activeCall.Strike && (activePE.LastPrice < activeCE.LastPrice ? activeCE.LastPrice < activePE.LastPrice * _thresholdRatio : activePE.LastPrice < activeCE.LastPrice * _thresholdRatio))
                            //        {
                            //            //_activeCall = OptionUniverse[(int)InstrumentType.CE][atmStrike];
                            //            //_activePut = OptionUniverse[(int)InstrumentType.PE][atmStrike];
        ...
        {
            if (currentTime.GetValueOrDefault(DateTime.Now).TimeOfDay >= new TimeSpan(15, 15, 00))// && _referenceStraddleValue != 0)
            {
                foreach(var tradedStraddle in StraddleUniverse)
                {
                    if(tradedStraddle.Value[0].IsTraded)
                    {
                        TradeEntry(tradedStraddle.Value[0], currentTime.Value, tradedStraddle.Value[0].LastPrice, _tradeQty, true);
                    }
                    if (tradedStraddle.Value[1].IsTraded)
                    {
        ...
        private Instrument TradeEntry(Instrument option, DateTime currentTime, decimal lastPrice, int tradeQty, bool buyOrder)
        {
            ...
            try
            {
                ...
                Order order = MarketOrders.PlaceOrder(_algoInstance, option.TradingSymbol, option.InstrumentType, lastPrice,
                    ...
                    algoIndex, currentTime, Constants.ORDER_TYPE_MARKET, broker: Constants.KOTAK, httpClient: _httpClient);
            ...
            try
            {
                    ...
                    || StraddleUniverse.Keys.Last() <= _baseInstrumentPrice + _maxDistanceFromBInstrument)
                {
                    ...
                    var OptionUniverse = dl.LoadCloseByStraddleOptions(_expiryDate, _baseInstrumentToken, 
                        _baseInstrumentPrice, _maxDistanceFromBInstrument, out optionsDictionary, out mappedTokens);
                    ...
                    foreach (var pair in optionsDictionary)
                    {
                        ...
                        InstrumentTimeofCross.TryAdd(pair.Key, null);
                    }
