...
{
    public class ManageStangleWithCutOff_Buy : IObserver<Tick[]>
    { 
        public ManageStangleWithCutOff_Buy()
        {
            LoadActiveData();
        }
        ...
        private void ReviewStrangle(InstrumentLinkedList optionList, Tick[] ticks, InstrumentLinkedList backupOptionList)
        {
            ...
            if (strangleNode.Prices.Count > 0)
            {
                    ...
                    CheckForReSale(strangleNode, Math.Max(Math.Abs(strangleNode.Prices.First()) * 0.20m, optionList.MaxLossPoints) * 0.5m))
                {
                    ...
                    strangleNode.Prices.Add(PlaceOrder(option.TradingSymbol, buyOrder: true, option.LastPrice));
                    ...
                    dl.UpdateListData(optionList.listID, option.InstrumentToken, strangleNode.Index, 0, option.InstrumentType,
                       0, lastOptionPrice, 0, AlgoIndex.StrangleWithCutoff_Buy);
                    ...
                    (option.LastPrice < strangleNode.Prices.First() - Math.Max(strangleNode.Prices.First() * 0.20m, optionList.MaxLossPoints) )))
                {
                    ...
                    strangleNode.Prices.Add(PlaceOrder(option.TradingSymbol, buyOrder: false, option.LastPrice)); //buy prices are negative and sell are positive. as sign denotes cashflow direction
        ...
        private bool CheckForReSale(InstrumentListNode optionNode, decimal safetyBand)
        {
            ...
            decimal firstbuyingPrice = Math.Abs(optionNode.Prices.First());
            bool buyback = false;
            if (currentOption.LastPrice >= firstbuyingPrice - safetyBand)
            {
                buyback = true;
            }
            return buyback;
        ...
        private void LoadActiveData()
        {
            ...
            //    new InstrumentLinkedList()
            //};
            AlgoIndex algoIndex = AlgoIndex.StrangleWithCutoff_Buy;
            DataLogic dl = new DataLogic();
            ...
            decimal ceMaxLossPercent = 0, int strangleId = 0)
        {
            if (currentPE.LastPrice * currentCE.LastPrice != 0)
            {
                ...
                if (strangleId == 0)
                {
                    ...
                    put.Prices.Add(PlaceOrder(currentPE.TradingSymbol, true, pePrice));
                    call.Prices.Add(PlaceOrder(currentCE.TradingSymbol, true, cePrice));

                    //Update Database
                    DataLogic dl = new DataLogic();
                    strangleId = dl.StoreStrangleData(currentCE.InstrumentToken, currentPE.InstrumentToken, call.Prices.Last(),
                       put.Prices.Last(), AlgoIndex.StrangleWithCutoff_Buy, ceMaxProfitPoints, ceMaxLossPoints, peMaxProfitPoints, peMaxLossPoints,
