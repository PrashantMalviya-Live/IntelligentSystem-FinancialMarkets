...
namespace Algorithms.Algorithms
{
    public class PriceDirectionalFutureOptions : ICZMQ
    {
        ...
        private decimal _referenceFuturePrice;
        ...
        private const int TARGET_MINIMUM_DISTANCE_FROM_FUTURE = 2600;
        private const int TARGET_OPTION_MAX_PREMIUM = 210;
        private const int TARGET_OPTION_MIN_PREMIUM = 100;
        ...
        public PriceDirectionalFutureOptions(
            ...
            decimal pnl, bool futureLong= true, bool doubleSide =false, int algoInstance = 0,  IHttpClientFactory httpClientFactory = null)
        {
        ...
        private void ActiveTradeIntraday(string channel, string data)
        {
            ...
            try
            {
                ...
                else if (_hedgeOption != null && _hedgeOption.Size != 0)
                {
                    ...
                    decimal pnl = (_future.CurrentPrice - _future.EntryPrice) * (_future.Size/1000) * (futureLong ? 1 : -1)
                    ...
                    decimal futurePnl = 0;
#if BACKTEST
                    futurePnl = (_future.CurrentPrice - _future.EntryPrice) * (_future.Size / 1000) * (futureLong ? 1 : -1);
                    ...
                    if (pnl > _targetProfit 
                        //|| ((futureLong? _future.CurrentPrice < _referenceFuturePrice : _future.CurrentPrice > _referenceFuturePrice) && pnl < _stopLoss * -1)
                        //|| pnl < _stopLoss * -1
                        // || (futurePnl < 0 && _targetOption.CurrentPremium < _targetOption.EntryPremium && pnl < _stopLoss * -1)
                        ||
        ...
        private void AddFuture(L1Orderbook orderbook, bool futureLong, int additionalFutureQty)
        {
            ...
            _pnl += ((_hedgeOption.CurrentPremium - _hedgeOption.EntryPremium) * (_hedgeOption.Size / 1000) - Convert.ToDecimal(order.PaidCommission)) - Convert.ToDecimal(_hedgeOption.PaidCommission);
        ...
        private void TakeTrade(L1Orderbook orderbook, bool futureLong, bool doubleSide)
        {
            ...
            if (orderbook.Symbol == "BTCUSD")
            {
                if (_future.Size == 0)
                {
                    ...
                    _referenceFuturePrice = _future.CurrentPrice;
            ...
            if (_futureIsSet)
            {
                ...
                //50% of target option
                _targetProfit = _targetOption.EntryPremium / 2;
        ...
        private bool CheckTargetOptionAvailability(bool futureLong, decimal futurePrice)
        {
            if (futureLong)
            {
                var higherCalls = callOptions.Where(x => x.Key >= futurePrice + TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM);
            ...
            else
            {
                ...
                var lowerputs = putOptions.Where(x => x.Key <= futurePrice - TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM);
        ...
        private KeyValuePair<decimal, decimal>? GetTargetOption(TradedCryptoFuture future, bool futureLong)
        {
            ...
            if (futureLong)
            {
                ...
                if(callOptions.Any(x => x.Key >= future.CurrentPrice + TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM))
                {
                    var higherCalls = callOptions.Where(x => x.Key >= future.CurrentPrice + TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM);
            ...
            else
            {
                ...
                if (putOptions.Any(x => x.Key <= future.CurrentPrice - TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM))
                {
                    var lowerputs = putOptions.Where(x => x.Key <= future.CurrentPrice - TARGET_MINIMUM_DISTANCE_FROM_FUTURE && x.Value <= TARGET_OPTION_MAX_PREMIUM && x.Value >= TARGET_OPTION_MIN_PREMIUM);
        ...
        private KeyValuePair<decimal,decimal>? GetHedgeOption(TradedCryptoFuture future, TradedCryptoOption tradedOption, bool futureLong)
        {
            ...
            if (futureLong)
            {
                ...
                //As far as possible with minimum profitability
                KeyValuePair<decimal, decimal> highestStrikeOption;
                var lowerPuts = putOptions.Where(x=>x.Key <= future.CurrentPrice);
                if (lowerPuts.Count() == 0)
                {
                    //return null;

                    highestStrikeOption = putOptions.OrderBy(x => x.Key > future.CurrentPrice).First();
                }
                else
                {
                    highestStrikeOption = lowerPuts.OrderByDescending(x => x.Key).First();
                }
            ...
            else
            {
                ...
                KeyValuePair<decimal, decimal> lowestStrikeOption;
                if (higherCalls.Count() == 0)
                {
                    //return null;
                    lowestStrikeOption = callOptions.OrderByDescending(x => x.Key < future.CurrentPrice).First();
                }
                else
                {
                    lowestStrikeOption = higherCalls.OrderBy(x => x.Key).First();
                }
