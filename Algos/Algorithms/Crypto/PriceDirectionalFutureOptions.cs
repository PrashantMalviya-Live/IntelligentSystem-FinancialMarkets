...
namespace Algorithms.Algorithms
{
    public class PriceDirectionalFutureOptions : ICZMQ
    {
            ...
            try
            {
                ...
                else if (_targetOption != null && _targetOption.Size != 0)  //(_hedgeOption != null && _hedgeOption.Size != 0)
                {
                    ...
                    decimal pnl = _doubleSide ?
                        ...
                        - 2 * (_targetOption.PaidCommission + _targetOption2.PaidCommission + _hedgeOption.PaidCommission + _hedgeOption2.PaidCommission)
                    ...
                    if (pnl > _targetProfit //|| _pnl < _stopLoss * -1
                        ||
                    ...
                    else
                    {
                        ...
                        if(_doubleSide)
                        {
                            ...
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption, true, _future);
                            ...
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption2, false, _future2);
                                additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;
                        ...
                        else
                        {
                            ...
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption, _futureLong, _future);
                                additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;
        ...
        /// <returns></returns>
        private int GetAdditionalFutureQuantity(TradedCryptoOption targetOption, bool futureLong, TradedCryptoFuture future)
        {
            int additionlQty = 0;
            if (futureLong)
            {
                ...
                while (true)
                {
                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size/1000) - (distanceFromLowerThreshold) * future.Size/1000;
                    ...
                    if (pnlAtThreshold > 15)
                    {
                        ...
                        int newFutureQty = Convert.ToInt32(incremental) + Convert.ToInt32(future.Size);

                        if(newFutureQty >= targetOption.Size)
                        {
                            additionlQty = Convert.ToInt32(targetOption.Size) - Convert.ToInt32(future.Size);
                        ...
                        decimal newFutureEntryPrice = (future.CurrentPrice * incremental + future.Size * future.EntryPrice) / newFutureQty;
            ...
            else
            {
                ...
                while (true)
                {
                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size/1000) - ((distanceFromUpperThreshold) * future.Size / 1000);
            ...
            TradedCryptoOption hedgeOption, TradedCryptoFuture future, TradedCryptoOption targetOption)
        {
            ...
            if (hedgeOption.Size != 0)
            {
                ...
                _pnl += ((hedgeOption.CurrentPremium - hedgeOption.EntryPremium) * (hedgeOption.Size/1000) - Convert.ToDecimal(order.PaidCommission));
        ...
        private decimal ExitOption(TradedCryptoOption option, DateTime currentTime)
        {
            ...
            if (option.Size > 0)
            {
              ...
              httpClient: _httpClientFactory == null ? null : _httpClientFactory.CreateClient());
                ...
                //cdl.UpdateOrder(_algoInstance, order);
                pnl = (Convert.ToDecimal(order.AverageFilledPrice) - option.EntryPremium)* (option.Size/1000) * (boughtOption ? 1 : -1) - Convert.ToDecimal(order.PaidCommission);
            }
            return pnl;
        }
