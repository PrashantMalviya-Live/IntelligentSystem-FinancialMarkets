...
namespace Algorithms.Algorithms
{
    public class PriceDirectionalFutureOptions : ICZMQ
    {
        ...
        private decimal _nextLongTriggerPoint;
        private decimal _nextShortTriggerPoint;
        ...
        private void ActiveTradeIntraday(string channel, string data)
        {
            ...
            try
            {
                ...
                else if (_targetOption != null && _targetOption.Size != 0)  //(_hedgeOption != null && _hedgeOption.Size != 0)
                {
                    ...
                    decimal futurePnl = (_future.CurrentPrice - _future.EntryPrice) * (_future.Size / 1000)
                    ...
                    if (pnl > _targetProfit 
                        || (pnl < _trailingStopLoss && _trailingStopLoss > 0) 
                        || (pnl < _stopLoss * -1)// && futurePnl < -50)
                        //|| futurePnl < -50
                        ||
                        ...
                        )
                    {
                        ...
                        dl.UpdateAlgoPnl(_algoInstance, pnl, currentime, futurePnl.ToString());
                    ...
                    else
                    {
                        ...
                        if(_doubleSide)
                        {
                            entryTicket = CheckEntryForAdditionalQty(_targetOption, true, _future);

                            //long future case
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption, true, _future, _future.Size, _nextLongTriggerPoint);
                                additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;

                                if (additionlQty > 0)
                                {
                                    AddTargetOption(orderbook, true, additionlQty, _hedgeOption, _future, _targetOption);
                                    
                                    additionlQty = GetAdditionalFutureQuantity(_targetOption, true, _future, _future.Size, _nextLongTriggerPoint);
                                    additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;
                            ...
                            entryTicket = CheckEntryForAdditionalQty(_targetOption2, false, _future2);

                            //long future case
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption2, false, _future2, _future2.Size, _nextShortTriggerPoint);
                                additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;

                                if (additionlQty > 0)
                                {
                                    AddTargetOption(orderbook, false, additionlQty, _hedgeOption2, _future2, _targetOption2);

                                    additionlQty = GetAdditionalFutureQuantity(_targetOption2, false, _future2, _future2.Size, _nextShortTriggerPoint);
                                    additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;
                                    if (additionlQty > 0 && _targetOption2.CurrentPremium > _targetOption2.EntryPremium)
                                    {
                                        _shortFutureCounter++;
                            ...
                            if(_future.Size > _futureQty && _future.EntryPrice > _future.CurrentPrice && _longFutureCounter >= 3)
                            {
                                ...
                                // ReduceFuture(orderbook, true, Convert.ToInt32(_future.Size - _futureQty), _hedgeOption, _future, _targetOption);
                                ...
                                //_nextLongTriggerPoint = GetNextTriggerPoint(_targetOption, true, _future, _future.CurrentPrice, correctSize);

                                //int additionlQty = 0;//GetAdditionalFutureQuantity(_targetOption, true, _future, correctSize, _nextLongTriggerPoint);

                                //correctSize += additionlQty;
                                //if (correctSize >= _futureQty && correctSize < _future.Size)
                                //{
                            ...
                            if (_future2.Size > _futureQty && _future2.EntryPrice < _future2.CurrentPrice && _shortFutureCounter >= 3)
                            {
                               ...
                               // ReduceFuture(orderbook, false, Convert.ToInt32(_future2.Size - _futureQty), _hedgeOption, _future2, _targetOption);
                                ...
                                //_nextShortTriggerPoint = GetNextTriggerPoint(_targetOption2, false, _future2, _future2.CurrentPrice, correctSize);

                                //int additionlQty = 0;// GetAdditionalFutureQuantity(_targetOption2, false, _future2, correctSize, _nextShortTriggerPoint);

                                //correctSize += additionlQty;

                                //if (correctSize >= _futureQty && correctSize < _future2.Size)
                                //{
                        ...
                        {
                            entryTicket = CheckEntryForAdditionalQty(_targetOption, _futureLong, _future);

                            //long future case
                            if (entryTicket)
                            {
                                int additionlQty = GetAdditionalFutureQuantity(_targetOption, _futureLong, _future, _future.Size, _futureLong?_nextLongTriggerPoint:_nextShortTriggerPoint);
                                additionlQty = additionlQty == 0 ? FUTURE_STEP_QUANTITY : additionlQty;


                                AddFuture(orderbook, _futureLong, additionlQty, _hedgeOption, _future, _targetOption);
                                //_nextTriggerPoint = CheckEntryForAdditionalQty(_future, _targetOption);
                            }
                        }
        ...
        /// <returns></returns>
        private int GetAdditionalFutureQuantity(TradedCryptoOption targetOption, bool futureLong, 
            TradedCryptoFuture future, decimal futureSize, decimal nextTriggerPoint)
        {
            int additionlQty = 0;
            if (futureLong)
            {
                ...
                while (true)
                {
                    //check pnl at threshold

                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size / 1000) - (distanceFromLowerThreshold * futureSize / 1000) + (future.CurrentPrice - future.EntryPrice) * futureSize / 1000;
                    ...
                    if (pnlAtThreshold > 15)
                    {
                        ...
                        if (newTriggerPoint > nextTriggerPoint + 200 && newTriggerPoint > future.CurrentPrice + 200)
                        {
            ...
            else
            {
                ...
                while (true)
                {
                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size / 1000) - (distanceFromUpperThreshold * futureSize / 1000) + (future.EntryPrice - future.CurrentPrice) * futureSize / 1000;
        ...
        private int GetCorrectFutureQuantity(TradedCryptoOption targetOption, bool futureLong, TradedCryptoFuture future, decimal futureSize)
        {
            ...
            if (futureLong)
            {
                ...
                while (true)
                {
                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size / 1000)
                        + ((future.CurrentPrice - future.EntryPrice) * future.Size / 1000)
                        - (distanceFromLowerThreshold) * futureSize / 1000;
                    ...
                    if (pnlAtThreshold > 15)
                    {
                        ...
                        //if (newTriggerPoint < _nextLongTriggerPoint + 200)
                        //{
                            additionlQty = incremental;
                            break;
                        //}
            ...
            else
            {
                ...
                {
                    decimal pnlAtThreshold = (targetOption.EntryPremium * targetOption.Size / 1000)
                        + ((future.EntryPrice - future.CurrentPrice) * future.Size / 1000)
                        - ((distanceFromUpperThreshold) * futureSize / 1000);
                    ...
                    if (pnlAtThreshold > 15)
                    {
                        ...
                        //if (newTriggerPoint > _nextShortTriggerPoint - 200)
                        //{
                            additionlQty = incremental;
                            break;
                        //}
        ...
        private bool CheckEntryForAdditionalQty(TradedCryptoOption targetOption, bool futureLong, TradedCryptoFuture future)
        {
            ...
            if (futureLong)
            {
                ...
                _nextLongTriggerPoint = (upperThreshold + lowerThreshold) / 2;


                return (futureLong ? future.CurrentPrice > _nextLongTriggerPoint : future.CurrentPrice < _nextLongTriggerPoint)
               && (future.Size < targetOption.Size)
               && (targetOption.CurrentPremium > targetOption.EntryPremium);
                //&& futureLong ? future.CurrentPrice > upperThreshold - 3000: future.CurrentPrice < lowerThreshold + 3000;
            }
            else
            {
                ...
                //Check if next trigger point has reached.
                _nextShortTriggerPoint = (upperThreshold + lowerThreshold) / 2;

                return (futureLong ? future.CurrentPrice > _nextShortTriggerPoint : future.CurrentPrice < _nextShortTriggerPoint)
               && (future.Size < targetOption.Size)
               && (targetOption.CurrentPremium > targetOption.EntryPremium);
                //&& futureLong ? future.CurrentPrice > upperThreshold - 3000: future.CurrentPrice < lowerThreshold + 3000;
            }
        ...
        private decimal GetNextTriggerPoint(TradedCryptoOption targetOption, bool futureLong, TradedCryptoFuture future, decimal fromFuturePrice, decimal futureSize)
        {
            decimal upperThreshold;
            decimal lowerThreshold;
            decimal nextTriggerPoint;

            if (futureLong)
            {
                upperThreshold = targetOption.Strike + (futureSize * (targetOption.Strike - fromFuturePrice) + (targetOption.EntryPremium * targetOption.Size)) / (targetOption.Size - futureSize==0?1: targetOption.Size - futureSize);
                lowerThreshold = fromFuturePrice - (targetOption.EntryPremium * targetOption.Size) / futureSize;

                //Check if next trigger point has reached.
                nextTriggerPoint = (upperThreshold + lowerThreshold) / 2;
            }
            else
            {
                lowerThreshold = targetOption.Strike - (futureSize * (fromFuturePrice - targetOption.Strike) + (targetOption.EntryPremium * targetOption.Size)) / (targetOption.Size - futureSize==0?1: targetOption.Size - futureSize);
                upperThreshold = fromFuturePrice + (targetOption.EntryPremium * targetOption.Size) / futureSize;
            ...
            TradedCryptoOption hedgeOption, TradedCryptoFuture future, TradedCryptoOption targetOption)
        {
            ...
            if (targetOption.CurrentPremium > targetOption.EntryPremium && targetOption.Size < 500)
            {
                ...
                CryptoOrder order = CryptoOrders.PlaceDEOrder(_algoInstance, targetOption.Symbol, Convert.ToInt32(size), productId: 0, false,
            ...
            TradedCryptoOption hedgeOption, TradedCryptoFuture future, TradedCryptoOption targetOption)
        {
            ...
            future.EntryPrice = Convert.ToDecimal(order.AverageFilledPrice);// (future.EntryPrice * future.Size - Convert.ToDecimal(order.AverageFilledPrice) * order.Size) / (future.Size - order.Size);
